...
ifeq ($(debug),0)
	CXXFLAGS += -ggdb -O3 -pipe -fdata-sections -ffunction-sections
else
#	CXXFLAGS += -ggdb -fprofile-arcs -ftest-coverage
	CXXFLAGS += -ggdb -fdata-sections -ffunction-sections
endif

CXXFLAGS += -Wall -Wno-unknown-pragmas -D_LINUX_ -DLOGFMTWITHTID -D$(centos_ver)  $(addprefix -I, $(INCDIR)) 

#LDFLAGS  +=  $(addprefix -L, $(LINKDIR)) -Wl,-rpath='$$ORIGIN:../bin' -Wl,--gc-sections
LDFLAGS  :=  $(addprefix -L, $(LINKDIR)) $(LDFLAGS) $(shell net-snmp-config --agent-libs) -Wl,-rpath='$$ORIGIN:../bin'
