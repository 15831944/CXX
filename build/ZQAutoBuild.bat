...
use Env qw(SSDIR ZQPROJSPATH TEMP ITVSDKPATH BuildVerNum);
...
if ($ARGV[0])
{
	&GetOptions(
               ...
               'onlygetsource'	=>	\$opt_onlygetsource,
		...
		$ONLYSOURCE = 1					if $opt_onlygetsource;
...
$SSDIR = $VSSROOT;
$ZQPROJSPATH = fixPathToOS($BUILD_ROOT."/ZQProjs");
$EMAILCMD = fixPathToOS($BUILD_ROOT."/ZQProjs/build/utils/blat.exe");
$MD5SUM = fixPathToOS($BUILD_ROOT."/ZQProjs/build/utils/md5sum.exe -b ");
...
#	updateVersionRC($profile);
	...
	if ($profile->{output_pkg})
	{
		...
		foreach $ofile (@$outputfiles)
		{
			...
			if ($newfilename eq "TianShanSymbols.zip")
			{
			$newfilename = TianShanSymbols.".V".$profile->{prod_ver}.".".$profile->{build_num}.".".zip;
			}
			else
			{
...
sub xmlparser($)
{
	...
	my $cmd = "$ZQPROJSPATH\\build\\utils\\xmlwf.exe  $xml_path\\*.xml >error";
	system($cmd);
