...
CacheStoreImpl::CacheStoreImpl(ZQ::common::Log& log, ZQ::common::Log& eventlog, const char* serviceName,
				...
				: _contentStore(contentStore), _adapter(contentStore._adapter), _log(log), _eventlog(eventlog), _thpool(thpool), _prxStreamService(pStreamSevice),
				  ...
				  _defaultTransferServerPort(12000), _catchUpRTminPercent(50), _catchUpRT(60000), _bProxySessionBook(true), _thirdPartyCache(false)
{
...
void CacheStoreImpl::openDB(const char* runtimeDBPath)
{
	...
	if (!_thirdPartyCache)
	{
		_log(ZQ::common::Log::L_INFO, FLOGFMT(CacheStore, openDB, "ignore DB[CacheTask] per taking 3rd-party cache"));
		return;
	}
...
#pragma message ( __TODO__ "quit if the subtotal latency (stampStartForwarding - stampStart) has exceeded a number of msec")
			...
			else _log(ZQ::common::Log::L_DEBUG, EXPSTRMFMT("%d-th ContentEdge[%s]/[%d] has NULL store"), i+1, topCand.csd.netId.c_str(), topCand.csd.loadStream);
...
void CacheStoreImpl::penalize(CacheStoreDsptr& store, int penaltyToCharge)
{
	...
	if (store.penalty <0)
		store.penalty =0;
	else if (_penaltyMax >0 && store.penalty > (int)_penaltyMax)
...
void CacheStoreImpl::OnForwardFailed(std::string storeNetId, int penaltyToCharge)
{
	...
	int pen =1;
	_log(ZQ::common::Log::L_DEBUG, FLOGFMT(CacheStore, OnForwardFailed, "charging neighor[%s] by penalty[%d]"), storeNetId.c_str(), pen);
	penalize(it->second, pen);
}
