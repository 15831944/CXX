...
{
	storelog(ZQ::common::Log::L_INFO, CLOGFMT(CacheStoreProbe, "probe starts"));
	...
	storelog(ZQ::common::Log::L_INFO, CLOGFMT(CacheStoreProbe, "quits"));
	return 0;
}
...
int CacheStoreProbe::OnCapturedData(const void* data, const int datalen, ZQ::common::InetHostAddress source, int sport)
{
	...
	if (MSGCODE_CacheStore != pMsg->msgCode)
		return datalen;
		...
		try {
			...
			if (_store._neighbors.end() != it)
			{
				...
				if (it->second.penalty >0)
					it->second.penalty--;
				else it->second.penalty =0;

				if (it->second.stampStartup < pMsg->stampStartup)
				{
				...
				if (CacheStoreImpl::sfLoggingDomainMessages & _store._flags)
					storelog(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheStoreProbe, "OnCapturedData() peer[%s]: sessInterface[%s] endpoint[%s] updated, load[%d/%d/%d]"),
...
{
	std::string bindaddr= _store._groupBind.getHostAddress(), grpaddr= _store._groupAddr.getHostAddress();
	storelog(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheStoreBarker, "open group barker: bind=%s group=[%s]:%d"), bindaddr.c_str(), grpaddr.c_str(), _store._groupPort);
...
{
	storelog(ZQ::common::Log::L_INFO, CLOGFMT(CacheStoreBarker, "barker starts"));
	while (!_bQuit)
	{
		try
		{
			...
			if (CacheStoreImpl::sfEnableDomainMessageDump & _store._flags)
				...
				storelog(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheStoreBarker, "CacheStoreBarker() advertizing this[%s] to domain[%s]: load[%d/%d/%d]"), msg.netId, msg.domain, msg.loadStream, msg.loadImport, msg.loadCacheWrite);
	...
	storelog(ZQ::common::Log::L_INFO, CLOGFMT(CacheStoreBarker, "barker quits"));
	return 0;
}
