...
void ContentStateBase::doPopulateFromFs(const ::Ice::Current& c)
{
	...
	case ::TianShanIce::Storage::csNotProvisioned:
		...
		if (populateError)
		{
			MOLOG(ZQ::common::Log::L_WARNING, ContentStateLOGFMT(ContentState, "doPopulateFromFs() quit due to portal populateEr"));
			return;
		}
...
void ContentStateNotProvisioned::OnTimer(const ::Ice::Current& c)
{
	...
	if (!_content.bDirtyAttrs && (stampNow - _content.stampCreated) > _store._timeoutNotProvisioned)
	{
