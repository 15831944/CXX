...
int A3Request::sendRequest(const char* uri, std::string& buffer) 
{
	std::string url = _strHost + (uri ? uri :""); 
	if (0 != _http.httpConnect(url.c_str())) 
	{
		A3REQLOG(ZQ::common::Log::L_WARNING, "sendRequest() failed at connecting endpoint[%s]", url.c_str());
		_http.uninit();
...
int A3Request::getContentInfo(MessageCtx& msgCtx) 
{
	...
	int code = sendRequest("GetContentInfo", str);
	if(code == 200) 
	{
