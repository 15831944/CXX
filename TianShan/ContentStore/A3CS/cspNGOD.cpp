...
void ContentStoreImpl::getStorageSpace(ContentStoreImpl& store, uint32& freeMB, uint32& totalMB, const char* rootPath)
{
	...
	{
		store._log(ZQ::common::Log::L_ERROR,"ContentStoreImpl::getStorageSpace() top path is NULL");
		freeMB = 0;
		totalMB = 0;
		return;
	}
	...
	{
		store._log(ZQ::common::Log::L_ERROR,"ContentStoreImpl::getStorageSpace() request error, %s",req.getStatusMessage().c_str());
		freeMB = 0;
		totalMB = 0;
	}
}
...
uint64 ContentStoreImpl::checkResidentialStatus(ContentStoreImpl& store, uint64 flagsToTest, ContentImpl::Ptr pContent, const ::std::string& contentFullName, const ::std::string& mainFilePathname)
{
	...
	store._log(ZQ::common::Log::L_DEBUG, CLOGFMT(NGODStorePortal, "checkResidentialStatus() parameter mainFilePathname[%s] associated target[%s] content[%s]"), mainFilePathname.c_str(), targetVolName.c_str(), shortContentName.c_str());
...
//			throw (::TianShanIce::InvalidParameter, ::TianShanIce::InvalidStateOfArt) 
{
	...
	if (NULL == store._ctxPortal)
		ZQTianShan::_IceThrow <TianShanIce::InvalidParameter> (store._log, EXPFMT(NGODStorePortal, 1004, "listMainFiles() illegal portal context"));
	...
	while (!pPortalCtx->bQuit())
	{
...

		{
			...
			if (pPortalCtx->_volumeMap.end() == pPortalCtx->_volumeMap.find(volName))
				ZQTianShan::_IceThrow <TianShanIce::InvalidParameter> (store._log, EXPFMT(NGODStorePortal, 1001, "listMainFiles() volume[%s] not found, rootPath=%s"), volName.c_str(), rootPath);
			
			NGODStorePortal::VolumeInfo& vi = pPortalCtx->_volumeMap[volName];
			if (vi.stampLastSync == 0 )
			{
				bNeedWait = true;
				store._log(ZQ::common::Log::L_WARNING, CLOGFMT(NGODStorePortal, "listMainFiles() volume[%s] has never been sync-ed, pending this query..."), volName.c_str());
//				pPortalCtx->_hStartList.reset();
	...
	if (!bListed)
		ZQTianShan::_IceThrow <TianShanIce::ServerError> (store._log, EXPFMT(NGODStorePortal, 1002, "listMainFiles() volume[%s] failed to list the recent contents"), volName.c_str());

	store._log(ZQ::common::Log::L_DEBUG, CLOGFMT(NGODStorePortal, "listMainFiles() volume[%s] found %d contents, took "FMT64"ms"), volName.c_str(), ret.size(), ZQTianShan::now() -stamp1);
...
bool ContentStoreImpl::populateAttrsFromFile(ContentStoreImpl& store, ContentImpl& content, const ::std::string& mainFilePathname)
{
	...
	{
		store._log(ZQ::common::Log::L_ERROR, CLOGFMT(NGODStorePortal, "populateAttrsFromFile() mainfile is NULL"));
		return false;
	}
	...
	{
		store._log(ZQ::common::Log::L_ERROR, CLOGFMT(NGODStorePortal, "populateAttrsFromFile() invalid mainFilePathname[%s] with no " FNSEPS), mainFilePathname.c_str());
		return false;
	}

	store._log(ZQ::common::Log::L_DEBUG,CLOGFMT(NGODStorePortal, "populateAttrsFromFile() enter"));
	...
	{
		store._log(ZQ::common::Log::L_ERROR, CLOGFMT(NGODStorePortal, "populateAttrsFromFile() mainFilePathname[%s] of targetVol[%s] not found"), mainFilePathname.c_str(), targetVolName.c_str());
		return false;
	}
	...
	/*
	{
		...
		if (pPortalCtx->_volumeMap.end() == itVol)
		{
			store._log(ZQ::common::Log::L_ERROR, CLOGFMT(NGODStorePortal, "populateAttrsFromFile() mainFilePathname[%s]: volume[%s] not found"), mainFilePathname.c_str(), volName.c_str());
			return false;
		}
		...
		{
			store._log(ZQ::common::Log::L_ERROR, CLOGFMT(NGODStorePortal, "populateAttrsFromFile() mainFilePathname[%s]: content[%s] not found in volume[%s]"), mainFilePathname.c_str(), contentName.c_str(), volName.c_str());
			return false;
		}
	...
	*/
	...
	store._log(ZQ::common::Log::L_DEBUG,CLOGFMT(NGODStorePortal, "populateAttrsFromFile() updating content[%s] metadata"), contentName.c_str());
 ...
 //                  TianShanIce::ServerError, ::TianShanIce::InvalidStateOfArt)
{
	if (NULL == store._ctxPortal)
	{
		store._log(ZQ::common::Log::L_ERROR, CLOGFMT(NGODStorePortal, "submitProvision() illegal portal context: NULL"));
		return NULL;
	}
...
//	std::string strName = content.ident.name.substr(content.identVolume.name.length()+1);//content.getName(Ice::Current());
	...
	store._log(ZQ::common::Log::L_DEBUG, CLOGFMT(NGODStorePortal, "submitProvision() parsed sourceUrl[%s]: url[%s] user[%s] passwd[%s]"), sourceUrl.c_str(), msgData.params["sourceURL"].c_str(), msgData.params["userName"].c_str(), msgData.params["password"].c_str());
	...
	{
		store._log(ZQ::common::Log::L_DEBUG, CLOGFMT(NGODStorePortal, "content[%s] submitProvision() customizing for Huawei"), content.ident.name.c_str());
		if (!bCapture)
		{
	...
	{
		store._log(ZQ::common::Log::L_DEBUG, CLOGFMT(NGODStorePortal, "content[%s] submitProvision() customizing for Concurrent"), content.ident.name.c_str());
	...
	{
		store._log(ZQ::common::Log::L_DEBUG, CLOGFMT(NGODStorePortal, "submitProvision() adding portal record vol[%s]-target[%s] content[%s]"), 
		...
		if (pPortalCtx->_volumeMap.end() == itVol)
		{
			store._log(ZQ::common::Log::L_ERROR, CLOGFMT(NGODStorePortal, "submitProvision() content[%s] portal record volume[%s] target[%s] not found"), content.ident.name.c_str(), volName.c_str(), targetVolName.c_str());
			return NULL;
		}
	...
	store._log(ZQ::common::Log::L_DEBUG, CLOGFMT(NGODStorePortal, "content[%s] submitProvision() issuing A3 request"), content.ident.name.c_str());
	A3Request req(*pPortalCtx);
	...
	{
		store._log(ZQ::common::Log::L_INFO, CLOGFMT(NGODStorePortal, "submitProvision() provideID[%s] assetID[%s] volumeName[%s] captureStart[%s] captureEnd[%s] transferBitRate[%s] sourceURL[%s] responseURL[%s] ,provisioning submitted [%s]"), 
	...
	if (bRecordAdded)
	{
		store._log(ZQ::common::Log::L_DEBUG, CLOGFMT(NGODStorePortal, "submitProvision() withdrawing portal record vol[%s]-target[%s] content[%s] per TransferContent failed[%d]"), 
...
std::string ContentStoreImpl::getExportURL(ContentStoreImpl& store, ContentImpl& content, const ::TianShanIce::ContentProvision::ProvisionContentKey& contentkey, const ::std::string& transferProtocol, ::Ice::Int transferBitrate, ::Ice::Int& ttl, ::TianShanIce::Properties& params)
{
	...
	if (NULL == store._ctxPortal)
	{
		store._log(ZQ::common::Log::L_ERROR, CLOGFMT(NGODStorePortal, "getExportURL() illegal portal context: NULL"));
		return url;
	}
...
//	msgData.transferBitRate = content.getBitRate(Ice::Current());
	...
	store._log(ZQ::common::Log::L_DEBUG, CLOGFMT(NGODStorePortal, "getExportURL() content[%s] querying SS[%s] for PID[%s] PAID[%s]"), contentName.c_str(), A3Url.c_str(), msgData.params["assetID"].c_str(), msgData.params["providerID"].c_str());
	...
	{
		params.clear();
		store._log(ZQ::common::Log::L_INFO, CLOGFMT(NGODStorePortal, "getExportURL() content[%s] succeeded: %s"), contentName.c_str(), statusMsg.c_str());
...
//		throw (::TianShanIce::ServerError, ::TianShanIce::InvalidStateOfArt)
{
	if (NULL == store._ctxPortal)
	{
		store._log(ZQ::common::Log::L_ERROR, CLOGFMT(NGODStorePortal, "cancelProvision() illegal portal context: NULL"));
		return;
	}
	...
	std::string A3Url = pPortalCtx->getA3Url();
	store._log(ZQ::common::Log::L_DEBUG, CLOGFMT(NGODStorePortal, "cancelProvision() content[%s] querying SS[%s] for PID[%s] PAID[%s]"), contentName.c_str(), A3Url.c_str(), msgData.params["assetID"].c_str(), msgData.params["providerID"].c_str());
	...
	if (errorCode >=200 && errorCode < 300) //ok
	{
		store._log(ZQ::common::Log::L_INFO, CLOGFMT(NGODStorePortal, "cancelProvision() content[%s] succeeded: %s"), contentName.c_str(), statusMsg.c_str());
		return;
	}
