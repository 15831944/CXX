...
int A3Listener::OnHttpMessage(const MsgInfo& info, const std::string& contentBody)
{
	...
	{
		A3RESPLOG(ZQ::common::Log::L_ERROR,"A3Listener::handleMsg() message is NULL,from [%s:%d]",info.ip.c_str(), info.port);
		return MSG_INTERNAL_ERR;
	}
	A3RESPLOG(ZQ::common::Log::L_DEBUG,"A3Listener::OnHttpMessage() Have a  message,uri [%s], from [%s:%d]", info.uri.c_str(), info.ip.c_str(), info.port);
	...
	{	
		...
		{
			A3RESPLOG(ZQ::common::Log::L_ERROR,"A3Listener::handleMsg() parse TransferStatus message failed,from [%s:%d]",info.ip.c_str(), info.port);
			return MSG_INTERNAL_ERR;
		}
	...
	else if(stricmp(TYPE_CHECKSUM, strType.c_str()) == 0)//content checksum message
	{
		...
		{
			A3RESPLOG(ZQ::common::Log::L_ERROR,"A3Listener::handleMsg() parse ContentChecksum message failed, from [%s:%d]", info.ip.c_str(), info.port);
			return MSG_INTERNAL_ERR;
		}
	...
	{
		A3RESPLOG(ZQ::common::Log::L_ERROR,"A3Listener::handleMsg() do not know the message type [%s],from [%s:%d]",strType.c_str(), info.ip.c_str(), info.port);
		return MSG_INTERNAL_ERR;
	}
...
protected:
	...
	virtual int run(void)
	{
...

		{
			...
			if (200 != A3Req.request(A3Request::A3_GetVolumeInfo, msgData)) // && atoi(msgData.params["state"].c_str()) == A3Request::OPERATIONAL
			{
		...
		int64 stampSync=0;
		{
			_csp._store._log(ZQ::common::Log::L_DEBUG, CLOGFMT(VolumeSyncCmd, "quering GetContentInfo of volume[%s]"), _volName.c_str());
	...
	while(!_bQuit)
	{
		...
		if (_interval >0)
		{
			...
			if (dirtyVolumes.size() >0)
				cslog(ZQ::common::Log::L_DEBUG, CLOGFMT(NGODStorePortal, "found [%d] dirty volumes of [%d] total volumes"), dirtyVolumes.size(), _volumeMap.size());
		}
...
{	
	cslog(ZQ::common::Log::L_DEBUG, CLOGFMT(NGODStorePortal, "OnTransferStatus() from client[%s], content status providerID[%s], assetID[%s], volumeName[%s], state[%s], percentComplete[%s%%]"), 
	...
	
	{
		...
		{
			_store._log(ZQ::common::Log::L_WARNING, CLOGFMT(NGODStorePortal, "OnTransferStatus() from client[%s], unknown target volume[%s]"), info.ip.c_str(), targetVolName.c_str());
		...
		{
			_store._log(ZQ::common::Log::L_WARNING, CLOGFMT(NGODStorePortal, "OnTransferStatus() from client[%s], couldnot find volume record for target volume[%s]"), info.ip.c_str(), targetVolName.c_str());
			return MSG_UNKNOWN_PAID;
		}

		_store._log(ZQ::common::Log::L_DEBUG, CLOGFMT(NGODStorePortal, "OnTransferStatus() from client[%s], associated volume[%s] for target[%s]"), info.ip.c_str(), volName.c_str(), targetVolName.c_str());
	...
	//		_store._log(ZQ::common::Log::L_WARNING, CLOGFMT(NGODStorePortal, "OnTransferStatus() from client[%s], content[%s] not found in volume[%s]"), info.ip.c_str(), contentName.c_str(), msgData.params["volumeName"].c_str());
	//		return MSG_UNKNOWN_PAID;
		}
		...
		if (bNeedPopulate)
			_store._log(ZQ::common::Log::L_INFO, CLOGFMT(NGODStorePortal, "OnTransferStatus() from client[%s], volume[%s] content[%s] state change detected: %s -> %s"), info.ip.c_str(),
...
*/
	...
	else if ( strTransferState == A3_STATE_STREAMABLE)
	{
		...
		if (oldCI.contentState != strTransferState)
		{
			...
			if (_store._streamableLength >0)
			{
				...
				_store._log(ZQ::common::Log::L_DEBUG, CLOGFMT(NGODStorePortal, "OnTransferStatus() %s content[%s] set " METADATA_PlayTime "[%s] per streamableLength[%d]"),
	...
	else if (strTransferState == A3_STATE_PENDING)
	{
		return MSG_OK;
	}
	else
	{
		_store._log(ZQ::common::Log::L_WARNING, CLOGFMT(NGODStorePortal, "OnTransferStatus(), unkown state: %s"), strTransferState.c_str());
		return MSG_OK;
	}
	...
	{
		_store._log(ZQ::common::Log::L_WARNING, CLOGFMT(NGODStorePortal, "content[%s] OnProvisionEvent() vol[%s] caught exception[%s] in OnTransferStatus"), contentName.c_str(), volName.c_str(), ex.ice_name().c_str());
	}
...
{
	cslog(ZQ::common::Log::L_DEBUG, CLOGFMT(NGODStorePortal, "OnContentChecksum() from client[%s], content checksum providerID[%s], assetID[%s], volumeName[%s], checksum[%s], md5DateTime[%s], resultCode[%s]"), 
	...
	if (_volTargetNameIdx.end() == _volTargetNameIdx.find(targetVolName))
	{
		_store._log(ZQ::common::Log::L_WARNING, CLOGFMT(NGODStorePortal, "OnContentChecksum() from client[%s], unknown target volume[%s]"), info.ip.c_str(), targetVolName.c_str());
		return MSG_UNKNOWN_PAID;
	}
	...
	if (_volumeMap.end() == itVol)
	{
		_store._log(ZQ::common::Log::L_WARNING, CLOGFMT(NGODStorePortal, "OnContentChecksum() from client[%s], could not find volume record for target volume[%s]"), info.ip.c_str(), targetVolName.c_str());
		return MSG_UNKNOWN_PAID;
	}
	...
	if (itVol->second.contents.end() == itCont)
	{
		_store._log(ZQ::common::Log::L_WARNING, CLOGFMT(NGODStorePortal, "OnContentChecksum() from client[%s], content[%s] not found in volume[%s]"), info.ip.c_str(), contentName.c_str(), msgData.params["volumeName"].c_str());
		return MSG_UNKNOWN_PAID;
	}
...
bool NGODStorePortal::queryInfoByContent(ContentInfo& ci, const std::string& contentName, const std::string& volumeName, bool byTargetVol)
{
	...
	std::string a3volume;
	{
		...
		{
			_store._log(ZQ::common::Log::L_ERROR, CLOGFMT(NGODStorePortal, "queryInfoByContent() unknown volume[%s]"), volName.c_str());
			return false;
		}

		a3volume = it->second.targetName;
		ContentMap::iterator itCont = it->second.contents.find(contName);
		if (it->second.contents.end() != itCont && itCont->second.stampFromServer > (ZQTianShan::now() - 10*60*1000))
		{
			_store._log(ZQ::common::Log::L_DEBUG, CLOGFMT(NGODStorePortal, "queryInfoByContent() found cached info for content[%s] of volume[%s]"), contName.c_str(), a3volume.c_str());
			ci = itCont->second;
			return true;
		}
	}
	...
	_store._log(ZQ::common::Log::L_DEBUG, CLOGFMT(NGODStorePortal, "queryInfoByContent() issuing GetContentInfo for volume[%s] PID[%s] PAID[%s]"), a3volume.c_str(), PID.c_str(), PAID.c_str());
	A3Request::MessageCtx msgCInfo;
	...
	std::string a3url = _defaultA3Url;
	{
		...
		if( errorCode < 200 || errorCode >= 300)
		{
			_store._log(ZQ::common::Log::L_ERROR, CLOGFMT(VolumeSyncCmd, "GetContentInfo for volume[%s] PID[%s] PAID[%s] failed: %s"), a3volume.c_str(), PID.c_str(), PAID.c_str(), A3Req.getStatusMessage().c_str());
			return false;
		}
	} // free A3Req that may be very large and no more useful 
	...
	_store._log(ZQ::common::Log::L_ERROR, CLOGFMT(VolumeSyncCmd, "GetContentInfo for volume[%s] PID[%s] PAID[%s] failed: server didn't respond the matched content"), a3volume.c_str(), PID.c_str(), PAID.c_str());
	return false;
}
