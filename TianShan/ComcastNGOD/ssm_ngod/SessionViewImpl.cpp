...
void SessionViewImpl::updateSOPXML(const std::string strFile)
{
	...
	sopOut << "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
		...
		<< "-->\n"
		<< "<SOPRestriction enableWarmup=\"" << sopConfig.sopRestrict.enableWarmup << "\" retryCount=\"" << sopConfig.sopRestrict.retryCount << "\" "
		<< "penaltyOfTimeout=\"" << sopConfig.sopRestrict.timeoutPenalty 
		<< "\" maxPenaltyValue=\"" << sopConfig.sopRestrict.maxPenaltyValue
		<< "\" penaltyMask=\""<< sopConfig.sopRestrict.penaltyEnableMask << "\" "
		<< "\" execMask=\""<< sopConfig.sopRestrict.execMask << "\" "
		<< "streamerQueryInterval=\"" << sopConfig.sopRestrict.streamerQueryInterval << "\" replicaUpdateInterval=\"" << sopConfig.sopRestrict.replicaUpdateInterval << "\" >\n"
	...
	{
		sopOut	<< "  <sop name=\"" << itSop->second.name 
			<< "\" serviceGroup=\"" << itSop->second.serviceGroup 
			<< "\" sopGroup= \"" << itSop->second.sopGroupName <<"\" >\n";
		NGOD::SOPRestriction::SopHolder& sopData = itSop->second;			
		std::vector<NGOD::Sop::StreamerHolder>& streamers = sopData.streamerDatas;				
		std::vector<NGOD::Sop::StreamerHolder>::iterator itStreamer = streamers.begin ();		
		for ( ; itStreamer != streamers.end() ; itStreamer ++ )
		{
			sopOut << "    <streamer netId=\"" << itStreamer->netId << "\" serviceEndpoint=\"" << itStreamer->serviceEndpoint <<  "\" volume=\"" << itStreamer->volume << "\" totalBW=\"" << itStreamer->totalBW << "\" maxStream=\"" << itStreamer->maxStream << "\" importChannel=\"" << itStreamer->importChannel << "\" enabled=\"" << itStreamer->enabled << "\" />\n";
		}
		sopOut << "  </sop>\n";
	}
