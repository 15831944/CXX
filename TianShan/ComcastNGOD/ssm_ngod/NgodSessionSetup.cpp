...
bool NgodSessionI::renderStream( const NgodRequestSetupPtr& request , const StreamerSelection& streamerSel ,bool& bSkipVolume )
{
	...
	MLOG(ZQ::common::Log::L_INFO,SESSFMT(NgodSessionI,"successfully render stream[%s]"), PROXY2STR(mStream).c_str() );
	return true;
}
...
int NgodSessionI::processSetup( const NgodRequestSetupPtr& request )
{
	...
	MLOG(ZQ::common::Log::L_INFO,SESSFMT(NgodSessionI,"setup with sop[%s] volume[%s] bandwidth[%lld]"),
	...
	for( int i = 0 ; i <= retryCount ; i++ )
	{
		...
		MLOG(ZQ::common::Log::L_DEBUG,SESSFMT(NgodSessionI,"got a streamer[%s]"), streamerSel.getSelectedStreamerNetId().c_str() );
		bFirstTry = false;
		...
		if ( !renderStream( request , streamerSel , bSkipVolume ) )
		{
			...
			{
				errlog(request, errorcodeServiceUnavail, "failed to render stream[%s] on [%s]",
					PROXY2STR(mStream).c_str(),	streamerSel.getSelectedStreamerNetId().c_str() );
			}
