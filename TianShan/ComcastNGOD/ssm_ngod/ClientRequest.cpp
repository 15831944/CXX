...
{	
	...
	MLOG(ZQ::common::Log::L_DEBUG, SESSFMT(NgodRequestSetup, "playlist size[%d] clips[%d:%d] primary[%.3f~%.3f]"),
...
void NgodRequestSetup::parsePlaylistItem( const std::string& str )
{
	...
	if( tmpVec.size() >= 3 )
	{
		...
		for ( int i = 2 ; i< itempMaxSize ;i++ )
		{
			...
			{
				const std::string& range = tmpVec[2];
				info.range = range;
				std::string::size_type posDash = range.find('-');
				if( POSOK( posDash ))
				{
					info.cuein	= convertToNumberWithPrototcol( range.substr( 0 , posDash)  , (NgodProtoclVersionCode)protocolVerCode );
					info.cueout	= convertToNumberWithPrototcol( range.substr( posDash + 1 ) , (NgodProtoclVersionCode)protocolVerCode );
				}
				else
				{
					info.cuein	= convertToNumberWithPrototcol( range , (NgodProtoclVersionCode)protocolVerCode );
				}		
			}
		}		
	}
		...
		for(; iter != ngodConfig.adsReplacment.providers.end(); iter++)
		{
			if (!stricmp(info.pid.c_str(),iter->pid.c_str())) // ads
			{
				...
				if( ngodConfig.adsReplacment.playOnce)
				{
					...
					MLOG(ZQ::common::Log::L_DEBUG,SESSFMT(PrepareItemInfoCommand,"Ads[%s#%s] playOnce"), info.pid.c_str() , info.paid.c_str() );
				}
			}
		}
	}
