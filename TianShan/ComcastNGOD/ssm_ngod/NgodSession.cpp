	...
	case TianShanIce::stOutOfService:
		{
			try
			{
				MLOG(ZQ::common::Log::L_WARNING,CLOGFMT(NgodSessionI,"onTimer() meet stOutOfService, destroy current session"));
				destroy(NULL);
			}
...
::Ice::Int NgodSessionI::processRequest(const ::NGOD::NgodClientRequestPtr& request, const ::Ice::Current& c) 
{
	try
	{
		...
		case requestSETUP:
			{
				NgodRequestSetupPtr setuprequest = NgodRequestSetupPtr::dynamicCast(request);
				return processSetup(setuprequest);
			}
			break;
		case requestPLAY:
			{
				NgodRequestPlayPtr setuprequest = NgodRequestPlayPtr::dynamicCast(request);
				return processPlay(setuprequest);
			}
			break;
		case requestPAUSE:
			{
				NgodRequestPausePtr setuprequest = NgodRequestPausePtr::dynamicCast(request);
				return processPause(setuprequest);
			}
			break;
		case requestTEARDOWN:
			{
				NgodRequestTeardownPtr setuprequest = NgodRequestTeardownPtr::dynamicCast(request);
				return processTeardown(setuprequest);
			}
			break;
		case requestGETPARAMETER:
			{
				NgodRequestGetParameterPtr setuprequest = NgodRequestGetParameterPtr::dynamicCast(request);
				return processGetParam(setuprequest);
			}
			break;
		case requestPING:
			{
		...
		case requestRESPONSE:
			{
				NgodRequestAnnounceResponsePtr announceResponse = NgodRequestAnnounceResponsePtr::dynamicCast(request);
				return processAnnounceResp(announceResponse);
			}
			break;
		default:
			assert(false);
...
void NgodSessionI::errlog( const NgodClientRequestIPtr& request , int errCode , const char* fmt ... )
{
	...
	{
		ELOG(ZQ::common::Log::L_INFO, EVENTLOGFMT(NgodSessionI,"state(%s)"), errorCodeTransformer(errCode) );
		ELOG(ZQ::common::Log::L_ERROR, EVENTLOGFMT(NgodSessionI,"%s"), szLocalBuffer );
	}
#if defined ZQ_OS_MSWIN
...
void NgodSessionI::addTSEventRecord( const NGOD::StreamEventAttr& a)
{
...
void NgodSessionI::addRPEventRecord( const NGOD::StreamEventAttr& a)
{
...
bool NgodSessionI::getCachedSsInfo( StreamSessionInfo& info ) {
	RLock sync(*this);
	int64 timeLast = -1;
	ZQTianShan::Util::getPropertyDataWithDefault(mProps, "streaminfo_cache_timestamp", -1, timeLast);
	if( timeLast < 0 )
		return false;
	int64 timeDelta = ZQ::common::now() - timeLast;
...
void NgodSessionI::addTDEventRecord( const std::string& reason , const std::string& strNpt , const std::string& stopIdx )
{
	...
	span.start();
	{
		...
		addEventRecord( sessionEvent );
	}
	if( span.stop() > 100 )
	{
		MLOG(ZQ::common::Log::L_DEBUG,SESSFMT(NgodSessionI,"addTDEventRecord() use [%lld] to add EndEvent record"),
			span.span() );
	}
}

void NgodSessionI::addSCEventRecord( const std::string& targetState , const std::string& newState,
									...
									int status, const std::string& reqRange , const std::string& reqScale )
{
	...
	if( span.stop() > 100 )
	{
		MLOG(ZQ::common::Log::L_DEBUG,SESSFMT(NgodSessionI,"addSCEventRecord() use [%lld] to add stream/user event record"),
			span.span() );
	}
}

void NgodSessionI::addEventRecord( const SessionEventRecord& record )
{
	WLock sync(*this);
