...
void showHelp()
{
	std::cout << "Usage: A3MessageClient <http://ip:port> [bindip] [port]\n"
		<< "Options:\n"
		<< "set <user.>key=value\n"       
		...
		<< "        key=user.key         (TransferConent ContentAsset metedata)\n"
...
void set(std::vector<std::string>&strParamter)
{
	...
	if(strKey.size() > 5 && strKey.substr(0,5) == "user.")
	{
		std::string propkey = strKey.substr(5);
