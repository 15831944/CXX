...
*/
		void A3MessageHandler::GetVolumeInfo(const std::string reqID, const CRG::IRequest* request, CRG::IResponse* response)
		{
			...
			if(_env._backStoreType == backContentLib)
			{
				...
				{
					setReponseWithoutBody(reqID,request, response, 404, GVITATUS_404);
					return;
				}
				const TianShanIce::Repository::ContentLibPrx&  clPrx = _env.connectToContentLib();
				if(!clPrx)
				{
					setReponseWithoutBody(reqID,request, response, 404, "failed to connected to contentlib service");
					return;
				}
				try
				{
			...
			else  if(_env._backStoreType == backCacheStore)///query volume info with CacheStroe service
			{
				...
				if(!cacheStroePrx)
				{
					setReponseWithoutBody(reqID,request, response, 404, "failed to connected to cachestore service");
					return;
				}
...
//                  TianShanIce::Storage::VolumePrx vlprx = csprx->openVolume(xmlElement[Key_volumeName]);
			...
			envlog(ZQ::common::Log::L_INFO, A3Fmt(A3MessageHandler, "get volume [%s] info completed took %d ms"),xmlElement[Key_volumeName].c_str(), ZQTianShan::now() - lStart);
		}
...
*/
		void A3MessageHandler::GetContentInfo(const std::string reqID, const CRG::IRequest* request, CRG::IResponse* response)
		{
			...
			if(_env._backStoreType == backContentLib)
			{
				...
				if(!clPrx)
				{
					setReponseWithoutBody(reqID,request, response, 404, "failed to connect to contentlib service");
					return;
				}
				try
				{
			...
			else if(_env._backStoreType == backCacheStore)///query content info with CacheStroe service
			{
				...
				if(strProviderId.empty() && strProviderAsserId.empty())
				{
					setReponseWithoutBody(reqID,request, response, 404, "unsupported for list all content");
					return;
				}
				...
				if (!strProviderId.empty() && !strProviderAsserId.empty()&& strProviderId != "*" && strProviderAsserId != "*")
				{
					...
					if(!cacheStroePrx)
					{
						setReponseWithoutBody(reqID,request, response, 404, "failed to connect to cachestore service");
						return;
					}	

					std::string strMD5DataTime = GenerateUTCTime();
					//std::string strVolumeName = "";
					std::string strFilesize = "", strSupportFilesize = "", strContentState = "", strStampCreated = "", strMD5CheckSum = "";
			...
			envlog(ZQ::common::Log::L_INFO, A3Fmt(A3MessageHandler, "get content info completed took %d ms"), ZQTianShan::now() - lStart);
		}
...
/*			///vailde the source url
			if ((strSourceURL.find(TianShanIce::Storage::potoFTP)  == std::string::npos) &&
				...
				(strSourceURL.find("udp") == std::string::npos) &&
				(strSourceURL.find("c2pull") == std::string::npos)&&
				(strSourceURL.find("c2http") == std::string::npos))*/
				...
				stricmp(strProtocol.c_str(), "cifs") == 0)
			{
				...
				std::string strUserName = itorElement->second;
				...
				std::string strPassWord = itorElement->second;

				strSourceURL.insert(pos, strUserName+ ":" + strPassWord + "@" );
			}
		...
		//	std::string strContentType = (strLocalType != "") ? (strLocalType + ":" +strSubType) : TianShanIce::Storage::ctMPEG2TS;
			...
			if(_env._backStoreType == backContentLib)
			{
				...
				catch(TianShanIce::InvalidParameter&ex)
				{
					...
					setReponseWithoutBody(reqID, request, response, 404, "Internal server error");
					return;
				}
				catch(TianShanIce::ServerError&ex)
				{
...
//					METADATA_PersistentTill(ISO);
			...
			else if(_env._backStoreType == backAuqaServer)
			{
			   ...
			   ////setp2. 组织CPESession所需要的Metadata以及需要存放在AquaServer上ContentName.metadata的Metadata
				...
				if(pPrx == NULL)
				{
					...
					setReponseWithoutBody(reqID, request, response, 400, "Bad request, failed to create provision session");
					return;
				}
			...
			envlog(ZQ::common::Log::L_INFO, A3Fmt(A3MessageHandler, "Transfering Content volume[%s]contentname[%s]completed took %d ms"), strVolume.c_str(), strContentName.c_str(), ZQTianShan::now() - lStart);
		}
		void A3MessageHandler::CancelTransfer(const std::string reqID, const CRG::IRequest* request, CRG::IResponse* response)
		{
			...
			if (!parseMsgContent(reqID, strMsgContent.c_str(), strMsgContent.length(), xmlElement))
			{
				setReponseWithoutBody(reqID, request, response, 404, CTSTATUS_404);
				return;
			}
			StringMap::iterator  itorElement;
			///check parameter key : volumeName
			...
			envlog(ZQ::common::Log::L_INFO, A3Fmt(A3MessageHandler, "Cancel Transfer complete took %d ms"), ZQTianShan::now() - lStart);
		}
		void A3MessageHandler::DeleteContent(const std::string reqID, const CRG::IRequest* request, CRG::IResponse* response)
		{
			...
			if (!parseMsgContent(reqID, strMsgContent.c_str(), strMsgContent.length(), xmlElement))
			{
				setReponseWithoutBody(reqID, request, response, 404, DCSTATUS_404);
				return; 
			}
			...
			if(_env._backStoreType == backContentLib)
			{
				...
				catch (const TianShanIce::InvalidStateOfArt& ex)
				{
					...
					setReponseWithoutBody(reqID, request, response, 409, DCSTATUS_409);
					return ;
				}
				catch (const Ice::Exception& ex)
				{	
  ...
  */              
				...
				filelist.push_back(contentName + _env._aquaContentMdata->getAquaMdataFileExtension());
			...
			envlog(ZQ::common::Log::L_INFO, A3Fmt(A3MessageHandler, "Delete Content() complete took %d ms"), ZQTianShan::now() - lStart);
		}
		void A3MessageHandler::GetContentChecksum(const std::string reqID, const CRG::IRequest* request, CRG::IResponse* response)
		{
			...
			envlog(ZQ::common::Log::L_INFO, A3Fmt(A3MessageHandler, "get content checksum complete took %d ms"), ZQTianShan::now() - lStart);
		...
		void A3MessageHandler::ExposeContent(const std::string reqID, const CRG::IRequest* request, CRG::IResponse* response)
		{
			...
			if (!parseMsgContent(reqID, strMsgContent.c_str(), strMsgContent.length(), xmlElement))
			{
				setReponseWithoutBody(reqID, request, response, 404, ECSTATUS_404);
				return;
			}
			...
			envlog(ZQ::common::Log::L_DEBUG, A3Fmt(A3MessageHandler, "success to expose content [%s]"), strContentName.c_str());
			...
			strMsgContent = buf.str();
			setResponseWithBody(reqID, request, response, 200, "OK", strMsgContent);
			envlog(ZQ::common::Log::L_INFO, A3Fmt(A3MessageHandler, "Expose Content complete took %d ms"), ZQTianShan::now() - lStart);
		...
		void A3MessageHandler::GetTransferStatus(const std::string reqID, const CRG::IRequest* request, CRG::IResponse* response)
		{
			...
			setResponseWithBody(reqID, request, response, 200, "OK", strMsgContent);
			envlog(ZQ::common::Log::L_INFO, A3Fmt(A3MessageHandler, "Get Transfer Status complete took %d ms"), ZQTianShan::now() - lStart);
