	...
	namespace A3Message
	{
		...
		bool A3MsgEnv::doInit(const std::string& strLogPath, const std::string& strCfgPath)
		{
			...
			else if(_backStoreType == backAuqaServer)
			{
				...
				_pCdmiFuse = new CdmiFuseOps(_log, _thrdPool, _A3Config.rootUrl, _A3Config.userDomain, _A3Config.homeContainer, _A3Config.flags);
				if(NULL == _pCdmiFuse)
				{
...
//#pragma message ( __MSGLOC__ "TODO: impl here")
		...
		bool A3MsgEnv::addContentInfo(std::string contentName, ContentInfo contentInfo)
		{
			ZQ::common::MutexGuard gd(_lockContentInfos);
			_contentInfos[contentName] = contentInfo;
			return true;
		}
		bool A3MsgEnv::removeContentInfo(std::string contentName)
		{
			ZQ::common::MutexGuard gd(_lockContentInfos);
		...
		ContentInfo A3MsgEnv::getContentInfo(std::string contentName)
		{
