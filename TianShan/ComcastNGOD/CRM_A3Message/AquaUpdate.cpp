...
#define JSON_HAS(OBJ, CHILDNAME) (OBJ.isObject() && OBJ.isMember(CHILDNAME))
		...
		bool A3AquaBase::getMetadataInfo(const std::string& paid, const std::string& pid,  TianShanIce::Properties& metadata)
		{
			...
			int cdmiCode = 0;
			bool bmdMd = getMDFileMetadata(strAquaContentMDName, mdMdValue, cdmiCode);
            if(bmdMd)
			{
				metadata.clear();
			...
			///step2. read metadata from main content file metadata
			if(statId > ::TianShanIce::Storage::csProvisioningStreamable && statId <= ::TianShanIce::Storage::csInService)
			{
				metadata.clear();
				...
				cdmiCode = -1;
				bool bMainFileMd = getMainFileMetadata(strMainContentName, mainFileMdValue, cdmiCode);
				if(bMainFileMd && JSON_HAS(mainFileMdValue, "metadata"))
				{
				...
				{
					_log(ZQ::common::Log::L_ERROR, CLOGFMT(A3AquaBase, "getMetadataInfo()failed to read metadata from mainfile[%s]"), strMainContentName.c_str());
					bmdMd = false;
				}
			}
