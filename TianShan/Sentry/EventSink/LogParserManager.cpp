...
{
    using namespace ZQ::common;
    LOG(Log::L_INFO, CLOGFMT(LogParserManager, "Enter addMonitoring with [log=%s], [type=%s], [syntax=%s], [key=%s]"), logger.logFile.c_str(), logger.logType.c_str(), logger.syntaxFile.c_str(), logger.syntaxKey.c_str());

    if(logger.logFile.empty() || logger.syntaxFile.empty() || logger.syntaxKey.empty())
    {
        LOG(Log::L_ERROR, CLOGFMT(LogParserManager, "addMonitoring() Bad logger info. file=%s"), logger.logFile.c_str());
        return false;
    }
    if(!logger.logType.empty() && !_pMsgSrcFac->checkType(logger.logType))
    {
        LOG(Log::L_ERROR, CLOGFMT(LogParserManager, "addMonitoring() Bad log type. type=%s"), logger.logType.c_str());
        return false;
    }
    ZQ::common::MutexGuard sync(_lockMgmt);
    if(_mgmtData.find(logger.logFile) != _mgmtData.end())
    { // under monitoring
        LOG(Log::L_WARNING, CLOGFMT(LogParserManager, "addMonitoring() Logger is under monitoring. file=%s"), logger.logFile.c_str());
        return false;
    }
    ...
    {
        LOG(Log::L_ERROR, CLOGFMT(LogParserManager, "addMonitoring() Failed to load syntax definition file. file=%s"), logger.syntaxFile.c_str());
        return false;
    }

    if(syntaxDef.syntaxMap.end() == syntaxDef.syntaxMap.find(logger.syntaxKey))
    { // no syntax found
        LOG(Log::L_ERROR, CLOGFMT(LogParserManager, "addMonitoring() Can't find syntax definition. key=%s"), logger.syntaxKey.c_str());
        return false;
    }
    ...
    { // logger type not fit
        LOG(Log::L_ERROR, CLOGFMT(LogParserManager, "addMonitoring() Logger type [%s] conflict with actual log type [%s] in the syntx definition"), logger.logType.c_str(), syntax.evntSrcType.c_str());
        return false;
    }
    ...
    for(size_t iSender = 0; iSender < senders.size(); ++iSender) {
        ...
        if(!record) {
            LOG(Log::L_ERROR, CLOGFMT(LogParserManager, "addMonitoring() Failed to get the position of handler(%s), file(%s)"), senders[iSender].strType.c_str(), logger.logFile.c_str());
            continue;
        }
        /*
    ...
    try {
        ...
        LOG(Log::L_ERROR, CLOGFMT(LogParserManager, "addMonitoring() Failed to create message handler object. Give up the monitoring. file=%s, type=%s. Exception:%s"), logger.logFile.c_str(), msgSrcType.c_str(), e.getString());
        return false;
    }
    ...
    LOG(Log::L_INFO, CLOGFMT(LogParserManager, "addMonitoring() Successful to monitor the log file. file=%s"), logger.logFile.c_str());
    return true;
}
