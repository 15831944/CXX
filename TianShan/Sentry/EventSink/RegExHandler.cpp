...
RegExpHandler::RegExpHandler(ZQ::common::Log& log, const std::string& exp, const EventTemplate& tmpl, IEventSender* sender, const Literal::Library& lib)
:_log(log), _pSender(sender)
{
    try
    {
        _expr.assign(exp);
    ...
    _log(ZQ::common::Log::L_INFO, ReLOGFMT(RegExpHandler, "RE handler inited. expression:%s"), exp.c_str());
}
RegExpHandler::~RegExpHandler()
{
}

void RegExpHandler::handle(const char* msg, int len, const MessageIdentity& mid)
{
	boost::cmatch result;
	try {
		...
		_log(ZQ::common::Log::L_DEBUG, ReLOGFMT(RegExpHandler, "Message matched[%s]: %s"), buf.str().c_str(), result.str().c_str());
		
		Event evnt;
		generateEvent(result, evnt, mid.stamp);
		_pSender->post(evnt, mid);
		int64 stampPost = ZQ::common::now();
		_log(ZQ::common::Log::L_INFO, ReLOGFMT(RegExpHandler, "Event posted [%d/%d]: %s"), (int) (stampMatched - stampStart), (int) (stampPost - stampStart), buf.str().c_str());
	}
	catch (const std::exception& e)
	{
		_log(ZQ::common::Log::L_WARNING, ReLOGFMT(RegExpHandler, "Got std::exception(%s) in the regex_match(). message(%s)"), e.what(), msg);
	}
}
// fill the template to generate an event
void RegExpHandler::generateEvent(const boost::cmatch& matchResult, Event& evnt, int64 stamp) const
{
    char timebuf[64];
...
std::string RegExpHandler::fixupParam(const boost::cmatch& matchResult, const std::string& s, const std::string& T) const
{ 
...
std::string RegExpHandler::fixupMacro(const std::string& s) const
{
...
std::string RegExpHandler::fixup(const boost::cmatch& matchResult, const std::string& s, const std::string& T) const
{
	return fixupMacro(fixupParam(matchResult, s, T));
}

std::string RegExpHandler::fixupAndExecute(const boost::cmatch& matchResult, const Literal::Expression& ex, const std::string& T) const
{
