...
//	SentryEnv::Adapters* pAdpt =NULL;
	...
	{
		envlog(ZQ::common::Log::L_ERROR, CLOGFMT(LocalAdapterRefreshing, "failed to query the data from local adapter callback of process[%d]: %s"), _processInfo.processId, ex.ice_name().c_str());
	    return -1;
    }
