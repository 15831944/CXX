...
protected:
	...
	void		setLastErrorMsg( const std::string& errMsg );
...
{
public:
	SessStateChangeGuard(SessionImpl& sess);	

	virtual ~SessStateChangeGuard();

	void changing( bool bChanging);
	
	bool isInChanging() const;
	
private:
	SessionImpl&			_sess;
	//IceUtil::RWRecMutex::SESSWLOCK _lock;	
};
