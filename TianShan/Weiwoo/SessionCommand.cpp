...
int SessionProvisionCommand::run(void)
{
	try 
	{
		...
		envlog(ZQ::common::Log::L_DEBUG,CLOGFMT(SessionProvisionCommand,"Session[%s] enter Provision"),strSessID.c_str());
		SessStateProvisioned(_env, *_pSess).enter();
		envlog(ZQ::common::Log::L_INFO,CLOGFMT(SessionProvisionCommand,"Session[%s] Leave Provision"),strSessID.c_str());
		_amdCB->ice_response();
		return 0;
	}
