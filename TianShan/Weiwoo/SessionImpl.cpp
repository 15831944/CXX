...
::TianShanIce::SRM::SessionPrx SessionManagerImpl::createSession(const ::TianShanIce::SRM::Resource& assetUri, const ::Ice::Current& c)
{
	...
	
	{
		...
		if (assetUri.resourceData.end() != itGrp && ::TianShanIce::vtStrings == itGrp->second.type && itGrp->second.strs.size() >0)
			sessId = itGrp->second.strs[0] + ":" +sessId;
	}

	envlog(ZQ::common::Log::L_DEBUG, CLOGFMT(SessionManager, "creating session[%s] for URI[%s]"),sessId.c_str(), strUri.c_str());

	int currentPendingSize = _env._thpool.pendingRequestSize();
	if( currentPendingSize  > gWeiwooServiceConfig.lMaxPendingRequestSize )
	{
		envlog(ZQ::common::Log::L_ERROR,CLOGFMT(SessionManager, "session[%s] rejected per pending request[%d] maxPendingSize[%d]"),
...
// 		{
// 			envlog(ZQ::common::Log::L_WARNING,CLOGFMT(SessionImpl,"onRestore() sess[%s] failed to associate its ticket due to [%s]"),
...
*/
	...
	case ::TianShanIce::stOutOfService:
		SessStateOutOfService(_env, *this).doDestroy(c);
	};
}
...
::Ice::Int SessionImpl::setPrivateData2(const ::TianShanIce::ValueMap& valMap, const ::Ice::Current&/* = ::Ice::Current()*/)
{
	...
	{
		envlog(ZQ::common::Log::L_INFO,CLOGFMT(SessionImpl,"Session[%s] SetPrivateData2,Key =%s"),ident.name.c_str(), it->first.c_str());
...
{
	envlog(ZQ::common::Log::L_DEBUG, SESSLOGFMT("addResource() with type is %s"),ResourceTypeStr[type]);
...
{	
	envlog(ZQ::common::Log::L_DEBUG,
		CLOGFMT(SessionImpl,"provision_async() session[%s] pending threadPool[%d/%d] pendingRequest[%d]"),
...
{
	envlog(ZQ::common::Log::L_DEBUG,
		CLOGFMT(SessionImpl,"serve_async() session[%s] pending threadPool[%d/%d] pendingRequest[%d]"),
		sessId.c_str(),_env._thpool.activeCount(),_env._thpool.size(),_env._thpool.pendingRequestSize());
