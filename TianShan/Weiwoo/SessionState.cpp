...
SessStateChangeGuard::SessStateChangeGuard(SessionImpl& sess)
:_sess(sess)
{
	_sess._bStateInChange = true;		
#ifdef _DEBUG
	_sess._env._log(ZQ::common::Log::L_DEBUG, SESSIONSTATELOGFMT(SessionState, "state in changing"));
#endif
}
...
void SessStateChangeGuard::changing( bool bChanging) 
{
_sess._bStateInChange = bChanging;
}
bool SessStateChangeGuard::isInChanging() const
{
return _sess._bStateInChange;
}
...
void SessStateBase::setLastErrorMsg( const std::string& errMsg )
{
...
#pragma message(__MSGLOC__"TODO:check the ticket expiration here")
	...
	if (nextWait <=0 )
	{
		envlog(ZQ::common::Log::L_DEBUG,
				SESSIONSTATELOGFMT(SessionState, "met expiration %llu, entering stOutOfService"),
				_sess.expiration);
...
void SessStateBase::_destroySess(const ::Ice::Identity& ident)
{
	...
	{
		ZQTianShan::_IceThrow<TianShanIce::ServerError> (envlog, "SessStateBase",1052,SESSIONSTATEFMT(SessStateBase, "_destroySess() caught unknown exception"));
	}
}
...
{
	SessStateChangeGuard sscg(_sess);
...
//			setLastErrorMsg("Invalid state to continue entering InService after reserving paths");
	...
	{
		setLastErrorMsg("failed to reserve path resource");
		_updateExpiration(now() + TIMEOUT_YIELD_AFTER_ERROR_OCCURED);
...
#pragma message(__MSGLOC__"TODO:Should I renew the ticket here with _ttlIdleSess")
		...
		catch(const ::TianShanIce::BaseException& e)
		{
			...
			char	szBuf[2048];
			szBuf[2047]='\0';
			snprintf(szBuf,2047,"Can't create stream with ticket %s and streamer %s and exception is :%s",
				pathTicketId.c_str(),
				streamerInfo.ifep.c_str(),
				e.message.c_str());
		...
		{
			itSelectedEval->ticket->setPenalty(gWeiwooServiceConfig.lMaxPenaltyValue);
			envlog(ZQ::common::Log::L_WARNING, SESSIONSTATELOGFMT(SessStateInService, "unknown exception caught when connect to Streamer[%s]"), streamerInfo.ifep.c_str());
			char	szBuf[2048];
			szBuf[2047]='\0';
			snprintf(szBuf,2047,"Can't create stream with ticket %s and streamer %s and exception is :Unexpet error",
				pathTicketId.c_str(),
				streamerInfo.ifep.c_str()	);
			vecErrMsg.push_back(szBuf);
		}
...
#pragma message ( __MSGLOC__ "TODO: should we go directly to OutService if InService failed ?")
	...
	{
		setLastErrorMsg("failed to render stream");
		_updateExpiration(now() + TIMEOUT_YIELD_AFTER_ERROR_OCCURED);
		be.ice_throw();
	}
	catch(const ::Ice::Exception& e)
	...
	{
		setLastErrorMsg("failed to commit stream");
		_updateExpiration(now() + TIMEOUT_YIELD_AFTER_ERROR_OCCURED);
...
void SessStateOutOfService::failedToDestroyInstance()
{
	...
	if( lastDestroyTime <= 0 )
	{
		...
		envlog(ZQ::common::Log::L_INFO,SESSIONSTATELOGFMT(SessStateOutOfService,"destroy stream in %ld ms"),_env._ttlIdleSess );
		_updateExpiration( now() + _env._ttlIdleSess);
	}
	else
	{
...
// -----------------------------
bool SessStateOutOfService::canEnterState( const ::TianShanIce::State& targetState, SessStateChangeGuard& guard )
{
	SESSWLOCK(_sess);
	if( guard.isInChanging() && targetState == _sess._stateChangeTargetState)
		return false;
	
	guard.changing( true );//this flag will be reset to false when SessStateChangeGuard is desctructed
...
void SessStateOutOfService::enter(void)
{
	...
	{
		envlog(ZQ::common::Log::L_INFO, SESSIONSTATELOGFMT(SessStateOutOfService, "enter() session is in state change to OutOfService, skip"));
	}

	envlog(ZQ::common::Log::L_DEBUG, SESSIONSTATELOGFMT(SessStateOutOfService, "enter() weiwoo session destroy "));
	...
	// step 1. TODO ouble check the resource and clean up
	envlog(ZQ::common::Log::L_DEBUG, SESSIONSTATELOGFMT(SessStateOutOfService, "double check the resource to free"));
	::TianShanIce::SRM::SessionPrx proxyThis = IdentityToObj(Session, _sess.ident);
	
#pragma message ( __MSGLOC__ "TODO: double check the resource and clean up")

	// step 2. determin the reasons
	std::string mixedReason = "session terminated", strTeardownReason, strTerminateReason;
	TianShanIce::Properties purchaseDetachProps;
	{
		//	fixup the teardown/terminated reason here
		////////////////////////////////////////////////////
		...
		if (_sess.privdata.end() != itVal && itVal->second.type == TianShanIce::vtStrings && itVal->second.strs.size()>0)
			strTeardownReason = oldTeardownReason = itVal->second.strs[0];

		itVal = _sess.privdata.find(SYS_PROP(terminateReason));
		if (_sess.privdata.end() != itVal && itVal->second.type == TianShanIce::vtStrings && itVal->second.strs.size()>0)
			strTerminateReason = oldTerminateReason = itVal->second.strs[0];

		if (!strTeardownReason.empty())
		{
			if (atol(strTeardownReason.c_str()) < 100000) // the teardown reason string doesn't carry a code, insert one
				strTeardownReason = "100000 " + strTeardownReason;
		}
		else if (gWeiwooServiceConfig.lMixTeardownReasonAndTerminateReason >= 1) 
			strTeardownReason = strTerminateReason;

		if (strTerminateReason.empty() && !strTeardownReason.empty())
		{
			if (gWeiwooServiceConfig.lMixTeardownReasonAndTerminateReason >= 1)
				strTerminateReason = strTeardownReason;
			// ticket#16644 to leave empty strTerminateReason
			//else
			//	strTerminateReason = "100000 User Requested TEARDOWN";
		}

		mixedReason = strTeardownReason;

		// flush to private data if adjusted
		if (0 != strTeardownReason.compare(oldTeardownReason) || 0 != strTerminateReason.compare(oldTerminateReason))
		{
			envlog(ZQ::common::Log::L_DEBUG, SESSIONSTATELOGFMT(SessStateOutOfService, "stop reasons updated: TM[%s]=>[%s], TD[%s]=>[%s]"),
				oldTerminateReason.c_str(), strTerminateReason.c_str(), oldTeardownReason.c_str(), strTeardownReason.c_str());	

			::TianShanIce::Variant vReason;
			vReason.bRange = false;
			vReason.type = TianShanIce::vtStrings;
			vReason.strs.push_back(strTerminateReason);

			// SESSWLOCK(_sess);
			MAPSET(TianShanIce::ValueMap, _sess.privdata, SYS_PROP(terminateReason), vReason);

			vReason.strs.clear();
			vReason.strs.push_back(strTeardownReason);
			MAPSET(TianShanIce::ValueMap, _sess.privdata, SYS_PROP(teardownReason), vReason);
		}
	}

	// step 3. destroy stream if it is still running
	bool bDestroyStreamOK = true;
	try
	{
		...
		if (stream)
		{
			...
			envlog(ZQ::common::Log::L_INFO, SESSIONSTATELOGFMT(SessStateOutOfService, "destroy stream %s "),strProxy.c_str());			
			Ice::Context ctx;
			std::string mySelf ="WeiwooService:";
			mySelf += _env._communicator->proxyToString(proxyThis);			
			ctx["caller"] = mySelf;			
			ctx["reason"] = mixedReason;
			...
			try { 
				...
				for (::TianShanIce::Properties::iterator itFB = feedback.begin(); itFB != feedback.end(); itFB++)
				{
					...
					MAPSET(TianShanIce::Properties, purchaseDetachProps, itFB->first, itFB->second);
	...
	{
		envlog(ZQ::common::Log::L_WARNING, SESSIONSTATELOGFMT(SessStateOutOfService, "caught TianShanIce Exception when destroy stream:%s"),ex.message.c_str());
		bDestroyStreamOK= false;
	}
	catch( const Ice::ObjectNotExistException&)	{ }
	catch( const Ice::ObjectNotFoundException&)	{ }
	catch (Ice::Exception& ex)
	{
		envlog(ZQ::common::Log::L_WARNING, SESSIONSTATELOGFMT(SessStateOutOfService, "caught Ice exception when destroy stream:%s"), ex.ice_name().c_str());
		bDestroyStreamOK= false;		
	}
	catch(...)
	{	
		envlog(ZQ::common::Log::L_WARNING, SESSIONSTATELOGFMT(SessStateOutOfService, "caught exception when destroy stream"));
	}
	...
	{
		failedToDestroyInstance();
	}

	// step 4. free ticket if it is still allocated
	bool bDestroyTicketOk = true;
	try
	{
		...
		if(ticket)
		{
			...
			envlog(ZQ::common::Log::L_DEBUG, SESSIONSTATELOGFMT(SessStateOutOfService,"destroy on-going pathticket %s"),
				idTicket.name.c_str());
			if (ticket)
			{
				Ice::Context ctx;
	...
	{
		bDestroyTicketOk = false;
		envlog(ZQ::common::Log::L_ERROR, SESSIONSTATELOGFMT(SessStateOutOfService,"when destroy ticket,catch a TianShanIce exception %s"),
			ex.message.c_str());
	}
	catch( const Ice::ObjectNotExistException&){}
	catch( const Ice::ObjectNotFoundException&){}
	...
	{
		bDestroyTicketOk = false;
		envlog(ZQ::common::Log::L_ERROR, SESSIONSTATELOGFMT(SessStateOutOfService,"when destroy ticket,catch a ice exception %s"), ex.ice_name().c_str());	
	}
	catch(...)
	{	
		envlog(ZQ::common::Log::L_WARNING, SESSIONSTATELOGFMT(SessStateOutOfService, "caught exception when destroy PathTicket"));
	}
	...
	// step 5. destroy purchase if it is still bound
	try
	{
		envlog(ZQ::common::Log::L_DEBUG, SESSIONSTATELOGFMT(SessStateOutOfService, "detach puchase [%s]"),_sess.purchasePrxStr.c_str());
		...
		{		
			...
			{
				// add the reason into the detach.prop
				MAPSET(TianShanIce::Properties, purchaseDetachProps, SYS_PROP(terminateReason), strTerminateReason);
				MAPSET(TianShanIce::Properties, purchaseDetachProps, SYS_PROP(teardownReason),  strTeardownReason);

				purchase->detach_async( new AMI_Purchase_detachImpl(), _sess.ident.name, purchaseDetachProps);
				purchase = NULL;
			}
			else
			{
				envlog(ZQ::common::Log::L_WARNING, SESSIONSTATELOGFMT(SessStateOutOfService, "Can't get purchase proxy through %s"),
					_sess.purchasePrxStr.c_str());
			}
		}
	}
	catch (TianShanIce::BaseException& ex)
	{
		envlog(ZQ::common::Log::L_WARNING, SESSIONSTATELOGFMT(SessStateOutOfService, "caught tianshanIce exception when destroy purchase:%s"),
			ex.message.c_str());
	}
	catch (Ice::Exception& ex)
	{
		envlog(ZQ::common::Log::L_WARNING, SESSIONSTATELOGFMT(SessStateOutOfService, "caught ice exception when destroy purchase:%s"),
			ex.ice_name().c_str());
	}
	catch(...)
	{	
		envlog(ZQ::common::Log::L_WARNING, SESSIONSTATELOGFMT(SessStateOutOfService, "caught exception when destroy purchase"));
	}
	...
	// step End. commit state change
	_updateExpiration(now() + _env._ttlOutOfServiceSess);
...
#pragma message(__MSGLOC__"TODO : transfer liveTxn into YTDTxn")
	...
	{
		envlog(ZQ::common::Log::L_DEBUG, CLOGFMT(SessStateBase, "TianShanIce exception when postYTD:%s"),
				ex.message.c_str());
	}
	catch (Ice::Exception& ex)
	{
		envlog(ZQ::common::Log::L_DEBUG, CLOGFMT(SessStateBase, "ice exception when postYTD:%s"),
				ex.ice_name().c_str());
	}
	catch (...) 
	{
		envlog(ZQ::common::Log::L_ERROR, CLOGFMT(SessStateBase, "exception occurs when post YTD"));
	}
}
...
void SessStateOutOfService::OnTimer(const ::Ice::Current& c)
{
	...
	{
		SESSRLOCK(_sess);
		nextWait  = _sess.expiration - now();
	}
	
	if (nextWait >= _env._ttlOutOfServiceSess)
	{
		_updateExpiration(now() + _env._ttlOutOfServiceSess); // make sure never exceed _ttlOutOfServiceSess
		return;
	}

	if (nextWait <= 0)
	{
		envlog(ZQ::common::Log::L_DEBUG, SESSIONSTATELOGFMT(SessStateOutOfService, 
			"destroy due to timeout (%d msec) in the stOutOfService"), _env._ttlOutOfServiceSess);
		_destroySess(c.id);
	}
