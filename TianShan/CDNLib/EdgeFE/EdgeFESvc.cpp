...
ZQ::common::Config::Loader< ZQ::StreamService::EdgeFEConfig> gEdgeFEConfig( "EdgeFE" ".xml");

ZQ::common::Config::ILoader			*configLoader = &gEdgeFEConfig;
...
bool CdnSSSerice::initializeLogger( )
{
	try
	{
		mSessionLogger.open((getLoggerFolder()+ ZQ_App_Name + ".Sess.log").c_str(),
			gEdgeFEConfig.sessionLogLevel,
			ZQLOG_DEFAULT_FILENUM,
			gEdgeFEConfig.sessionLogFileSize,
			gEdgeFEConfig.sessionLogBufferSize,
			gEdgeFEConfig.sessionLogTimeout);
	}
...
bool CdnSSSerice::InitializeIceRunTime() {
	...
	ZQTianShan::Util::getPropertyDataWithDefault( gEdgeFEConfig.iceProperties,"Ice.ThreadPool.Server.Size",10,threadSize);
	ZQTianShan::Util::getPropertyDataWithDefault( gEdgeFEConfig.iceProperties,"Ice.ThreadPool.Server.SizeMax",20,threadSizeMax);

	threadSize = MAX(threadSize , atoi(gEdgeFEConfig.dispatchThreadCount.c_str()) );
	threadSizeMax = MAX(threadSizeMax , atoi(gEdgeFEConfig.dispatchThreadCountMax.c_str()) );

	gEdgeFEConfig.iceProperties.erase("Ice.ThreadPool.Server.Size");
	gEdgeFEConfig.iceProperties.erase("Ice.ThreadPool.Server.SizeMax");

	ZQTianShan::Util::updatePropertyData(gEdgeFEConfig.iceProperties,SERVICE_ADAPTER_NAME".ThreadPool.Size",threadSize);
	ZQTianShan::Util::updatePropertyData(gEdgeFEConfig.iceProperties,SERVICE_ADAPTER_NAME".ThreadPool.SizeMax",threadSizeMax);

	std :: map<std::string, std::string>::const_iterator it = gEdgeFEConfig.iceProperties.begin();
	for( ; it != gEdgeFEConfig.iceProperties.end() ; it ++ )
	{		
	...
	if( gEdgeFEConfig.enableIceTrace >= 1 )
	{
		try
		{
			mIceFileLogger.open( (getLoggerFolder()+ ZQ_App_Name + ".IceTrace.log").c_str() ,
									gEdgeFEConfig.iceTraceLevel,
									ZQLOG_DEFAULT_FILENUM,
									gEdgeFEConfig.iceTraceLogSize,
									10240,
									2);
		}
		catch( const ZQ::common::FileLogException& ex)
...
bool CdnSSSerice::initializeServiceParameter( ) 
{
	try
	{
		mCdnEnv->mainAdapter = ZQADAPTER_CREATE( mIc , SERVICE_ADAPTER_NAME , gEdgeFEConfig.serviceEndpoint.c_str() , *m_pReporter );
	...
	catch( const Ice::Exception& ex)
	{
		SERVICELOG(ZQ::common::Log::L_ERROR,CLOGFMT(CdnSSSerice,"initializeServiceParameter() failed to create adapter at [%s] due to [%s]"),
			gEdgeFEConfig.serviceEndpoint.c_str(), ex.ice_name().c_str() );
		return false;
	}

	mCdnEnv->mTransferServerHttpIp					=	gEdgeFEConfig.c2StreamerConfig.httpBindIp;
	mCdnEnv->mTransferServerHttpPort				=	gEdgeFEConfig.c2StreamerConfig.httpBindPort;

	mCdnEnv->streamsmithConfig.iUseMemoryDB			=	1;
	mCdnEnv->streamsmithConfig.iRenewTicketInterval	=	gEdgeFEConfig.ticketRenewInterval;
	...
	if(0) {
		...
		mCdnEnv->streamsmithConfig.iPlaylistTimeout		=	gEdgeFEConfig.sessionTimeout;
	} else {
		mCdnEnv->streamsmithConfig.iPreloadTimeInMS		=	5000;
		mCdnEnv->streamsmithConfig.iSupportPlaylist		=	ZQ::StreamService::LIB_SUPPORT_NORMAL_PLAYLIST;
		mCdnEnv->streamsmithConfig.iPlaylistTimeout		=	gEdgeFEConfig.sessionTimeout;
		mCdnEnv->streamsmithConfig.iPerRequestedInterval=   gEdgeFEConfig.perRequestedInterval;
	}

	mCdnEnv->iceCommunicator						=	mIc;

	mCdnEnv->mNetId									=	gEdgeFEConfig.netId;
...
bool CdnSSSerice::startServer( )
{
	...
	int32 threadCount = gEdgeFEConfig.mainThreadCount;
	threadCount = threadCount < 3 ? 3 : threadCount;

	mCdnEnv->getMainThreadPool().resize(threadCount);

	mpServiceImpl->strCheckpointPeriod		= gEdgeFEConfig.perfCheckpointPeriod;
	mpServiceImpl->strDbRecoverFatal		= gEdgeFEConfig.perfDbRecoverFatal;
	mpServiceImpl->strSavePeriod			= gEdgeFEConfig.perfSavePeriod ;
	mpServiceImpl->strSaveSizeTrigger		= gEdgeFEConfig.perfSaveSizeTrigger ;
	mpServiceImpl->iEvictorStreamSize		= gEdgeFEConfig.perfSessionCacheSize;
	mpServiceImpl->iReplicaReportInterval	= gEdgeFEConfig.streamerReplicaConfig.defaultUpdateInterval;
	...
	std::string cdnDbPath = ZQTianShan::Util::fsConcatPath(gEdgeFEConfig.dbPath, ZQ_App_SvcName);
	
	if(!mpServiceImpl->start( cdnDbPath ,
		gEdgeFEConfig.eventChannelEndpoint,
		gEdgeFEConfig.streamerReplicaConfig.listenerEndpoint,
		gEdgeFEConfig.netId,
		ZQ_App_Name))
	{
	...
	//publish log
	{
		...
		const std::vector< MonitoredLog >& monLogs =  gEdgeFEConfig.monitoredLogs;
...
{
#ifdef ZQ_OS_MSWIN
    if(! ZQTianShan::Util::fsCreatePath( gEdgeFEConfig.crashDumpPath) )
    {
        SERVICELOG(ZQ::common::Log::L_ERROR,CLOGFMT(CdnSSService,"can't create path [%s] for mini dump "),
            gEdgeFEConfig.crashDumpPath.c_str());
        return S_FALSE;
    }	
    _crashDump.setDumpPath( (char*)gEdgeFEConfig.crashDumpPath.c_str() );
    _crashDump.enableFullMemoryDump( gEdgeFEConfig.enableCrashDump );
    _crashDump.setExceptionCB(CrashExceptionCallBack);
#else
    DUMP_PATH = gEdgeFEConfig.crashDumpPath.c_str();
	...
	gEdgeFEConfig.ticketRenewThreadpoolSize = MAX(gEdgeFEConfig.ticketRenewThreadpoolSize,3);
	mCdnEnv->getRenewTicketThreadPool().resize(gEdgeFEConfig.ticketRenewThreadpoolSize);
