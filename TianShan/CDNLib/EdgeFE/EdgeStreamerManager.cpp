...
bool CdnStreamerManager::listStreamer( SsReplicaInfoS& infos ) const
{
	...
	for( ; it != mStreamers.end() ; it ++ )
	{
		...
		info.groupId		=	gEdgeFEConfig.netId;
		...
		ZQTianShan::Util::updatePropertyData(info.props, STREAMERPROP_VOLUMENETID , gEdgeFEConfig.csStrReplicaGroupId );
		...
		ENVLOG(ZQ::common::Log::L_INFO, CLOGFMT(CdnStreamerManager,"listStreamer [%s] [%s] [%s/%s] capacity[%ld] found"),
			gEdgeFEConfig.netId.c_str(),
...
{
	int32 scanInterval = gEdgeFEConfig.resourceScanInterval;
	scanInterval = scanInterval < 5000 ? 5000 : scanInterval;
	scanInterval = scanInterval > (10000 * 1000 )? (10000 * 1000 ) : scanInterval;
	std::string upstreamIp = gEdgeFEConfig.cacheStoreConfig.upStreamBindip;
...
bool CdnStreamerManager::setC2Conf(C2Streamer::C2StreamerEnv& c2env )
{
	...
	const std::vector<C2StreamerSvcConfig::PORTCONF>& portSpeedConf = gEdgeFEConfig.c2StreamerConfig.portConf;
	...
	///get mount doc path
	EdgeFEConfig::Volumes::const_iterator itDocPath = gEdgeFEConfig.volumes.begin();
	bool bFoundDocPath =false;
	for( ; itDocPath != gEdgeFEConfig.volumes.end() ; itDocPath ++)
	{
	...
	conf.readBufferCount	= gEdgeFEConfig.c2StreamerConfig.readBufferCount;
	conf.readBufferCount	= MAX( conf.readBufferCount, 8 );
	conf.ioBlockSize 		= 4 * 1024;
	conf.minIdleInterval 	= 200;

	conf.mSocketKernelSndBuf = gEdgeFEConfig.c2StreamerConfig.perfTune.socketSndBuf;
	if(conf.mSocketKernelSndBuf < 32 * 1024)
		conf.mSocketKernelSndBuf = 32 * 1024;
	conf.mSocketKernelSndTimeo = gEdgeFEConfig.c2StreamerConfig.perfTune.socketSndTimeo;
	if( conf.mSocketKernelSndTimeo < 100 )
		conf.mSocketKernelSndTimeo = 100;

	conf.mFirstChunkWait = gEdgeFEConfig.c2StreamerConfig.perfTune.firstChunkWait;
	if(conf.mFirstChunkWait < 5)
		conf.mFirstChunkWait = 5;
	conf.mbUseTcpCork = gEdgeFEConfig.c2StreamerConfig.perfTune.useTcpCork >= 1;
	conf.mPendingSizeThreshold = gEdgeFEConfig.c2StreamerConfig.perfTune.threadpoolwarningThreshold;
	conf.mSocketSendMonitorThreashold = gEdgeFEConfig.c2StreamerConfig.perfTune.socketWriteMonitorThreshold;
	conf.mMaxIdleCountAtFileEnd = gEdgeFEConfig.c2StreamerConfig.sessConf.maxIdleCountAtEnd;
	conf.mDefaultTransferBitrate = gEdgeFEConfig.c2StreamerConfig.sessConf.defaultBitrate;
	conf.mFsReadThreshold	= gEdgeFEConfig.c2StreamerConfig.perfTune.fsReadThreshold;
	conf.mbQuitPerConnectionLost = gEdgeFEConfig.c2StreamerConfig.sessConf.quitPerConnectionLost >= 1;
	conf.mTimerThreadHold	= gEdgeFEConfig.c2StreamerConfig.perfTune.timerMonitorThreshold;
	conf.mSendPacketSize	= gEdgeFEConfig.c2StreamerConfig.perfTune.socketPacketSize;
	conf.mPacketsPerSend	= gEdgeFEConfig.c2StreamerConfig.perfTune.packetsPerSend;
	conf.mLocalBindPort		= gEdgeFEConfig.c2StreamerConfig.httpBindPort;
	if( conf.mPacketsPerSend < 1 )
		conf.mPacketsPerSend = 1;

	conf.maxRoundPerRun		= gEdgeFEConfig.c2StreamerConfig.perfTune.maxRoundPerRun;
	if( conf.maxRoundPerRun < 1 )
		conf.maxRoundPerRun = 1;

	//HLS part
	conf.mKeyFile5I			= gEdgeFEConfig.hlsServerConfig.keyfile;
	conf.mAquaRootUrl		= gEdgeFEConfig.hlsServerConfig.rooturl;
	conf.mHomeContainer		= gEdgeFEConfig.hlsServerConfig.homecontainer;
	conf.mServerHostUrl		= gEdgeFEConfig.hlsServerConfig.serverHostUrl;
	conf.mLogFlag			= gEdgeFEConfig.hlsServerConfig.logFlag;
	conf.mName2Bitrate		= gEdgeFEConfig.hlsServerConfig.subname2bitrate;
	conf.mDefaultBitrate	= gEdgeFEConfig.hlsServerConfig.defaultBitrate;

	//Cache part
	conf.mCacheDefaultBufferCount = gEdgeFEConfig.c2StreamerConfig.perfTune.cacheBufferCount;
	conf.mCacheDefaultBufferCount = MAX(conf.mCacheDefaultBufferCount,10);
	conf.mCacheBufferSize = gEdgeFEConfig.c2StreamerConfig.perfTune.cacheBufferSize;
	conf.mCacheReadaheadCount = gEdgeFEConfig.c2StreamerConfig.perfTune.cacheReadAheadCount;
	conf.mUseBufferIo  = gEdgeFEConfig.c2StreamerConfig.perfTune.useBufferIo != 0;
	conf.mIndexRecordCacheSize = gEdgeFEConfig.c2StreamerConfig.perfTune.indexRecordCacheSize;
	if( conf.mIndexRecordCacheSize < 1)
		conf.mIndexRecordCacheSize = 1;
	conf.attributesTimeoutInPwe = gEdgeFEConfig.c2StreamerConfig.perfTune.attrTimeout;
	conf.mBitrateInflationPercent = gEdgeFEConfig.c2StreamerConfig.perfTune.bitrateInflation;

	//AquaReader part
	conf.aquaReaderRootUrl = gEdgeFEConfig.c2StreamerConfig.aquaReaderConfig.rootUrl;
	conf.aquaReaderUserDomain = gEdgeFEConfig.c2StreamerConfig.aquaReaderConfig.userDomain;
	conf.aquaReaderHomeContainer = gEdgeFEConfig.c2StreamerConfig.aquaReaderConfig.homeContainer;
	conf.aquaReaderFlags = gEdgeFEConfig.c2StreamerConfig.aquaReaderConfig.cdmiFlags;
	conf.aquaReaderIoThreadPoolSize = gEdgeFEConfig.c2StreamerConfig.aquaReaderConfig.ioThreadPool;
	conf.aquaReaderCdmiOpsReadThreadPoolSize = gEdgeFEConfig.c2StreamerConfig.aquaReaderConfig.cdmiOpsThreadPool;
	conf.aquaReaderCacheBlockSize = gEdgeFEConfig.c2StreamerConfig.perfTune.cacheBufferSize;
	conf.aquaReaderCacheBlockCount = gEdgeFEConfig.c2StreamerConfig.aquaReaderConfig.cacheBufferCount;
	conf.aquaReaderHlsEnableMDataQuery = gEdgeFEConfig.c2StreamerConfig.aquaReaderConfig.aquaReaderHlsEnableMDataQuery;



	(getSsEnv()->getMainLogger())(ZQ::common::Log::L_INFO,CLOGFMT(setC2Conf,"set index record cache size: %d"),conf.mIndexRecordCacheSize);

	conf.clientType = gEdgeFEConfig.c2StreamerConfig.clientType;
	if (conf.clientType == 1)
	{
		  (getSsEnv()->getMainLogger())(ZQ::common::Log::L_DEBUG,CLOGFMT(setC2Conf,"the c2streamer will be c2client"));
		  conf.C2ClientUpStreamIP 		  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientUpStreamIP;
		  conf.C2ClientURL 				  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientURL;
		  conf.C2ClientTransfer 		  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientTransfer;
		  conf.C2ClientHttpCRGAddr 		  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientHttpCRGAddr;
		  conf.C2ClientHttpCRGPort 		  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientHttpCRGPort;
		  conf.C2ClientTransferRate 	  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientTransferRate;
		  conf.C2ClientIngressCapacity 	  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientIngressCapacity;
		  conf.C2ClientExclusionList 	  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientExclusionList;
		  conf.C2ClientTransferDelay 	  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientTransferDelay;
		  conf.C2ClientDefaultGetPort 	  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientDefaultGetPort;
		  conf.C2ClientWaitBufferTime 	  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientWaitBufferTime;
		  conf.C2ClientIndexTimeout 	  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientIndexTimeout;
		  conf.C2ClientIndexRetryTimes 	  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientIndexRetryTimes;
		  conf.C2ClientMainfileTimeout 	  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientMainfileTimeout;
		  conf.C2ClientMainfileRetryTimes = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientMainfileRetryTimes;
	}
	else if (conf.clientType == 2  || conf.clientType == 4 )
	{
		  ...
		  conf.httpProxyURL = gEdgeFEConfig.c2StreamerConfig.httpclient.httpProxyURL;
		  conf.segmentURL = gEdgeFEConfig.c2StreamerConfig.httpclient.segmentURL;
		  conf.httpTimeOut = gEdgeFEConfig.c2StreamerConfig.httpclient.httpTimeOut;
		  conf.httpRetry = gEdgeFEConfig.c2StreamerConfig.httpclient.httpRetry;
	}
	//disk cache 
	conf.diskCacheEnabled = false;
	if( 0 != gEdgeFEConfig.c2StreamerConfig.diskcache.diskCacheEnabled )
	{	
		conf.diskCacheEnabled = true;
		conf.cacheLoaderTimeout = gEdgeFEConfig.c2StreamerConfig.diskcache.cacheLoaderTimeout;
		conf.cacheIgnoreFiles = gEdgeFEConfig.c2StreamerConfig.diskcache.cacheIgnoreFiles;
		C2Streamer::DISKCACHECON& dcc = conf.diskCacheConfig;
		const std::vector<DiskCache::CACHEDIR>& cacheDir = gEdgeFEConfig.c2StreamerConfig.diskcache.cacheDir;
	...
	conf.forwardURL = gEdgeFEConfig.cacheStoreConfig.forwardURL;
	conf.defaultTimeOut = gEdgeFEConfig.c2StreamerConfig.c2bufrm.defaultTimeOut;
	conf.fioErrTimeOut = gEdgeFEConfig.c2StreamerConfig.c2bufrm.fioErrTimeOut;
	conf.httpErrTimeOut = gEdgeFEConfig.c2StreamerConfig.c2bufrm.httpErrTimeOut;
	conf.socketErrTimeOut = gEdgeFEConfig.c2StreamerConfig.c2bufrm.socketErrTimeOut;
	conf.otherErrTimeOut = gEdgeFEConfig.c2StreamerConfig.c2bufrm.otherErrTimeOut;

	int32 serviceThreadCount = gEdgeFEConfig.c2StreamerConfig.serviceThreadCount;
	serviceThreadCount = MAX( serviceThreadCount, 3 );
	int32 locateThreadCount = gEdgeFEConfig.c2StreamerConfig.locateThreadCount;
...
bool CdnStreamerManager::setC2Conf( C2Streamer::C2StreamerEnv& c2env ) {
	...
	conf.enableC2ClientHybrid = gEdgeFEConfig.c2StreamerConfig.enableC2ClientHybrid != 0;
	...
	std::vector<C2StreamerSvcConfig::C2StreamerUrlRuleHolder>::const_iterator itUrlRule = gEdgeFEConfig.c2StreamerConfig.urlRules.begin();
	for( ; itUrlRule != gEdgeFEConfig.c2StreamerConfig.urlRules.end(); itUrlRule ++ ) {
		C2Streamer::ConfPerSessionConfig perSessConf;
		C2Streamer::ConfWriter& c2Writer = perSessConf.writer;

		std::map<std::string, C2StreamerSvcConfig::C2StreamerWriterHolder>& confWriters = gEdgeFEConfig.c2StreamerConfig.writers;
		...
		c2Writer.udpSessMinStreamInterval = confWriter.udpSessMinStreamInterval;
		...
		if(c2Rule.readerType == C2Streamer::CLIENT_TYPE_C2CLIENT && conf.enableC2ClientHybrid ) {
			c2Rule.readerType = C2Streamer::CLIENT_TYPE_HYBRID_C2CLIENT;
		}
	...
	c2Hls.mKeyFile5I			= gEdgeFEConfig.hlsServerConfig.keyfile;
	c2Hls.mAquaRootUrl		= gEdgeFEConfig.hlsServerConfig.rooturl;
	c2Hls.mHomeContainer		= gEdgeFEConfig.hlsServerConfig.homecontainer;
	c2Hls.mServerHostUrl		= gEdgeFEConfig.hlsServerConfig.serverHostUrl;
	c2Hls.mLogFlag			= gEdgeFEConfig.hlsServerConfig.logFlag;
	c2Hls.mName2Bitrate		= gEdgeFEConfig.hlsServerConfig.subname2bitrate;
	c2Hls.mDefaultBitrate	= gEdgeFEConfig.hlsServerConfig.defaultBitrate;

	//CacheCenter
	C2Streamer::ConfCacheCenter& c2CacheCenter = conf.cacheCenter;
	ZQ::common::Config::Holder<CacheCenterConfig>& confCacheCenter = gEdgeFEConfig.c2StreamerConfig.c2CacheCenter;
	...
	c2CacheCenter.defaultTimeOut = gEdgeFEConfig.c2StreamerConfig.c2CacheCenter.defaultTimeOut;
	c2CacheCenter.fioErrTimeOut = gEdgeFEConfig.c2StreamerConfig.c2CacheCenter.fioErrTimeOut;
	c2CacheCenter.httpErrTimeOut = gEdgeFEConfig.c2StreamerConfig.c2CacheCenter.httpErrTimeOut;
	c2CacheCenter.socketErrTimeOut = gEdgeFEConfig.c2StreamerConfig.c2CacheCenter.socketErrTimeOut;
	c2CacheCenter.otherErrTimeOut = gEdgeFEConfig.c2StreamerConfig.c2CacheCenter.otherErrTimeOut;



	//AquaReader part
	C2Streamer::ConfAquaReader& c2AquaReader = conf.aquaReader;
	c2AquaReader.aquaReaderRootUrl = gEdgeFEConfig.c2StreamerConfig.aquaReaderConfig.rootUrl;
	c2AquaReader.aquaReaderUserDomain = gEdgeFEConfig.c2StreamerConfig.aquaReaderConfig.userDomain;
	c2AquaReader.aquaReaderHomeContainer = gEdgeFEConfig.c2StreamerConfig.aquaReaderConfig.homeContainer;
	c2AquaReader.aquaReaderMainFileExtension = gEdgeFEConfig.c2StreamerConfig.aquaReaderConfig.mainFileExtension;
	c2AquaReader.aquaReaderContentNameFormat = gEdgeFEConfig.c2StreamerConfig.aquaReaderConfig.contentNameFormat;
	c2AquaReader.aquaReaderFlags = gEdgeFEConfig.c2StreamerConfig.aquaReaderConfig.cdmiFlags;
	c2AquaReader.aquaReaderIoThreadPoolSize = gEdgeFEConfig.c2StreamerConfig.aquaReaderConfig.ioThreadPool;
	c2AquaReader.aquaReaderCdmiOpsReadThreadPoolSize = gEdgeFEConfig.c2StreamerConfig.aquaReaderConfig.cdmiOpsThreadPool;
	c2AquaReader.aquaReaderCacheBlockSize = gEdgeFEConfig.c2StreamerConfig.c2CacheCenter.cacheBufferSize;
	c2AquaReader.aquaReaderCacheBlockCount = gEdgeFEConfig.c2StreamerConfig.aquaReaderConfig.cacheBufferCount;
	c2AquaReader.aquaReaderHlsEnableMDataQuery = gEdgeFEConfig.c2StreamerConfig.aquaReaderConfig.aquaReaderHlsEnableMDataQuery;

	
	//C2Client Part
	C2Streamer::ConfC2Client& c2Client = conf.c2Client;
	c2Client.C2ClientUpStreamIP 		  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientUpStreamIP;
	c2Client.C2ClientURL 				  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientURL;
	c2Client.C2ClientTransfer 		  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientTransfer;
	c2Client.C2ClientHttpCRGAddr 		  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientHttpCRGAddr;
	c2Client.C2ClientHttpCRGPort 		  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientHttpCRGPort;
	c2Client.C2ClientTransferRate 	  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientTransferRate;
	c2Client.C2ClientIngressCapacity 	  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientIngressCapacity;
	c2Client.C2ClientExclusionList 	  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientExclusionList;
	c2Client.C2ClientTransferDelay 	  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientTransferDelay;
	c2Client.C2ClientDefaultGetPort 	  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientDefaultGetPort;
	c2Client.C2ClientWaitBufferTime 	  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientWaitBufferTime;
	c2Client.C2ClientIndexTimeout 	  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientIndexTimeout;
	c2Client.C2ClientIndexRetryTimes 	  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientIndexRetryTimes;
	c2Client.C2ClientMainfileTimeout 	  = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientMainfileTimeout;
	c2Client.C2ClientMainfileRetryTimes = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientMainfileRetryTimes;
	c2Client.C2ClientMinTransferRate = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientMinTransferRate;
	c2Client.C2ClientBitrateInflate = gEdgeFEConfig.c2StreamerConfig.c2client.C2ClientBitrateInflate;


	//HttpClient Part
	C2Streamer::ConfHttpClient& c2HttpClient = conf.httpClient;
	c2HttpClient.httpProxyURL = gEdgeFEConfig.c2StreamerConfig.httpclient.httpProxyURL;
	c2HttpClient.segmentURL = gEdgeFEConfig.c2StreamerConfig.httpclient.segmentURL;
	c2HttpClient.httpTimeOut = gEdgeFEConfig.c2StreamerConfig.httpclient.httpTimeOut;
	c2HttpClient.httpRetry = gEdgeFEConfig.c2StreamerConfig.httpclient.httpRetry;

	//DiskCache Part
	C2Streamer::ConfDiskCache& c2DiskCache = conf.diskCache;
	c2DiskCache.diskCacheEnabled = gEdgeFEConfig.c2StreamerConfig.diskcache.diskCacheEnabled != 0;
	c2DiskCache.cacheLoaderTimeout = gEdgeFEConfig.c2StreamerConfig.diskcache.cacheLoaderTimeout;
	c2DiskCache.cacheIgnoreFiles = gEdgeFEConfig.c2StreamerConfig.diskcache.cacheIgnoreFiles;
	C2Streamer::DISKCACHECON& dcc = c2DiskCache.diskCacheConfig;
	const std::vector<DiskCache::CACHEDIR>& cacheDir = gEdgeFEConfig.c2StreamerConfig.diskcache.cacheDir;
	std::vector<DiskCache::CACHEDIR>::const_iterator iterDiskCache = cacheDir.begin();
	int32 totalBufferCount = c2CacheCenter.mCacheDefaultBufferCount;
	if(c2DiskCache.diskCacheEnabled) {
		for( ; iterDiskCache != cacheDir.end() ; iterDiskCache ++ )	{
			if( iterDiskCache->homePath.empty() ) {
				continue;
			}
			...
			diskConfg.writeThreadCount = iterDiskCache->writeThreadCount;
			diskConfg.pendingsYield = iterDiskCache->pendingsYield;
			diskConfg.maxWriteMBps = iterDiskCache->maxWriteMBps;
			dcc.push_back(diskConfg);
		}
	}
...
bool CdnStreamerManager::startupC2Streamer()
{
	  ...
	  std::vector<int> cores;
	  {
		  ...
		  ZQ::common::stringHelper::SplitString(gEdgeFEConfig.c2StreamerConfig.cpuCores, cpuCores,","," ,\t\v");
		  if( !cpuCores.empty() ) {
	...
	mC2Service->getEventPublisher().setSinker( pSinker , C2Streamer::METHOD_SESS_UPDATE |
														C2Streamer::METHOD_RES_UPDATE | 
														C2Streamer::METHOD_UDP_EVENT );
	...
	//int32 httpThreadCount = gEdgeFEConfig.c2StreamerConfig.httpThreadCount;
	//httpThreadCount = MAX(httpThreadCount , 3);

	ENVLOG(ZQ::common::Log::L_INFO,CLOGFMT(CdnStreamerManager,"Start Http Engine at[%s][%s]"),
		 gEdgeFEConfig.c2StreamerConfig.httpBindIp .c_str(), gEdgeFEConfig.c2StreamerConfig.httpBindPort.c_str() );

	//if( gEdgeFEConfig.c2StreamerConfig.serverType == 1 ) {
	if(1) {
		...
		if(!mLibAsyncHttpServer->startAt(gEdgeFEConfig.c2StreamerConfig.httpBindIp, gEdgeFEConfig.c2StreamerConfig.httpBindPort )){
		...
		ENVLOG(ZQ::common::Log::L_INFO,CLOGFMT(CdnStreamerManager,"start async server at[%s:%s]"),
					gEdgeFEConfig.c2StreamerConfig.httpBindIp.c_str(), gEdgeFEConfig.c2StreamerConfig.httpBindPort.c_str());
//	}  else {
//		mC2HttpEngine->setEndpoint( gEdgeFEConfig.c2StreamerConfig.httpBindIp , gEdgeFEConfig.c2StreamerConfig.httpBindPort );
...
void CdnStreamerManager::queryStreamersInfo()
{
	...
	if( C2Streamer::cResourceStatus( request , response ) == C2Streamer::errorCodeOK )
	{
		...
		for( ; itPortInfo != retInfos.end() ; itPortInfo++ )
		{
			...
			attr.transferTcpPort		=	gEdgeFEConfig.c2StreamerConfig.httpBindPort;
...
void CdnStreamerManager::getShadowIndex(const std::string& upstreamUrl, const std::string& name,
		...
		const std::string& subtype  )
{
	static int clientType = gEdgeFEConfig.c2StreamerConfig.defaultReaderType;
	if( clientType == 2 ) { //http fetcher
		...
		if( subtype.length() > 0 &&subtype.at(0) != '.')
			filename = filename + ".";
		filename += subtype;
		c2sessmanager.getAssetAttribute("", filename, gEdgeFEConfig.c2StreamerConfig.defaultReaderType);
		return;
	}
	std::string key = pid+paid;
	ZQ::common::MutexGuard gd(mC2ShadowIndexMutex);
...
int32 C2EventSinkerI::publish(const C2Streamer::C2EventPtr request)
{
	...
	case C2Streamer::METHOD_RES_UPDATE:
		...
		break;
	case C2Streamer::METHOD_UDP_EVENT:
		{
			C2Streamer::C2UdpSessionEventPtr e = C2Streamer::C2UdpSessionEventPtr::dynamicCast( request );
			if(e) {
				return onSessEvent(e);
			}
		}
...
TianShanIce::Streamer::StreamState c2UdpStateToStreamState( C2Streamer::UdpSessionState state ) {
	switch(state) {
	case C2Streamer::UDPSTATE_NONE:
	case C2Streamer::UDPSTATE_INIT:
		return TianShanIce::Streamer::stsSetup;
	case C2Streamer::UDPSTATE_PLAY:
		return TianShanIce::Streamer::stsStreaming;
	case C2Streamer::UDPSTATE_PAUSE:
		return TianShanIce::Streamer::stsPause;
	case C2Streamer::UDPSTATE_DONE:
...
int32 C2EventSinkerI::onSessEvent( const C2Streamer::C2UdpSessionEventPtr e) {
	StreamParams paras;
	TianShanIce::Properties props;
	std::ostringstream ossId;
	ossId << e->subSessId;
	std::string sessId = ossId.str();
	switch(e->event) {
	case C2Streamer::C2UdpSessionEvent::UDPEVENT_STATE_CHANGE:
		{
			paras.streamState = c2UdpStateToStreamState(e->newStatus.state);
			mSvc.OnStreamEvent( SsServiceImpl::seStateChanged, sessId, paras, props );
		}
		break;
	case C2Streamer::C2UdpSessionEvent::UDPEVENT_SCALE_CHANGE:
		{
			paras.scale = e->newStatus.scale;
			mSvc.OnStreamEvent( SsServiceImpl::seScaleChanged, sessId, paras, props );
		}
		break;
	case C2Streamer::C2UdpSessionEvent::UDPEVENT_SESSION_DONE:
		{
			mSvc.OnStreamEvent( SsServiceImpl::seGone, sessId, paras, props );
		}
		break;
	default:
		assert( false && "logic error");
		...
		ZQ::IdxParser::IndexData& idxData)
{
	...
	{
		ZQTianShan::Util::updatePropertyData(props,METADATA_MAIFILE_EXTNAME(),subinfo.fileExtension );
	}

	ZQTianShan::Util::updatePropertyData(props, METADATA_BitRate , idxData.getMuxBitrate() );
	ZQTianShan::Util::updatePropertyData(props, METADATA_MAIFILENAME() , idxData.getMainFilePathName() );
	...
	ZQTianShan::Util::updatePropertyData(props, METADATA_INDEXFILENAME() , tmpIndexFileName);
...
{
	static int clientType = gEdgeFEConfig.c2StreamerConfig.defaultReaderType;
	static const std::string& proxyUrl =  gEdgeFEConfig.c2StreamerConfig.httpclient.httpProxyURL;
	static const std::string& segmentUrl = gEdgeFEConfig.c2StreamerConfig.httpclient.segmentURL;
