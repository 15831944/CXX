...
void setCacheStoreConfig( ZQTianShan::ContentStore::CacheStoreImpl* cache ) 
{
	...
	const ZQ::StreamService::CacheStoreConfig&	config = gEdgeFEConfig.cacheStoreConfig;

	cache->_groupBind = ZQ::common::InetHostAddress( gEdgeFEConfig.csEventMulticastLocalIp.c_str() );// use FileEvent multicast local bind ip as well
...
void cache_resetSnmpStat(ZQ::common::ServiceMIB::Ptr pMib)
{
	...
	pMib->addObject(new ZQ::SNMP::SNMPObject("cdnssStat-cache-Measure-Since", &strMeasuredSince, ZQ::SNMP::AsnType_String));
}
		...
		case 0:
			{
				...
				if (!bRegistered)
				{
					...
					pMib->addObject(new ZQ::SNMP::SNMPObject("cdnssStat-cache-missedSize",       &cacheStore->_sizeMissed,     ZQ::SNMP::AsnType_Int32));
					pMib->addObject(new ZQ::SNMP::SNMPObject("cdnssStat-cache-hotLocalsSize",    &cacheStore->_sizeHotLocals,  ZQ::SNMP::AsnType_Int32));
					pMib->addObject(new ZQ::SNMP::SNMPObject("cdnssStat-cache-requestsInTimeWin",&cacheStore->_reqsMissedInWin,ZQ::SNMP::AsnType_Int32));
					pMib->addObject(new ZQ::SNMP::SNMPObject("cdnssStat-cache-hitInTimeWin",     &cacheStore->_reqsHotLocalsInWin,  ZQ::SNMP::AsnType_Int32));
					bRegistered = true;
				}
			}
			break;
...
{
	if(gEdgeFEConfig.cacheStoreConfig.enable <= 0 )
	{
	...
	std::string endpoint = gEdgeFEConfig.serviceEndpoint;
...
Ice::ObjectPrx StartContentStore(ZQADAPTER_DECLTYPE& adapter, const std::string& serviceName, const std::string& logDir, ZQ::common::Log& log )
{
    try
	{
        ...
        ncsMainLog.open(file.c_str(), gEdgeFEConfig.mainLogLevel, ZQLOG_DEFAULT_FILENUM, 
                        gEdgeFEConfig.mainLogFileSize, gEdgeFEConfig.mainLogBufferSize, gEdgeFEConfig.mainLogTimeout);
    }
	...
	try
	{
		...
		cacheMainLog.open( file.c_str(), gEdgeFEConfig.mainLogLevel, ZQLOG_DEFAULT_FILENUM,
						gEdgeFEConfig.mainLogFileSize, gEdgeFEConfig.mainLogBufferSize, gEdgeFEConfig.mainLogTimeout);
	}
	catch( const ZQ::common::FileLogException& ex)
	{
    ...
    try {
        ...
        ncsEventLog.open(file.c_str(), gEdgeFEConfig.eventLogLevel, ZQLOG_DEFAULT_FILENUM, 
                         gEdgeFEConfig.eventLogFileSize, gEdgeFEConfig.eventLogBufferSize, gEdgeFEConfig.eventLogTimeout);
    }
    catch(const ZQ::common::FileLogException& ex) {
    ...
    try 
	{
        ...
        embedCS->_netId       = gEdgeFEConfig.netId;
        embedCS->_replicaGroupId = gEdgeFEConfig.csStrReplicaGroupId;
        embedCS->_replicaId = gEdgeFEConfig.netId;
        embedCS->_replicaPriority = gEdgeFEConfig.csReplicaPriority;
        embedCS->_replicaTimeout = gEdgeFEConfig.csReplicaTimeout;
        embedCS->_cacheLevel = gEdgeFEConfig.cacheLevel;
        embedCS->_cacheable = gEdgeFEConfig.isCacheMode; 
        embedCS->_contentEvictorSize = gEdgeFEConfig.contentEvictorSize;
        embedCS->_volumeEvictorSize = gEdgeFEConfig.volumeEvictorSize;
        embedCS->_storeFlags = STOREFLAG_populateSubfolders;
        embedCS->_timeoutNotProvisioned = gEdgeFEConfig.csTimeoutNotProvisioned;
        embedCS->_timeoutIdleProvisioning = gEdgeFEConfig.csTimeoutIdleProvisioning;
        ...
        if(!gEdgeFEConfig.csMasterEndpoint.empty()) {
            std::ostringstream oss;
            oss << SERVICE_NAME_ContentStore << ":" << gEdgeFEConfig.csMasterEndpoint;
            embedCS->subscribeStoreReplica(oss.str(), true); 
        }
        
        bool bMounted = false;
		int64 startTime = ZQ::common::now();
		int64 timeoutInterval = gEdgeFEConfig.csVolCheckTimeout;
		timeoutInterval = MAX( 2000, timeoutInterval );
		do
		{
			MountpointTypeChecker typechecker;
        	typechecker.init();
			ZQ::StreamService::EdgeFEConfig::Volumes::const_iterator iter = gEdgeFEConfig.volumes.begin();
			for(; iter != gEdgeFEConfig.volumes.end(); ++iter) 
			{
