										...
										INOUT TianShanIce::Streamer::PlaylistItemSetupInfo& info )
{
	...
	assert( cdnEnv != NULL );
//	CdnStreamerManager& manager = cdnEnv->getStreamerManager();
	//std::string::size_type pos = info.contentName.find_last_of('/');
	//if (pos != std::string::npos)
	//{
	//	info.contentName.substr(pos+1);
	//}
								...
								OUT std::string&	streamId )
{
 ...
 //	CdnStreamerManager& manager = cdnEnv->getStreamerManager();
 	CdnSsCommand command(cdnEnv,ss,ctx);
 	return command.doLoad(itemInfo ,streamId, timeoffset, scale, ctrlParams);		
}
...
{	
	CdnSsEnvironment* cdnEnv = dynamic_cast<CdnSsEnvironment*>(getSsEnv());
 	assert( cdnEnv != NULL );
	CdnSsCommand command(cdnEnv, ss, ctx );
	return command.doPlay( streamId, timeOffset, scale, ctrlParams);
}
								...
								INOUT StreamParams& ctrlParams )
{
	CdnSsEnvironment* cdnEnv = dynamic_cast<CdnSsEnvironment*>(getSsEnv());
 	assert( cdnEnv != NULL );
	CdnSsCommand command(cdnEnv, ss, ctx );
	return command.doPause( streamId, ctrlParams );
}
								...
								INOUT StreamParams& ctrlParams )
{
	CdnSsEnvironment* cdnEnv = dynamic_cast<CdnSsEnvironment*>(getSsEnv());
 	assert( cdnEnv != NULL );
	CdnSsCommand command(cdnEnv, ss, ctx );
	return command.doUdpPlay( streamId, -1, 0.0f, ctrlParams );
}
								...
								INOUT StreamParams& ctrlParams )
{
	CdnSsEnvironment* cdnEnv = dynamic_cast<CdnSsEnvironment*>(getSsEnv());
 	assert( cdnEnv != NULL );
	CdnSsCommand command(cdnEnv, ss, ctx );
	return command.doUdpPlay( streamId, timeOffset, scale, ctrlParams );
}
...

{
	CdnSsEnvironment* cdnEnv = dynamic_cast<CdnSsEnvironment*>(getSsEnv());
 	assert( cdnEnv != NULL );
	CdnSsCommand command(cdnEnv, ss, ctx );
	return command.doUdpPlay( streamId, -1, newScale, ctrlParams );
}
									...
									OUT StreamParams& ctrlParams )
{
	...
	assert( cdnEnv != NULL );
	CdnSsCommand command(cdnEnv,ss,ctx);
	return command.doGetStreamAttr( streamId, info, ctrlParams);
}
...
int CdnSsCommand::getStreamerType() const {
	const TianShanIce::SRM::ResourceMap& res = mStreamCtx.getContextResources();
	Ice::Int type = 0;
	
	ZQTianShan::Util::getResourceDataWithDefault(res, TianShanIce::SRM::rtStreamer, "Type", 0, type, 0);

	return (int)type;
}

int32 CdnSsCommand::doUdpGetStreamAttr( const std::string& streamId, const TianShanIce::Streamer::PlaylistItemSetupInfo& info, StreamParams& params ) {
	ENVLOG.debug(PLFMT(doUdpGetStreamAttr, "trying to get stream attr for[%s]"), streamId.c_str());
	int64 id = 0;
	sscanf(streamId.c_str(),"%ld", &id);
	SessionUdpControlRequestParamPtr request = new SessionUdpControlRequestParam(*mStreamerManager.getC2Env(), URLRULE_UDPSESSION);
	SessionUdpControlResponseParamPtr response = new SessionUdpControlResponseParam();

	request->requestHint = mStreamCtx.id();
	request->sessionId = mStreamCtx.id();
	if (params.mask & MASK_STREAM_STATE)
		request->subSessionId = 0;
	else
		request->subSessionId = id;
	request->assetName = info.contentName;
	request->method = C2Streamer::METHOD_UDP_RUN;
	request->ctrlCode = C2Streamer::SessionUdpControlRequestParam::CONTROL_INFOQUERY;
	int32 iRet = 0;
	if (request->subSessionId == 0){
		iRet = C2Streamer::cUdpInfoQuery( request, response);
	}
	else {
		iRet = C2Streamer::cUdpRun( request, response);
	}
	if( iRet != C2Streamer::errorCodeOK ) {
		ENVLOG.error(PLFMT(doUdpGetStreamAttr, "failed to get stream attr for subSession[%s]: %s"), streamId.c_str(), response->errorText.c_str());
		return ERR_RETURN_INVALID_PARAMATER;
	}
	params.timeoffset = response->timeOffset;
	params.duration = response->duration;
	params.scale = response->scale;
	params.streamState = TianShanIce::Streamer::stsStreaming;
	params.mask = MASK_TIMEOFFSET | MASK_SCALE | MASK_STREAM_STATE;
	if(params.duration > 0 ) {
		params.mask |= MASK_CONTENT_DURATION;
	}
	return ERR_RETURN_SUCCESS;
}

int32 CdnSsCommand::doGetStreamAttr( const std::string& streamId, const TianShanIce::Streamer::PlaylistItemSetupInfo& info, StreamParams& params ) {
	int type = 0;
	if( (type =  getStreamerType() ) != 0 ) {
		return doUdpGetStreamAttr(streamId, info, params);
	}
	return ERR_RETURN_NOT_SUPPORT;
}

int32	CdnSsCommand::doUdpPlay( const std::string& streamId, int64 timeOffset, float scale, StreamParams& ctrlParams) {
	int64 subSessionId = 0;
	sscanf(streamId.c_str(), "%ld", &subSessionId);
	SessionUdpControlRequestParamPtr request = new SessionUdpControlRequestParam(*mStreamerManager.getC2Env(), URLRULE_UDPSESSION);
	SessionUdpControlResponseParamPtr response = new SessionUdpControlResponseParam();

	request->requestHint = mStreamCtx.id();
	request->method = C2Streamer::METHOD_UDP_RUN;
	request->ctrlCode = SessionUdpControlRequestParam::CONTROL_PLAY;
	request->sessionId = mStreamCtx.id();
	request->subSessionId = subSessionId;
	request->timeOffset = timeOffset;
	request->inTimeOffset = ctrlParams.inTimeOffset;
	request->outTimeOffset = ctrlParams.outTimeOffset;
	request->inTimeOffset = 
	request->scale = scale;

	int32 iRet = C2Streamer::cUdpRun( request, response );
	if( iRet != 200 ) {
		ZQTianShan::_IceThrow<TianShanIce::ServerError>( ENVLOG , "CDNSS" , iRet , PLFMT(doPlay,"failed to play stream[%s]"), streamId.c_str());
	} else {
		ctrlParams.mask = MASK_TIMEOFFSET | MASK_SCALE;
		ctrlParams.timeoffset = response->timeOffset;
		ctrlParams.scale = response->scale;
		if( response->duration > 0 ) {
			ctrlParams.mask |= MASK_CONTENT_DURATION;
			ctrlParams.duration = response->duration;
		}
		ENVLOG.info(PLFMT(doPlay,"play stream[%s] successfully, status: timeOffset[%ld] scale[%f] duration[%ld]"), 
				streamId.c_str(), ctrlParams.timeoffset, ctrlParams.scale, ctrlParams.duration);
	}
	return ERR_RETURN_SUCCESS;
}

int32	CdnSsCommand::doPlay( const std::string& streamId, int64 timeOffset, float scale, StreamParams& ctrlParams) {
	int type = 0;
	if(( type = getStreamerType() ) != 0 ) {
		return doUdpPlay( streamId, timeOffset, scale, ctrlParams);
	}
	return ERR_RETURN_SUCCESS;
}

int32 CdnSsCommand::doUdpPause( const std::string& streamId, StreamParams& ctrlParams ) {
	int64 subSessionId = 0;
	sscanf(streamId.c_str(), "%ld", &subSessionId);
	SessionUdpControlRequestParamPtr request = new SessionUdpControlRequestParam(*mStreamerManager.getC2Env(), URLRULE_UDPSESSION);
	SessionUdpControlResponseParamPtr response = new SessionUdpControlResponseParam();
	...
	request->requestHint = mStreamCtx.id();
	request->method = C2Streamer::METHOD_UDP_RUN;
	request->ctrlCode = SessionUdpControlRequestParam::CONTROL_PAUSE;
	request->sessionId = mStreamCtx.id();
	request->subSessionId = subSessionId;

	int32 iRet = C2Streamer::cUdpRun( request, response );
	if( iRet != 200 ) {
		ZQTianShan::_IceThrow<TianShanIce::ServerError>( ENVLOG , "CDNSS" , iRet , PLFMT(doPlay,"failed to pause stream[%s]"), streamId.c_str());
	} else {
		ctrlParams.mask = MASK_TIMEOFFSET | MASK_SCALE;
		ctrlParams.timeoffset = response->timeOffset;
		ctrlParams.scale = response->scale;
		...
		ENVLOG.info(PLFMT(doPlay,"plause stream[%s] successfully, status: timeOffset[%ld] scale[%f] duration[%ld]"), 
				streamId.c_str(), ctrlParams.timeoffset, ctrlParams.scale, ctrlParams.duration);
	}
	return ERR_RETURN_SUCCESS;
...
int32 CdnSsCommand::doPause( const std::string& streamId, StreamParams& params ) {
	int type = 0;
	if(( type = getStreamerType() ) != 0 ) {
		return doUdpPause( streamId, params );
	}
	return ERR_RETURN_NOT_SUPPORT;
}

int32 CdnSsCommand::doUdpLoad( const TianShanIce::Streamer::PlaylistItemSetupInfo& info, std::string& sessId, int64 timeoffset, float scale, StreamParams& params) {
	ENVLOG.debug(PLFMT(doUdpLoad, "trying to load item[%s] with timeoffset[%ld] scale[%f]"), info.contentName.c_str() ,timeoffset, scale);
	SessionUdpControlRequestParamPtr request = new SessionUdpControlRequestParam(*mStreamerManager.getC2Env(), URLRULE_UDPSESSION);
	SessionUdpControlResponseParamPtr response = new SessionUdpControlResponseParam();
	...
	request->sessionId = mStreamCtx.id();
	request->method = C2Streamer::METHOD_UDP_RUN;
	request->ctrlCode = SessionUdpControlRequestParam::CONTROL_LOAD;
	request->assetName = info.contentName;
	request->timeOffset = timeoffset;
	request->scale = scale;
	request->inTimeOffset = params.inTimeOffset;
	request->outTimeOffset = params.outTimeOffset;

	int32 iRet = errorCodeOK;
		iRet = C2Streamer::cUdpRun(request, response);

	if( iRet != errorCodeOK ) {
		ENVLOG(ZQ::common::Log::L_INFO, PLFMT(doUdpLoad, "failed to load item[%s] due to: %s"), info.contentName.c_str(), response->errorText.c_str() );
		return ERR_RETURN_INVALID_PARAMATER;
	}
	std::ostringstream oss;
	oss << response->subSessionId;
	sessId = oss.str();
	ENVLOG.info(PLFMT(doUdpLoad,"succesfully load item[%s], sess[%s]"), info.contentName.c_str(), sessId.c_str() );
	return ERR_RETURN_SUCCESS;
}

int32 CdnSsCommand::doLoad( const TianShanIce::Streamer::PlaylistItemSetupInfo& info,  std::string& sessId, int64 timeoffset, float scale, StreamParams& params )
{
	int type = 0;
	if((type = getStreamerType()) != 0 ) {
		return doUdpLoad( info, sessId, timeoffset, scale, params);
	}
	...
	try
	{
		...
		{	
			...
			//ZQTianShan::Util::updateValueMapData( values , CDN_TRANSFERPORTNUM, gEdgeFEConfig.c2StreamerConfig.httpBindPort );
...
int32 CdnSsCommand::doUdpCommit( const ZQ::StreamService::PlaylistItemSetupInfos& items,
							 TianShanIce::SRM::ResourceMap& crResource) {

	C2Streamer::UdpStreamInitRequestParamPtr request = new C2Streamer::UdpStreamInitRequestParam(*mStreamerManager.getC2Env(), URLRULE_UDPSESSION);
	C2Streamer::UdpStreamInitResponseParamPtr response = new C2Streamer::UdpStreamInitResponseParam();

	request->requestHint = mStreamCtx.id();
	request->sessionId = mStreamCtx.id();
	request->method = C2Streamer::METHOD_UDP_INIT;
	request->ingressCapacity = ((long long)1) << 60; //big enough

	std::string hint = mStreamCtx.id() + " Dump Resource ";
	ZQTianShan::Util::dumpTianShanResourceMap( crResource, ENVLOG, mStreamCtx.id()+" Dump Resource ");
	Ice::Int destPort = 0;
	ZQTianShan::Util::getResourceDataWithDefault(crResource, TianShanIce::SRM::rtEthernetInterface, "destIP", "", request->udpClientAddress );
	ZQTianShan::Util::getResourceDataWithDefault(crResource, TianShanIce::SRM::rtEthernetInterface, "destPort", 0, destPort);
	request->udpClientPort = destPort;
	ZQTianShan::Util::getResourceDataWithDefault(crResource, TianShanIce::SRM::rtEthernetInterface, "srcIP" , "", request->udpServerAddress);
	ZQTianShan::Util::getResourceDataWithDefault(crResource, TianShanIce::SRM::rtTsDownstreamBandwidth, "bandwidth", 0, request->transferRate);

	response->responseHandler = new C2Streamer::C2UdpResponseHandler( request->udpClientAddress, request->udpClientPort, ENVLOG);

	ENVLOG.info(PLFMT(doUdpCommit, "init udp stream: bitrate[%ld] src[%s] destination;%s:%hu]"), 
			request->transferRate, request->udpServerAddress.c_str(),
			request->udpClientAddress.c_str(), request->udpClientPort );

	int32 iRet = C2Streamer::cUdpInit(request, response);
	if( iRet != C2Streamer::errorCodeOK )
	{
		switch( iRet )
		{
		case C2Streamer::errorCodeContentNotFound:
			{
				ZQTianShan::_IceThrow<TianShanIce::InvalidParameter>( ENVLOG , "CDNSS" , TianShanIce::errcodeAssetNotfound , PLFMT(doCommit,"failed to commit[%s] because[%s]"),
						mStreamCtx.id().c_str(), response->errorText.c_str() );

			}
			break;
		default:
			{
				ZQTianShan::_IceThrow<TianShanIce::ServerError>( ENVLOG , "CDNSS" , iRet , PLFMT(doCommit,"failed to commit[%s] because[%s]"),
						mStreamCtx.id().c_str(), response->errorText.c_str() );
			}
			break;
		}
	...
	{
		ENVLOG.info(PLFMT(doUdpCommit,"commit[%s] ok"), mStreamCtx.id().c_str());
	}
	return ERR_RETURN_SUCCESS;
}
							 ...
							 TianShanIce::SRM::ResourceMap &crResource)
{
	int type = 0;
	if( (type = getStreamerType() ) != 0 ) {
		ENVLOG.info(PLFMT(doCommit,"commit streamer, streamerType=[%d]"), type);
		return doUdpCommit( items, crResource );
	}
	if( items.size() <= 0 )
	{
	...
	getResourceDataWithDefault( crResource, rtURI, "uri", URLRULE_C2INIT, reqUri);
	...
	if(shadowIndexNotFound && gEdgeFEConfig.c2StreamerConfig.defaultReaderType == CLIENT_TYPE_DISKAIO )  // expose index == false will not trigger this problem
	{
	...
	else if( gEdgeFEConfig.c2StreamerConfig.defaultReaderType != CLIENT_TYPE_DISKAIO ) {
	...
	C2Streamer::TransferInitRequestParamPtr request = new C2Streamer::TransferInitRequestParam( *mStreamerManager.getC2Env(), reqUri);
	...
	if( iRet != C2Streamer::errorCodeOK )
	{
		...
		case C2Streamer::errorCodeContentNotFound:
			{
				ZQTianShan::_IceThrow<TianShanIce::InvalidParameter>( ENVLOG , "CDNSS" , TianShanIce::errcodeAssetNotfound , PLFMT(doCommit,"failed to load [%s] because[%s]"),
						fileName.c_str() , response->errorText.c_str() );

			}
			break;
		default:
			{
				ZQTianShan::_IceThrow<TianShanIce::ServerError>( ENVLOG , "CDNSS" , iRet , PLFMT(doCommit,"failed to load [%s] because[%s]"),
						fileName.c_str() , response->errorText.c_str() );
			}
			break;
		}
	...
	else
	{
		...
		ENVLOG(ZQ::common::Log::L_INFO,PLFMT(doCommit,"load item[%s] ok and transferId[%s] timeout[%d] availRange[%s] openForWrite[%s] upSessionUrl[%s]"),
...
int32 CdnSsCommand::doUdpDestroy(const std::string& sessId) {
	int64 subSessionId = 0;
	sscanf(sessId.c_str(),"%ld",&subSessionId);
	SessionUdpControlRequestParamPtr request = new SessionUdpControlRequestParam(*mStreamerManager.getC2Env(), URLRULE_UDPSESSION);
	SessionUdpControlResponseParamPtr response = new SessionUdpControlResponseParam();
	request->requestHint = mStreamCtx.id();
	request->method = C2Streamer::METHOD_UDP_RUN;
	request->ctrlCode = SessionUdpControlRequestParam::CONTROL_UNLOAD;
	request->sessionId = mStreamCtx.id();
	request->subSessionId = subSessionId;
	int32 iRet = C2Streamer::cUdpRun( request, response );
	return ERR_RETURN_SUCCESS;
...
int32 CdnSsCommand::doUdpValidateItem( TianShanIce::Streamer::PlaylistItemSetupInfo& info) {
	return ERR_RETURN_SUCCESS;
}
...
int32 CdnSsCommand::doValidateItem(TianShanIce::Streamer::PlaylistItemSetupInfo &info)
{
	...
	//if (pos!= std::string::npos)
	//{
	//	info.contentName = info.contentName.substr(pos+1);
	//}
	int type = 0;
	if( (type = getStreamerType() ) != 0 ) {
		ENVLOG.info(PLFMT(doValidateItem,"validate item, streamerType=[%d]"), type );
		return doUdpValidateItem(info);
	}
	const TianShanIce::Properties&	crProps = mStreamCtx.getContextProperty();
