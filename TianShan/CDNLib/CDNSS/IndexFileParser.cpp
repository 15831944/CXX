...
int32 MemFileReader::read( void* buf, size_t count, size_t size )
{
	...
	szCopy = (size*count) > szLeft ? szLeft : (size*count);
...
int32 CsicoIndexFileParser::getTag( FileReader* reader, IndexRecordHeader& tal ) {
	int32 nsize = -1;
	do {
...
int32 CsicoIndexFileParser::parseIndexRecord( FileReader* reader, IndexRecord& record ) {
	...
	if( record.lastParsedOffset() == 0 ) {
		...
		record.setdatabitrate(indexHeader.getMuxBitrate());
        IDXPARSERLOG(ZQ::common::Log::L_INFO,CLOGFMT(parseIndexRecord,"MainExtension[%s], PlayTime[%d]"), indexHeader.getSubFileName(0).c_str(), indexHeader.getPlayTime() );
	}
	...
	char tempBuffer[0x1B], tmpBufTrick[0x09],tempBufferAVC[0x1A];
	uint32 base = 0, prevTimeOffset = 0, curTimeOffset = 0;
	bool IsFirstFrame  = true;
	bool bfinish = false;
	while(true) {
		IndexRecordHeader recHeader;
		int32 errcode = getTag(reader,recHeader);
		if( errorCodeOK != errcode ) {
			if (bfinish == true) break;
			else
				return errcode;// we may encounter EOF, so return true is OK
		}
		if( recHeader.type != 0x02 && recHeader.type != 0x0E){
			if ( (reader->seek(recHeader.length, FileReader::SEEK_FROM_CUR)) < 0 )
			{
				IDXPARSERLOG(ZQ::common::Log::L_INFO,CLOGFMT(parseIndexRecord,"mSize[%ld], curpos[%ld]"),reader->getMsize(), reader->tell());
				return errorCodeNotFull;
			}
			continue;
		}
		if( recHeader.type == 0x02 && recHeader.length > 0x1B) {
			IDXPARSERLOG(ZQ::common::Log::L_ERROR,
				CLOGFMT(IndexFileParser,"bad index record data, IFrame record should not bigger than  0x1B, file[%s]"),
				reader->getFileName().c_str() );
			return errorCodeExceed;//false;
		}
		else if( recHeader.type == 0x0E && recHeader.length > 0x1A ){
			IDXPARSERLOG(ZQ::common::Log::L_ERROR,
					CLOGFMT(IndexFileParser,"bad AVC index record data, IFrame record should not bigger than  0x1A, file[%s]"),
					reader->getFileName().c_str());
			return errorCodeExceed;//false;
		}
		if( recHeader.type == 0x02 && reader->read(tempBuffer,recHeader.length) != recHeader.length) {
			IDXPARSERLOG(ZQ::common::Log::L_ERROR,
				CLOGFMT(IndexFileParser,"Mpeg:insufficient data, file[%s]"),
				reader->getFileName().c_str() );
			return errorCodeNotFull;//false;
		}
		if( recHeader.type == 0x0E && reader->read(tempBufferAVC,recHeader.length) != recHeader.length) {
			IDXPARSERLOG(ZQ::common::Log::L_ERROR,
				CLOGFMT(IndexFileParser,"AVC:insufficient data, file[%s]"),
				reader->getFileName().c_str() );
			return errorCodeNotFull;//false;
		}
		
		// we only care about normal play file index record
		uint32 rawTimeOffset = 0;
		if (recHeader.type == 0x02 && recHeader.length >= 0x16){
			memset(&tempBuffer[17],0,5);
			rawTimeOffset= (uint32)((*(uint64*)&tempBuffer[11])/27000);
		}
		else if (recHeader.type == 0x0E && recHeader.length >= 0x1A){
			memset(&tempBufferAVC[16],0,5);
			rawTimeOffset= (uint32)((*(uint64*)&tempBufferAVC[10])/27000);
		}
		if(IsFirstFrame && (rawTimeOffset > base))
		{
			IsFirstFrame = false;
			prevTimeOffset = rawTimeOffset;
			curTimeOffset = base;
		}
		else if(rawTimeOffset >= prevTimeOffset)
		{
			curTimeOffset += (rawTimeOffset - prevTimeOffset);
			prevTimeOffset = rawTimeOffset;
		}
		else if(rawTimeOffset < curTimeOffset)
		{
			curTimeOffset += rawTimeOffset;
			prevTimeOffset = rawTimeOffset;
		}
		if (recHeader.type == 0x02)
			record.addRecord(*(uint32*)tempBuffer, curTimeOffset);
		else if (recHeader.type == 0x0E)
			record.addRecord(*(uint32*)tempBufferAVC, curTimeOffset);
		...
		do {
			...
			if( recHeader.type != 0x03 && recHeader.type != 0x0F){
				continue;				
			}
			else if (recHeader.type == 0x03 || recHeader.type == 0x0F)
			{
				...
				speed = indexHeader.getSubFileSpeed(SubFileIdx);
				parseret++;
				record.addRecTrick(PkgOffset,curTimeOffset,speed, SubFileIdx, indexHeader.getSubFileName(SubFileIdx),indexHeader.getAllFilePlayTime(SubFileIdx));
			}
			subRecords ++;
...
void IndexRecord::rewriteRecords(const IndexRecord& rec)
{
	...
	mRecords.resize(rec.getRecords().size());
	memcpy(&mRecords[0],&rec.getRecords()[0],rec.getRecords().size() * sizeof(rec.getRecords()[0]));
	...
	mLastOffset = rec.getLastOffset();
	mPwe = rec.IsPwe();
	mSubFileCount = rec.getSubFileCount();
}
...
uint64 IndexRecord::binarySearch(const RecordInfo* low, const RecordInfo* high, uint64& timeOffset) const
{
    const RecordInfo* low1 = NULL;
	uint32 inittime = timeOffset;
	while( (low+1) < high ) {
		...
		timeOffset = high->timeOffset;
        low1 = high -1;
        if ( fabsSmaller(low1->timeOffset, high->timeOffset, inittime) )       //fabs(inittime - low1->timeOffset) < fabs(inittime - high->timeOffset) ) 
        {
