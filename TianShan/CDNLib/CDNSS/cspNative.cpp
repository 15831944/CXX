...
struct NeMappingMessage
{
	...
	bool fromBuffer( const char* buf, size_t dataSize, const std::string& ignoreSender )
	{
		...
		if( messagegroup != gCdnSSConfig.csStrReplicaGroupId )
			return false;
...
int McastEventReceiver::run()
{
	...
	while(!mbQuit)
	{
		try {
			...
			FileEventMessage msg;
			if ( msg.fromBuffer(szBuf, len, gCdnSSConfig.netId)) // jira SCS-36, netId could be a string name that was not translated to numeric IP
				dispatchEvent( msg, false );
			else
			{
				NeMappingMessage mappingMsg;
				if( mappingMsg.fromBuffer(szBuf, len, gCdnSSConfig.netId ) )
				{
...
void McastEventReceiver::onNeMapping(  )
{
	...
	msg.netid = gCdnSSConfig.netId;
	msg.endpoint = gCdnSSConfig.serviceEndpoint;
	msg.messagegroup   = gCdnSSConfig.csStrReplicaGroupId;
	if( !mMulticaster )
		return;
...
{
	event.messageSender  = gCdnSSConfig.netId;
	event.senderEndpoint = gCdnSSConfig.serviceEndpoint;
	event.messageGroup   = gCdnSSConfig.csStrReplicaGroupId;
...
protected:
	int run()
	{
		int32 interval = gCdnSSConfig.csNeMappingSendInterval;
...
void ContentStoreImpl::initializePortal(ContentStoreImpl& store) {
	...
	if (!ctx->prov->init(store._adapter->getCommunicator(), 
        csPrx, gCdnSSConfig.cpcEndpoint, gCdnSSConfig.cpcRegisterInterval)) {		
 		store._log.flush();
	...
	//set auto sync
	if(gCdnSSConfig.isAutoSync == 1)
		store._autoFileSystemSync = true;

	ctx->prov->setTrickSpeeds(gCdnSSConfig.trickSpeedCollection);
    ctx->sink->startWatch();
}
...
void checkFileSetStatus( const std::string& mainFilePathName, const std::vector<std::string>& fileset, uint64& flag, ContentStoreImpl& store, const ContentImpl::Ptr content, FilesystemSink* sink )
{
	...
	{	
		...
		if( masterReplicaNetId == gCdnSSConfig.netId || masterReplicaNetId.empty() || masterReplicaEndpoint.empty() )
		{
												...
												const ContentImpl::Ptr content, const std::string& fullName,const std::string& mainFilePathname) 
{
    ...
    if(flagsToTest & RSDFLAG(frfAbsence)) 
	{
		...
		if( mainfilesize < gCdnSSConfig.csMinimalMainfileSize )
		{
			ret |= RSDFLAG(frfAbsence);
			store._log(ZQ::common::Log::L_INFO,CLOGFMT(ContentStorePortal,"MainFile[%s] check Residential, mark as frfAbsence due to mainfilesize[%lld] < configsize[%lld]"),
				mainFilePathname.c_str(), mainfilesize, gCdnSSConfig.csMinimalMainfileSize );
		}
    }
...
static bool populateAttrsFromFileInter(Context* pCtx, ContentStoreImpl& store, ContentImpl& content, const std::string& mainFilePathname) {
	...
	{
		setMetaData( content, mainFilePathname, METADATA_MAIFILE_EXTNAME(), subinfo.fileExtension );
	}

	setMetaData( content, mainFilePathname, METADATA_BitRate, idxData.getMuxBitrate() );
	setMetaData( content, mainFilePathname, METADATA_MAIFILENAME(), idxData.getMainFilePathName() );
	...
	setMetaData( content, mainFilePathname, METADATA_INDEXFILENAME(), tmpIndexFileName);
...
//                   TianShanIce::ServerError, TianShanIce::InvalidStateOfArt)
{
	...
	if (!transferBitrate) {
		transferBitrate = gCdnSSConfig.defaultProvisionBW;
	}
	...
	::TianShanIce::Properties sessMdata;
	{
		...
		{
			if (stricmp(gCdnSSConfig.cpcDefaultIndexType.c_str(),"VVC") == 0)
				sessMdata.insert(::TianShanIce::Properties::value_type(CPHPM_INDEXTYPE, "VVC"));
			else if(stricmp(gCdnSSConfig.cpcDefaultIndexType.c_str(),"VVX") == 0)
				sessMdata.insert(::TianShanIce::Properties::value_type(CPHPM_INDEXTYPE, "VVX"));
			else if(stricmp(gCdnSSConfig.cpcDefaultIndexType.c_str(),"VV2") == 0)
...
void FilesystemSink::startWatch() {
	...
	const std::string& localIp = gCdnSSConfig.csEventMulticastLocalIp;
	const std::string& groupIp = gCdnSSConfig.csEventMulticastGroupIp;
	int groupPort = gCdnSSConfig.csEventMulticastGroupPort;

	if( gCdnSSConfig.csEnableEventMulticast >= 1)
	{
