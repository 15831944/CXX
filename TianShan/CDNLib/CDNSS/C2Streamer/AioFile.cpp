...
namespace C2Streamer
{
	...
	bool CacheCenter::start( size_t bufferSize, size_t defaultBufferCount , size_t delta ) {
		...
		else if ( 1 == mEnv.getConfig().clientType)
		{
			...
			requestParams.url                  = mEnv.getConfig().C2ClientURL;
			requestParams.upstreamIP           = mEnv.getConfig().C2ClientUpStreamIP;
			requestParams.clientTransfer       = mEnv.getConfig().C2ClientTransfer;
			requestParams.locateIP             = mEnv.getConfig().C2ClientHttpCRGAddr;
			requestParams.locatePort           = mEnv.getConfig().C2ClientHttpCRGPort;
			requestParams.defaultGetPort       = mEnv.getConfig().C2ClientDefaultGetPort;
			requestParams.transferRate         = mEnv.getConfig().C2ClientTransferRate;
			requestParams.ingressCapacity      = mEnv.getConfig().C2ClientIngressCapacity;
			requestParams.exclusionList        = mEnv.getConfig().C2ClientExclusionList;
			requestParams.transferDelay        = mEnv.getConfig().C2ClientTransferDelay;
			requestParams.waitBufferTime       = mEnv.getConfig().C2ClientWaitBufferTime;
			requestParams.indexTimeout         = mEnv.getConfig().C2ClientIndexTimeout;
			requestParams.indexRetryTimes      = mEnv.getConfig().C2ClientIndexRetryTimes;
			requestParams.mainfileTimeout      = mEnv.getConfig().C2ClientMainfileTimeout;
			requestParams.mainfileRetryTimes   = mEnv.getConfig().C2ClientMainfileRetryTimes;
			requestParams.transferDelete       = mEnv.getConfig().C2ClientEnableTransferDelete == 0 ? false : true;
			requestParams.alignment   		   = mEnv.getConfig().C2ClientAlignment;
			//requestParams.maxBufferCountPerRequest = mEnv.getConfig().C2ClientMaxBufferCountPerRequest;
	...
	else if( 3 == mEnv.getConfig().clientType ) 
	{
		AquaReader::Ptr r = new AquaReader( mEnv, this );
		MLOG(ZQ::common::Log::L_INFO, CLOGFMT(CacheCenter,"start() create AquaReader: rootUrl:[%s] userDomain[%s] homeContainer[%s]"), 
				mEnv.getConfig().aquaReaderRootUrl.c_str(),
				mEnv.getConfig().aquaReaderUserDomain.c_str(),
				mEnv.getConfig().aquaReaderHomeContainer.c_str());
        mReaderPtr = r;
	}
		else
		{
	...
	BufferUser CacheCenter::makeRequest( AioFile* file, uint64 offset, size_t size, bool preRead ) {
		...
		int64        bufferSize = 0;
		{
			...
			for( uint64 i = start ; i < end; i += mBufferSize ) {
				...
				if( it == mCachedData.end() ) {
					...
					//mCachedData[ ri ] = buf;
					std::pair<CACHEDMAP::iterator,bool> ret = mCachedData.insert( CACHEDMAP::value_type(ri, buf) );
					if( !ret.second ) {
						assert( false && "duplicate record found ");
					}
				} else  if( firstBlock ) {
					bufret = it->second;
					useBuffer( bufret );
		...
		if( bufs.size() > 0 ) {
			...
			{
				Buffer* buf = bufs[i];
				MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheCenter, "makeRequest() get file[%s] offset[%lu], buffer size[%d]."),buf->filename().c_str(), buf->offsetInFile(), buf->bufSize());
	...
	void CacheCenter::chainBuffer( Buffer* buf ) {
		...
		mFreeBufferCount ++;
		MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheCenter,"chainBuffer:Id[%zu] free[%zu] total[%zu] cached[%zu] outstanding[%zu] :%p"),
			 buf->id(), mFreeBufferCount, mBufferCount, mCachedData.size(), mOutStandingIoReqCount, buf );
	}
	...
	void CacheCenter::unchainBuffer( Buffer* buf ) { 
		...
		assert(buf->vacant());
		MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheCenter,"unchainBuffer:Id[%zu] free[%zu] total[%zu] cached[%zu] outstanding[%zu]: %p"),
				buf->id(), mFreeBufferCount, mBufferCount, mCachedData.size(), mOutStandingIoReqCount, buf );
	}
	...
	void CacheCenter::unrefBuffer( Buffer* buf) {
		ZQ::common::MutexGuard gd(mLocker);
		// if( (buf->mUsers.dec() <= 0) && ( buf->reqCount == 0) ) {
	...
	//////////////////////////////////////////////////////////
	// Buffer
	Buffer::Buffer( CacheCenter& cc, char* addr, size_t size, size_t id )
	:mCc(cc),
	mId(id),
	mN(0),
	mBuf(addr),
	...
	size_t AioRunner::pushBuffer( const std::vector<Buffer*>& bufs ) {
		boost::mutex::scoped_lock gd(mLocker);
	...
	int AioRunner::run( ) {
		...
		while(mbRunning) {
			...
			if( iocbArray.capacity() < bufs.size() ) {
			...
			if( sw.span()/1000 > mEnv.getConfig().mFsIoCommitThreshold ){
					MLOG(ZQ::common::Log::L_INFO, CLOGFMT(AioRunner,"took [%lu]ms to submit [%d] blocks read request"),
							sw.span()/1000, (int)bufs.size());
			}
		}
