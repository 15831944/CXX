...
BufferUser CacheCenter::findPauseFrame( const ConfWriter* conf) {
	assert( conf != NULL);
	if(conf->mSendPacketSize != 188) {
		MLOG.error(CLOGFMT(CacheCenter, "findPuaseFrame, sendPacketSize is not 188 , reject request"));
		return BufferUser();
	}
	size_t size = conf->mSendPacketSize * conf->mPacketsPerSend;
	if( size <= 0 ) {
		return BufferUser();
	}
	ZQ::common::MutexGuard gd(mLocker);
	...
	for( size_t i = 0 ; i < conf->mPacketsPerSend; i ++ ) {
...
AioFile* CacheCenter::open( const std::string& filename, int readerType ) {
	AioFileAttr attr;
	attr.name = filename;
...
void CacheCenter::makeRequest( AioFile* file, std::vector<BufferUser>& bufs ) {
		...
		return;
	{
		...
		for (int i = 0; i < (int)bufs.size(); i++ )
		{
			...
			buf->mBuffStat = true;
			//MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheCenter, "makeRequest() get file[%s] offset[%lu], reqId[%ld] size[%d]."),
	...
	MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheCenter,"makeRequest() read called with file[%s] buffers[%d] size[%ld] reqId[%ld]"), 
...
BufferUser CacheCenter::makeRequest( AioFile* file, uint64 offset, int readerType, bool withDiskCache) {
	...
	bool followerBlockAvailable = false;
	...
	int64			bitrate = file->getBitrate();
	{
		...
		for( uint64 i = start ; i < end; i += mBufferSize ) {
			...
			if( withDiskCache || it == mCachedData.end() ) {
				...
				MLOG.debug(CLOGFMT(CacheCenter, "makeRequest() set bitrate[%ld] to [%s], reqId[%ld]"),
...
void CacheCenter::unrefBuffer( Buffer* buf) {
	int vOld = buf->mUsers.get();
	int vNew = buf->mUsers.dec();
	//MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheCenter, "unrefBuffer() dec reference count [%d==>%d] of buf[%ld]"),vOld, vNew, buf->reqId());
	//if(buf->mUsers.dec() > 0)
	//	return;
	if(vNew > 0)
		return;
	ZQ::common::MutexGuard gd(mLocker);
	if( (buf->mUsers.get() <= 0) && (!buf->mBuffStat ) ) {
...
void    CacheCenter::onRead( const std::vector<BufferUser>& bufs, bool fromDiskCache )
{
	...
	for (int i = 0; i < (int)bufs.size(); i++ )
	{
		...
		if(cache && mEnv.getConfig().diskCache.diskCacheEnabled && !fromDiskCache && !buf->lastError() && CLIENT_TYPE_DISKAIO != buf->readerType()) {
			int64 timeNow = ZQ::common::now();
			XOR_Media::DiskCache::CacheDir::write_async( buf, timeNow, this);
		}
		MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheCenter, "onRead() get file[%s] data [%lu: %d], reqId[%ld] err[%d/%d]"),
				buf->filename().c_str(), buf->offsetInFile(), (int)buf->getDataSize(), buf->reqId(), buf->errCategory(), buf->errCode());
...
void CacheCenter::onCacheRead(Error errCode, BufferUser buf)
{
	std::vector<BufferUser> bufVec(1,buf);
	{
		...
		if(buf->mbReturnedFromDiskCache) {
			MLOG.debug(CLOGFMT(CacheCenter,"onCacheRead, buf[%s:%ld] id[%ld] already returned" ),
	...
	if(buf.valid()) {
		if( (errCode ==  DiskCacheSink::cacheErr_OK || errCode == DiskCacheSink::cacheErr_Hit) && buf.valid())	{
			MLOG(ZQ::common::Log::L_INFO, CLOGFMT(CacheCenter,"onCacheRead() buf[%s:%lu] id[%ld] gotten"),
					buf.fileName().c_str(), buf.offsetInFile(), buf.reqId() );
			{
				ZQ::common::MutexGuard gd(mLocker);
		...
		MLOG(ZQ::common::Log::L_INFO, CLOGFMT(CacheCenter,"onCacheRead() buf[%s:%lu] id[%ld] missed"),
				buf.fileName().c_str(), buf.offsetInFile(), buf.reqId() );
		//buf->setDataSize(0);
		AioFile *file = open(buf.fileName(), buf->readerType());
...
BufferUser CacheCenter::getCacheBufUser(const std::string& fileName, const int64 stampAsOfOrign, int64 bitrate, long readerType) {
	...
	MLOG.debug(CLOGFMT(CacheCenter, "getCacheBufUser() set bitrate[%ld] to file[%s], reqId[%ld]"), bitrate, fileName.c_str(),buf->reqId());
	buf->request();
	useBuffer(buf);
...
BufferUser AioFile::read( uint64 offset, int64 bitrate ) {
	int64 reminder = offset % mCc.getBufferSize();
...
void Buffer::setDataSize( size_t size ) {
	//mCc.getEnv().getLogger()->debug(CLOGFMT(Buffer,"setDataSize() id[%zu] reqId[%ld] set size to [%zu]"), mId, mReqId, size);
