...
bool SessionDataRunner::prepareRunnerBuffer( BufferUser& bufUser, int64& bytesPrepared )
{
	...
	MLOG.debug( SESSFMT(SessionDataRunner,"prepareRunnerBuffer() read at [%s:%ld/%zu] bufReqId[%ld], cost[%ld]us"),
			mRequestFileName.c_str(), mOffsetInFile, bufUser.tell(), bufUser.bufReqId(), sw.stop() );
...
bool SessionDataRunner::dataProcessing(int32& dataSendCount)
{
	...
	while( mbRunning)
	{
		...
		{
			if( !bufUser.valid() || bufUser.dataLeft() <= 0 || !dataEnoughToRun(bufUser) )
			{
				...
				if( bufUser.asyncWait( mSess->getDataReadCb(), waitingByte) ) {
				...
				MLOG.debug( SESSFMT(SessionDataRunner, "run() buf[%ld] pos[%zu] dataLeft[%zu] waiting[%zu]"),bufUser.reqId(), bufUser.tell(), bufUser.dataLeft(), waitingByte);
				mFsWaitCount = 0;
				if( bytesPrepared == 0 || !dataEnoughToRun(bufUser) )
				{
					bool eagain = false;
					s2.start();
					...
					else
					{
						...
						if( maxIdleCountAtFileEnd > 0 )
						{
							if( mIdleCountAtFileEnd > maxIdleCountAtFileEnd ) {
								...
								mAioFile = mCc.open( reqFileName, mReaderType);
								if(!mAioFile )
								{
			...
			int64 sentBytes = 0;
			s3.start();
			int64 rc = transferData( bufUser, sendingBytes ,sentBytes ) ;
		...
		{//
			...
			MLOG.debug(SESSFMT(C2SessionDataRunner,"reach session end, quiting"));
			return false;
		}
...
resend:
			...
			if( rc <= 0 ) {
				...
				if( rc == LibAsync::ERR_BUFFERTOOBIG || rc == 0 ) {
					...
					MLOG(ZQ::common::Log::L_ERROR, SESSFMT(SessionDataRunner,"transferData() failed to transfer data to peer through connection[%ld], last error[%s/%d]"),
							mConnectionId, mResponseHandler->lastError().c_str(), errno );
				} 
				break;
			} else if( rc > 0 ) {
				pData += rc;
...
bool SessionDataRunner::startRunner( C2ResponseHandler::Ptr response, C2SessionPtr sess )
{
	...
	ZQ::common::MutexGuard gd(mMutex);
	assert( response != 0 );
	mAioFile = mCc.open( mRequestFileName, mReaderType);
	if(!mAioFile)
	{
...
bool SessionDataRunner::prepareUdpRunner( ) {
	...
	mResponseHandler->setCommOption(ZQHttp_OPT_TTL, 3);//set ttl
...
bool SessionDataRunner::postReachFileEnd( AssetAttribute::Ptr attr, bool directCall ) {
	if( attr->lastError() != 0 && !attr->pwe() ) {
		MLOG.info(SESSFMT(SessionDataRunner,"asset for [%s] attribute: lastError[%d]. attr reqId[%ld] take it as end-of-file"),
...
void SessionDataRunner::markAsConnBroken() {
	MLOG.debug( SESSFMT(SessionDataRunner,"onConnBroken"));
	mbConnBroken = true;
	stopRunner();
}
...
void SessionDataRunner::onDataGotten( bool bCheckFileEnd) {
	...
	if( waitTimeDelta >= (mConfWriter->mFsReadThreshold) ) {
		MLOG.debug(SESSFMT(SessionDataRunner,"prepareRunnerBuffer() took  [%ld/%lu]ms to get data from source, reqId[%ld]"),
	...
	if( mbTmpReachFileEnd || lastError != 0 ) {
		MLOG.debug(SESSFMT(SessionDataRunner,"prepareRunnerBuffer() we may reach the file end at offset[%lu], or encounter an error[%d/%d]"),
				mOffsetInFile, lastError>>28, lastError & 0x0FFFFFFF );
		if( bCheckFileEnd ) {
			MLOG.info(CLOGFMT(SessionDataRunner,"reachFileEnd(), reached file end / encounter an error  at [%s:%ld], stop runner"), 
...
void SessionDataRunner::onAsyncTimer() {
	//MLOG.debug( SESSFMT(SessionDataRunner,"onAsyncTimer"));
...
resend:
			...
			if( rc <= 0 ) {
				...
				if( rc == LibAsync::ERR_BUFFERTOOBIG || rc == 0 ) {
					...
					MLOG(ZQ::common::Log::L_ERROR, SESSFMT(PauseFrameDataRunner,"transferData() failed to transfer data to peer through connection[%ld], last error[%s/%d]"),
							mConnectionId, mResponseHandler->lastError().c_str(), errno );
				} 
				break;
			} else if( rc > 0 ) {
	...
	sendSw.stop();
	// MLOG.debug( SESSFMT( PauseFrameDataRunner,"transferData(), loop[%d] data sent[%ld]bytes, cost[%lu]us, lastrc[%d] "), 
	//		sendCount, sentBytes, sendSw.span(), rc );
