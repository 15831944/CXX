...
namespace ZQ
{
	namespace StreamService
	{
        ...
        RequestHandle::RequestHandle(ZQ::common::Log& log, IC2RequestCallBack::Ptr cb, RequestParams params, std::string instanceKey, int64 bufferReqID)
            ...
            _timeout(5000), _startTime(0), _startCalcLatency(0), _bIndex(false), _bPrintConnInfo(true),
        ...
        {
			//_log(ZQ::common::Log::L_DEBUG, CLOGFMT(RequestHandle,"[%s/%s] bufReqID[%ld] timed out when onHttpMessage comes in, timeout[%d]"),_instanceKey.c_str(), _xSessionID.c_str(), _bufferReqID, _timeout);

            if (ZQ::StreamService::Timeout == _status) {
				_log(ZQ::common::Log::L_DEBUG, CLOGFMT(RequestHandle,"[%s/%s] bufReqID[%ld] timed out when onHttpMessage comes in, timeout[%d]"),
					 _instanceKey.c_str(), _xSessionID.c_str(), _bufferReqID, _timeout);
				return false;
			}
            _headerMsg = msg;
        ...
        {
            if (ZQ::StreamService::Timeout == _status) return;
            _log(ZQ::common::Log::L_ERROR, CLOGFMT(RequestHandle, "[%s/%s] bufReqID[%ld] onHttpError() error code[%d], timeout[%d]"), 
					_instanceKey.c_str(), _xSessionID.c_str(), _bufferReqID, error, _timeout);
            cancel();
