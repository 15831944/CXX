...
mbQuit(false)
{
	mUdpInfoQuery = new C2UdpInfoQuery(env,*this);
}
...
void C2Service::stopService( )
{
	...
	if( !mEnv.getConfig().oldHls.mAquaRootUrl.empty() ) {
		mHlsServer.uninit();
	} else {
		mSessManager.stop();
		if( mEnv.getConfig().natEnabled >= 1 ) {
			mC2TunnelMon.stop();
			mIptables.stop();
		}
	}
...
bool C2Service::startService( )
{
	...
	if( !mEnv.getConfig().oldHls.mAquaRootUrl.empty() ) {
		...
		if( mEnv.getConfig().natEnabled >= 1 ) {
			...
			MLOG(ZQ::common::Log::L_INFO, CLOGFMT(C2Service,"nat disabled"));
		}
	}

	mCacheCenter.start(mEnv.getConfig().cacheCenter.mCacheBufferSize, 
			mEnv.getConfig().cacheCenter.mCacheDefaultBufferCount, 
			mEnv.getConfig().cacheCenter.mCacheDeltaBufferCount );
...
int32 C2Service::UdpInfoQuery(const SessionUdpControlRequestParamPtr request, SessionUdpControlResponseParamPtr response)
{
	return mUdpInfoQuery->UdpInfoQuery(request, response);
}
...
:mEnv(env),
mRecords(NULL){
	mRecords = new INDEXRECORDMAP(env.getConfig().cacheCenter.mIndexRecordCacheSize);
}
...
bool C2IndexRecordCenter::IndexRecordWrapper::parse( ) {
	...
	if(!bOK) {
		...
		MLOG(ZQ::common::Log::L_INFO, CLOGFMT(C2IndexRecordCenter,"took [%lu]us to parse [%s]:filesize[%lld] subfiles[%d] records[%d] duration[%u]ms"),
