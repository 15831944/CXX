...
LibAsync::Timer(Socket::getLoop())
{
	...
	_log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "HttpFetcher() httpfetcher[%ld] created using[%d:%d:%d:%d]."), _ID, a, b, c, d);
}
...
HttpFetcher::~HttpFetcher()
{
	  ...
	  _log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "~HttpFetcher() httpfetcher[%ld] reqID[%ld] segsess[%s] destoryed."), _ID, _reqID, _segSess.c_str());
}
...
void HttpFetcher::read(BufferUser buf)
{
	...
	_dataSize = 0;
	_log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "read() entry httpfetcher[%ld] retry[%d] data[%d]."), _ID, _retry, _bufferDataSize);
...
void	HttpFetcher::onReqMsgSent( size_t size)
{
	  ...
	  if(_reqBegin == 0)
	  {
			...
			_log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "onReqMsgSent() httpfetcher[%ld] reqID[%ld] set request begin time[%lld]."), _ID, _reqID, _reqBegin);
	  }
	  std::string locip="";
	  ...
	  _log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "onReqMsgSent() httpfetcher[%ld] reqID[%ld] [%s:%d==>%s:%d]."), _ID, _reqID, locip.c_str(), locport, peerip.c_str(), peerport);
	  ...
	  if( !getResponse() )
	  {
			_log(ZQ::common::Log::L_ERROR, CLOGFMT(HttpFetcher, "onReqMsgSent() httpfetcher[%ld] reqID[%ld] failed to associate response"),_ID, _reqID);
			return;
	  }
	  _log(ZQ::common::Log::L_INFO, CLOGFMT(HttpFetcher, "onReqMsgSent() httpfetcher[%ld] reqID[%ld] associated response"), _ID, _reqID);
}
...
void	HttpFetcher::onHttpDataReceived( size_t size )
{
	  ...
	  if ( 2 != httpStatus / 100 )
	  {
			_log(ZQ::common::Log::L_ERROR, CLOGFMT(HttpFetcher, "onHttpDataReceived() httpfetcher[%ld] reqID[%ld] segsess[%s] get http error[%d]."), _ID, _reqID, _segSess.c_str(), httpStatus);
			if ( _querySwitch )
			{
				  if ( _fileAttr != NULL )
				  {
			...
			else
			{
				  ...
				  _log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "onHttpDataReceived() httpfetcher[%ld] reqID[%ld] segsess[%s] call onRead to return data."), _ID, _reqID, _segSess.c_str() );
				  bool rty = true;
				  if(416 == httpStatus)
					  rty = false;
	  ...
	  if (!_httpComplete)
	  {
		...
		if( !_querySwitch )
		{
			if( !_dataBufer )
			{
				_log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "onHttpDataReceived() httpfetcher[%ld] reqID[%ld] segsess[%s] get the not chunked data bufer is null."),  _ID, _reqID, _segSess.c_str());
				return;
			}
			_log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "onHttpDataReceived() httpfetcher[%ld] reqID[%ld] segsess[%s] got the data[%d==>%d]."), _ID, _reqID, _segSess.c_str(), size, (int)(_bufferDataSize - _dataBufer->getDataSize()) );
		...
		*/
		...
		_log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "onHttpDataReceived() httpfetcher[%ld] reqID[%ld] segsess[%s] recved data[%d] and continue to recv data."), _ID, _reqID, _segSess.c_str(), size);
...
bool	HttpFetcher::onHttpMessage( const LibAsync::HttpMessagePtr msg)
{
	  if( _resBegin == 0 )
	  {
			...
			_log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "onHttpMessage() httpfetcher[%ld] reqID[%ld] set response begin time[%lld]."), _ID, _reqID, _resBegin);
		        if (!_querySwitch)	
	  ...
	  _log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "onHttpMessage() httpfetcher[%ld] reqID[%ld] segsess[%s] got the response chunked[%s] contentlegth[%ld] message[%s]."), _ID, _reqID, _segSess.c_str(), _chunked ? "true" : "false" , msg->contentLength(), msg->toRaw().c_str());
	  _responseHeader = msg;
	  return true;
}
/*
...
bool	HttpFetcher::onHttpBody( const char* data, size_t size)
{
	  ...
	  else
	  {
		  //  checkFileData(data, size);
			if(!_dataBufer)
			{
				_log(ZQ::common::Log::L_ERROR, CLOGFMT(HttpFetcher, "onHtttpBody(), httpfetcher[%ld] reqID[%ld] segsess[%s] the data buff is null."),_ID, _reqID, _segSess.c_str());
				return false;
			}
			if (_dataSize + size > _dataBufer->bufSize())
			{
				  _log(ZQ::common::Log::L_ERROR, CLOGFMT(HttpFetcher, "onHttpBody() httpfetcher[%ld] reqID[%ld] segsess[%s] recv data size [%d] greater the buff size[%d]."), _ID, _reqID, _segSess.c_str(), _dataSize + size, _dataBufer->bufSize());
				  return false;
			}
			_dataSize += size;
			if( _dataSize <= _bufferDataSize)
			{
				 _log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "onHttpBody() httpfetcher[%ld] reqID[%ld] segsess[%s] recv[%d] data[%d] less than data size[%d]."), _ID, _reqID, _segSess.c_str(), size,  _dataSize, _bufferDataSize);
				return true;
			}
...
void	HttpFetcher::onHttpComplete()
{
	  ...
	  if ( _resEnd == 0){
			...
			_log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "onHttpComplete() httpfetcher[%ld] reqID[%ld] segsess[%s] set response end [%lld]"), _ID, _reqID, _segSess.c_str(), _resEnd);
	  }
	  _httpComplete = true;
	  int buffSize = 0;
	//if the http result code  is not 2**, then 
	  ...
	  if ( 2 != httpStatus / 100 )
	  {
			_log(ZQ::common::Log::L_ERROR, CLOGFMT(HttpFetcher, "onHttpComplete() httpfetcher[%ld] reqID[%ld] segsess[%s] get http error[%d]."), _ID, _reqID, _segSess.c_str(), httpStatus);
			if ( _querySwitch )
			{
				  if ( _fileAttr != NULL )
				  {
			...
			else
			{
				  ...
				  _dataBufer->setDataSize(_bufferDataSize);
				  _dataBufer->setLastError(httpStatus, Buffer::ECATE_HTTP);
				  _log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "onHttpComplete() httpfetcher[%ld] reqID[%ld] segsess[%s] call onRead to return data."), _ID, _reqID, _segSess.c_str());
				  if(_bufferDataSize > 0)
				  {
				  	_dataBufer->fileSize( _offset + _bufferDataSize);
				  }
	  ...
	  _log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "onHttpComplete() httpfetcher[%ld] reqID[%ld] segsess[%s] the get the http response[%d]."), _ID, _reqID, _segSess.c_str(), httpStatus);
	  if ( _querySwitch )
	  {
	      bool retry = false;
		  if ( _queryBody.empty() )
		  {
			  _log(ZQ::common::Log::L_WARNING, CLOGFMT(HttpFetcher, "onHttpComplete() httpfetcher[%ld] reqID[%ld] segsess[%s] the query index body is empty."), _ID, _reqID, _segSess.c_str());
			  _readCallBack->onIndexInfo( _fileAttr, true);
			  _fileAttr = NULL;
			  return;
		  }
		  if ( _fileAttr == NULL )
		  {
			  _log(ZQ::common::Log::L_WARNING, CLOGFMT(HttpFetcher, "onHttpComplete() httpfetcher[%ld] reqID[%ld] segsess[%s] the assert attribute is NULL."), _ID, _reqID, _segSess.c_str());
			  return;
		  }
		  if( _replaceC2 )
		  {
				_log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "onHttpComplete() httpfetcher[%ld] reqID[%ld] segsess[%s] try to parse index file."), _ID, _reqID, _segSess.c_str());
				ZQ::IdxParser::IndexData idxData;
				bool result = parseIndex(_fileName, _queryBody.c_str(), _queryBody.size(), idxData);
				if ( result )
				{
					  _log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "onHttpComplete() httpfetcher[%ld] reqID[%ld] segsess[%s] parse index success mainFileSize[%ld]."), _ID, _reqID, _segSess.c_str(), idxData.getMainFileSize());
				...
				else
				{
					  ...
					  retry = true;
					  _log(ZQ::common::Log::L_ERROR, CLOGFMT(HttpFetcher, "onHttpComplete() httpfetcher[%ld] reqID[%ld] segsess[%s] parse index failed."), _ID, _reqID, _segSess.c_str());
					  return;
				}
		  }//if( _replaceC2 )
		  else
		  {	  
			  ...
			  if (n_pos != std::string::npos)
			  {
				  if (_queryBody.at(n_pos+3) == ':')
				  {
					  ...
					  if (e_pos != std::string::npos )
					  {
						  ...
						  _log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "onHttpComplete() httpfetcher[%ld] reqID[%ld] segsess[%s] pwe value[%s]."), _ID, _reqID, _segSess.c_str(), value.c_str());
						  getPwe = true;
			  ...
			  if (n_pos != std::string::npos)
			  {
				  if (_queryBody.at(n_pos+8) == ':')
				  {
					  ...
					  {
						  std::string value = _queryBody.substr(s_pos, e_pos - s_pos);
						  _log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "onHttpComplete() httpfetcher[%ld] reqID[%ld] segsess[%s] baseinfo value[%s]."), _ID, _reqID, _segSess.c_str(), value.c_str());
						  getBaseinfo = true;
	  ...
	  _log(ZQ::common::Log::L_INFO, CLOGFMT(HttpFetcher, "onHttpComplete() httpfetcher[%ld] reqID[%ld] segsess[%s] got content[%d]bytes, took [%d]ms to send, [%d]ms to receive"), _ID, _reqID, _segSess.c_str(), buffSize, reqTranTime, resSendTime);
}
...
void	HttpFetcher::onHttpError( int error )
{
	  ...
	  unsigned short  peerport=0;
	  getPeerAddress(peerip, peerport);
	  _log(ZQ::common::Log::L_ERROR, CLOGFMT(HttpFetcher, "onHttpError() httpfetcher[%ld] reqID[%ld] segsess[%s] get error[%d] sysErr[%d] con[%s:%d==>%s:%d], url[%s]."),
			  _ID, _reqID, _segSess.c_str(), error, errno, locip.c_str(), locport, peerip.c_str(), peerport, _segmenterUrl.c_str() );
	  
	  //_log(ZQ::common::Log::L_ERROR, CLOGFMT(HttpFetcher, "onHttpError() httpfetcher[%ld] reqID[%ld] segsess[%s] get error[%d] sysErr[%d]."), _ID, _reqID, _segSess.c_str(), error, errno);
	  if ( _querySwitch )
	  {
			if (_fileAttr != NULL )
			{
...
void	HttpFetcher::onTimer()
{
	  ...
	  _log(ZQ::common::Log::L_ERROR, CLOGFMT(HttpFetcher, "onTimer()httpfetcher[%ld] reqID[%ld] segsess[%s] entry client[%p]."), _ID, _reqID, _segSess.c_str(), this);
	  if ( _querySwitch )
	  {
			if( _fileAttr != NULL )
			{
...
void HttpFetcher::readAction()
{
	...
	_log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "readAction() httpfetcher[%ld] reqID[%ld] file[%s] client[%p] url[%s]."), 
	...
	{
		cancel();
		_log(ZQ::common::Log::L_ERROR, CLOGFMT(HttpFetcher, "readAction() httpfetcher[%ld] reqID[%ld] send request usr[%s] to[%s] sysErr[%d] failed."), _ID, _reqID, _segmenterUrl.c_str(), _proxyUrl.c_str(), errno);
		if (!_dataBufer)
			return;
		...
		//std::vector<C2Streamer::Buffer*> bufs;
		//bufs.push_back(_dataBufer);
		_log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "readAction() httpfetcher[%ld] reqID[%ld] call onRead to return data."), _ID, _reqID);
		if(_bufferDataSize > 0)
		{
			_dataBufer->fileSize( _offset + _bufferDataSize);
		}
	...
	_log(ZQ::common::Log::L_INFO, CLOGFMT(HttpFetcher, "readAction() httpfetcher[%ld] reqID[%ld] sent to[%s] conn[%s:%d==>%s:%d] request[%s] client[%p]"), _ID, _reqID, _proxyUrl.c_str(), locip.c_str(), locport, peerip.c_str(), peerport, reqMsg->toRaw().c_str(), this);
	return;
}
...
void HttpFetcher::queryIndexAction()
{
	...
	reqMsg->url(_segmenterUrl);
	reqMsg->header("HttpFetcherID",_ID);
	_log(ZQ::common::Log::L_DEBUG, CLOGFMT(HttpFetcher, "queryIndexAction() httpfetcher[%ld] reqID[%ld] fileName[%s] client[%p]."), _ID, _reqID, _fileName.c_str(), this);
	//update(_timeOut);
	update(500);
	if (!beginRequest(reqMsg, _segmenterUrl))
	{
		cancel();
		_log(ZQ::common::Log::L_ERROR, CLOGFMT(HttpFetcher, "queryIndexAction() httpfetcher[%ld] reqID[%ld] send request usr[%s] to[%s] failed."), _ID, _reqID, _segmenterUrl.c_str(), _proxyUrl.c_str());
		if (_fileAttr != NULL)
		{
			_fileAttr->lastError(AssetAttribute::ASSET_SOCKET);
			_readCallBack->onIndexInfo( _fileAttr, true );
	...
	_log(ZQ::common::Log::L_INFO, CLOGFMT(HttpFetcher, "queryIndexAction() httpfetcher[%ld] reqID[%ld] sent to[%s] conn[%s:%d==>%s:%d] arequest[%s] client[%p]."), _ID, _reqID, _proxyUrl.c_str(), locip.c_str(), locport, peerip.c_str(), peerport, reqMsg->toRaw().c_str(), this);
...
bool HttpFetcher::parseIndex(std::string& contentName, const char* indexData, size_t dataSize, ZQ::IdxParser::IndexData& idxData)
{
	  ...
	  {
			_log(ZQ::common::Log::L_ERROR,CLOGFMT(HttpFetcher,"parseIndex() httpfetcher[%ld] reqID[%ld] segsess[%s] failed to parse index data for[%s], data size[%u]"),  _ID, _reqID, _segSess.c_str(), _fileName.c_str(), (uint32)dataSize);
			return false;
	  }
	  return true;
}
