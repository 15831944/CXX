...
namespace C2Streamer {
	...
	void AquaReader::checkPendingRequest( ) {
		int pending = mPool.pendingRequestSize();
		int active = mPool.activeCount();
		static int size = mPool.size();
		if( pending >= size ) {
	...
	int AquaReader::doReadData( const std::string& filename, int64 startOffset, char* buf, int size ) {
		...
		MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(AquaReader,"doReadData() read data us[%ld]us"), ts.stop());
		if( CdmiRet_FAIL(rc) ) {
			return -rc;
		} else {
			return dataSize;
