...
namespace StreamService{
    ...
    bool C2ReadFile::startRequest()
    {
        ...
        std::vector<C2Streamer::BufferUser>::iterator beginIter = _readerBufs.begin();
        if (beginIter == _readerBufs.end())
        {
            _log(ZQ::common::Log::L_ERROR, CLOGFMT(C2ReadFile, "[%s] bufReqID[%ld] startRequest() no buffer"), _instanceKey.c_str(), _bufferReqID);
            return false;
        }
        size_t startOffset = (*beginIter)->offsetInFile() + (*beginIter)->getDataSize();
        size_t endOffset   = _readerBufs[_readerBufs.size()-1]->offsetInFile() + _readerBufs[_readerBufs.size()-1]->bufSize() - 1;
       
        std::ostringstream rangeOss;
        rangeOss << startOffset << "-" << endOffset;
        _params.range = rangeOss.str();

        _log(ZQ::common::Log::L_DEBUG, CLOGFMT(C2ReadFile, "[%s] bufReqID[%ld] startRequest() current retry times[%d/%d] request: bitrate[%s] range[%s]"),
				_instanceKey.c_str(), _bufferReqID, _currRetryCount, getMaxRetry(), _params.transferRate.c_str(),  _params.range.c_str());
        ...
        if (!_tmpBuf.empty())
        {
            ...
            for (; it != _readerBufs.end(); )
            {
                ...
                if (bufLeftSize > _tmpBuf.size())
                {
                    ...
                    std::vector<C2Streamer::BufferUser> bufsVec;
    ...
    bool C2ReadFile::addBuffer(const std::vector<C2Streamer::BufferUser>& bufs)
    {
    ...
    void C2ReadFile::onLocate(const LocateResponseData& resp)
    {
		...
		std::string::size_type posDash = resp.availableRange.find("-");
		int64 fileSize = 0;
		if (posDash != std::string::npos) {
			std::string strSize = resp.availableRange.substr(posDash+1);
			fileSize = strtol(strSize.c_str(), NULL, 10);
			if(fileSize>0) {
				std::vector<C2Streamer::BufferUser>::iterator itBuf = _readerBufs.begin();
				for( ; itBuf != _readerBufs.end(); itBuf++) {
					C2Streamer::BufferUser& bu = *itBuf;
					bu->fileSize(fileSize);
				}
			}
		}
    ...
    void C2ReadFile::onData(const char* data, const size_t& size, bool error)
    {
        ...
        for (; it != _readerBufs.end(); )
        {
            ...
            {
                char *startWritePoint = (*it)->buffer() + (*it)->getDataSize();
                memcpy(startWritePoint, data + allCopySize, leftDataSize);
                _log(ZQ::common::Log::L_DEBUG, CLOGFMT(C2ReadFile, "[%s] bufReqID[%ld]%ld onData() copy data[%ld] to buffer, buffer offset[%ld], dataSize[%d]"),
						_instanceKey.c_str(), _bufferReqID, it->bufReqId(), leftDataSize, (*it)->offsetInFile(), (*it)->getDataSize());

                (*it)->setDataSize((*it)->getDataSize() + leftDataSize);
				allCopySize += leftDataSize;
                leftDataSize = 0;
                break;
            }else{
                ...
                _log(ZQ::common::Log::L_DEBUG, CLOGFMT(C2ReadFile, "[%s] bufReqID[%ld]%ld onData() copy data[%d] to buffer, buffer offset[%ld]"),
						_instanceKey.c_str(), _bufferReqID, it->bufReqId(), copySize, (*it)->offsetInFile());
                ...
                if (currBufLeftSize == 0)
                {
                    ...
                    bufs.push_back(*it);
                    _log(ZQ::common::Log::L_DEBUG, CLOGFMT(C2ReadFile, "[%s] bufReqID[%ld]%ld onData() call onRead(), buffer number[%ld], first buffer offset[%ld], data size[%ld]"),
							_instanceKey.c_str(), _bufferReqID, it->bufReqId(), bufs.size(), (*bufs.begin())->offsetInFile(), (*bufs.begin())->getDataSize());
                    _cb->onRead(bufs);
                    it = _readerBufs.erase(it);
                }
            }
        ...
        if (leftDataSize > 0)
        {
            ...
            _log(ZQ::common::Log::L_INFO, CLOGFMT(C2ReadFile, "[%s] bufReqID[%ld] onData() no read buffer left, copy left data[%ld] to temp buffer"),
					_instanceKey.c_str(), _bufferReqID, leftDataSize);
    ...
    void C2ReadFile::onError(C2RequestErrorCategory category, const int& err, const std::string& msg)
    {
        ...
        if (_currRetryCount < getMaxRetry() && 
            (ZQ::StreamService::crTimeout == category ||
			 ...
			 ZQ::StreamService::crServiceUnavailable == category ))
        {
    ...
    void C2QueryIndex::onRecvComplete()
    {
        //ZQ::common::MutexGuard mg(_mutex);
        // parse index file
        _log(ZQ::common::Log::L_DEBUG, CLOGFMT(C2QueryIndex, "onRecvComplete()[%s] bufReqID[%ld] begin parse index file"), _instanceKey.c_str(), _reqID);
        ...
        if (result)
        {
            _log(ZQ::common::Log::L_DEBUG, CLOGFMT(C2QueryIndex, "onRecvComplete()[%s] bufReqID[%ld] parse index success"), _instanceKey.c_str(), _reqID);

            if ("yes" == _locateData.openForWrite)
            {
				_log(ZQ::common::Log::L_DEBUG, CLOGFMT(C2QueryIndex, "onRecvComplete()[%s] bufReqID[%ld] get file pwe is true."),_instanceKey.c_str(), _reqID);
                _assetAttr->pwe(true);
            }else{
            ...
            _log(ZQ::common::Log::L_DEBUG, CLOGFMT(C2QueryIndex, "onRecvComplete()[%s] bufReqID[%ld] query index successful"), _instanceKey.c_str(), _reqID);
        }else{
            // parse index failed
            _assetAttr->lastError(C2Streamer::AssetAttribute::ASSET_DATAERR);
            _cb->onIndexInfo(_assetAttr);
            _log(ZQ::common::Log::L_DEBUG, CLOGFMT(C2QueryIndex, "onRecvComplete()[%s] bufReqID[%ld] query index failed"), _instanceKey.c_str(), _reqID);
            _requestHandle = NULL;
            return;
        }
    ...
    void C2QueryIndex::onTransferDelete()
    {
        //ZQ::common::MutexGuard mg(_mutex);
        _phase = ZQ::StreamService::phaseDone;
        _log(ZQ::common::Log::L_DEBUG, CLOGFMT(C2QueryIndex, "onRecvComplete()[%s] bufReqID[%ld] transfer delete complete"), _instanceKey.c_str(), _reqID);
        _requestHandle = NULL;
    }
    ...
    C2Streamer::AssetAttribute::LASTERR C2QueryIndex::convertErrorCategoty(C2RequestErrorCategory category)
    {
        ...
        case crWaitBufferTimeout:
            return C2Streamer::AssetAttribute::ASSET_TIMEOUT;
		case crNotFound:
			return C2Streamer::AssetAttribute::ASSET_NOTFOUND;
		case crServiceUnavailable:
			return C2Streamer::AssetAttribute::ASSET_HTTP;
		default:
			assert(false);
