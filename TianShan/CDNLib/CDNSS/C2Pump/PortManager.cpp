...
{
	uint64 capacity = 0;
	const NicsConf& portSpeedMap	= mEnv.getConfig().nics.nics;
	NicsConf::const_iterator itPort = portSpeedMap.find( name );
...
void PortManager::unregisterSession( const std::string& ip , const std::string& sessId, int64 requestBW )
{
	...
	int64 usedCount = 0;
	{
		...
		portAttr = &(itPort->second);
		portAttr->usedBW	-= requestBW;
		portAttr->usedBW = MAX( 0 , portAttr->usedBW );// adjust to >= 0
		portAttr->usedCount --;	

		capacity = portAttr->capacity;
		usedBW = portAttr->usedBW;
		usedCount = portAttr->usedCount;
	}

	MLOG(ZQ::common::Log::L_DEBUG,CLOGFMT(PortManager,"unregisterSession() session[%s] unregistered : capacity[%ld] usedBW[%ld] sessionCount[%ld] ip[%s] portName[%s]"),
		sessId.c_str() , capacity , usedBW , usedCount, ip.c_str() , portName.c_str() );
...
std::vector<std::string> PortManager::getAvailPorts(int64 bw, const std::string& sessId) const
{
	...
	for( ; it != mPorts.end(); it ++)
	{
		...
		{
			MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(PortManager,"getAvailPorts(), do not select [%s] due to:status[%s] capacity[%ld] usedBW[%ld] natBW[%ld] requestBW[%ld]"),
		...
		MLOG.debug(CLOGFMT(PortManager,"getAvailPorts(), select [%s] as candidate, port weight[%d]"),
			info.portName.c_str(), info.weight );
...
bool PortManager::registerSession( const std::string& ip , const std::string& sessId , int64 requestBW )
{
	...
	int64 usedCount = 0;
	{
		...
		natBW = mSvc.getC2TunnelMon().queryBW( ip );

		if( portAttr->capacity < (requestBW + portAttr->usedBW + natBW) )
		{
			MLOG(ZQ::common::Log::L_ERROR, CLOGFMT( PortManager , "registerSession() not enough bandwidth for session[%s] requstBW[%ld] : capacity[%ld] usedBW[%ld] natBW[%ld] Port[%s]"),
				sessId.c_str() , requestBW , portAttr->capacity , portAttr->usedBW, natBW, portName.c_str() );
			return false;
		}
	...
	MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(PortManager,"registerSession() new session [%s] portName[%s] requestBW["FMT64"]: capacity["FMT64"] usedBW["FMT64"] natBW["FMT64"] sessionCount[%ld] ip[%s] "),
		sessId.c_str() , portName.c_str(), requestBW , capacity ,  usedBW , natBW, usedCount, ip.c_str() );
	return true;
}
...
void PortManager::updatePort( const PortAttr& attr )
{
	...
	PortStateUpdateEventPtr e = new PortStateUpdateEvent(mEnv);
...
static unsigned long long get_port_speed(char *name, unsigned long long* phycap)
{
	...
	///FIXME: if there is a configuration which tell us that user has already set the speed of port, USE IT
	{
		...
		const NicsConf& portSpeedMap	= conf.nics.nics;
		NicsConf::const_iterator itPort = portSpeedMap.find(std::string(name));
		if( itPort != portSpeedMap.end())
		{
			PLOG(ZQ::common::Log::L_INFO, CLOGFMT(get_port_speed,"use configuration speed [%ld] for port[%s]"),
	...
	{
	case SPEED_10:    *phycap =		10000000ULL;	break;
	case SPEED_100:   *phycap =		100000000ULL;	break;
	case SPEED_1000:  *phycap =		1000000000ULL;	break;
	case SPEED_2500:  *phycap =		2500000000ULL;	break;
	case SPEED_10000: *phycap =		10000000000ULL;	break;
...
static void addr_callback(struct nl_cache *link_nlc, struct nl_object *objp, int action)
{
	...
	case AF_INET:
		if (len != sizeof(addr.u.ipv4_addr)) 
		{
			PLOG(ZQ::common::Log::L_ERROR, CLOGFMT(addr_callback,"AF_INET addr has bogus len %d, expected %d"), 
												   len, (int)sizeof(addr.u.ipv4_addr) );
			return;
		}
		addr.af = AF_INET;
	...
	case AF_INET6:
		...
		{
			PLOG(ZQ::common::Log::L_ERROR, CLOGFMT(addr_callback,"AF_INET addr has bogus len %d, expected %d"),
													len, (int)sizeof(addr.u.ipv6_addr) );
			return;
		}
		addr.af = AF_INET6;
		isIpv6 	= true;
