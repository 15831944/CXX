...
int HandlerGetFile::process( )
{
	if(!createSession())
	{
		mResponse->updateStartline(errorCodeBadRequest, convertErrCodeToString(errorCodeBadRequest));
		mResponse->flushHeader(0);
		mResponse->complete();
...
void HandlerSessionTransfer::prepareResponseMsg( )
{
	...
	if(isSuccessCode(responsePara->errorCode))
	{
		...
		if( responsePara->cacheable && responsePara->range.bEndValid ) {
			mResponse->flushHeader( responsePara->range.rangeSize() );
		} else {
...
int HandlerC2Locate::process() {
    mLocateCb = new C2LocateCB(mRequest->getLoop(), this);
    ...
    mConn->getLocalEndpoint(mConnInfo.localIP, mConnInfo.localPort); 
	{
		...
		if( pending >= size ) {
			MLOG(ZQ::common::Log::L_INFO, CLOGFMT(HandlerC2Locate,"too many pending request: pending[%d] active[%d] threadSize[%d]"),
...
void HandlerC2Locate::postProcess(int code, const std::string& message)
{
