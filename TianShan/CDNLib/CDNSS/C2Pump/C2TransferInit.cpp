...
int32 C2Session::registerSession( const RequestParamPtr request , RequestResponseParamPtr response )
{
	...
	calcedBitrate *= request->transferRate / 100;
	...
	if( request->exposeIndex || readerType == CLIENT_TYPE_HYBRID ) {
	   ...
	   if(attr)
	   {
			...
			if( attr->lastError() == 0 ) {
				...
				updateLastError(request, response, errorCodeBadRequest, "processTransferInit() failed to query index info for [%s]", request->fileName.c_str());
				return errorCodeBadRequest;
			}
	   }
	   else
