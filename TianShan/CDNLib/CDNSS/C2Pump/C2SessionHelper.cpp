...
C2SessFile::C2SessFile( C2StreamerEnv& env, int readerType )
:mEnv(env),
mFilePosEnd(0),
mFilePosStart(0),
mbValid(false),
...
void C2SessFile::process( const std::string& filepath, const std::string& sessId)
{
	...
	MLOG(ZQ::common::Log::L_INFO,CLOGFMT(C2SessFile,"checkFileAttr() session[%s], parse file %s, time cost[%lu]us regularfile[%s] isdir[%s] posStart[%ld] posEnd[%ld] valid[%s]"),
...
bool C2SessFile::isValid( ) const
{
	///FIXME: 
	  //return true;
	MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(C2SessFile,"isValid() get client type [%d]."), mReaderType);
    if (CLIENT_TYPE_DISKAIO == mReaderType)
	{
		  return ( mbValid && S_ISREG( mFileStat.st_mode) && !S_ISDIR( mFileStat.st_mode) && mFilePosEnd > 0 );
    }
	else if ( CLIENT_TYPE_C2CLIENT == mReaderType ||
			CLIENT_TYPE_HTTPFETCHER == mReaderType ||
			CLIENT_TYPE_AQUAREADER == mReaderType ||
			CLIENT_TYPE_HTPPFETCHER2 == mReaderType ||
			CLIENT_TYPE_HYBRID == mReaderType )
	{
		  //mbValid = true;
		  return true;
	}
	...
	{
		  MLOG(ZQ::common::Log::L_ERROR,CLOGFMT(C2SessFile,"isValid() get invalid client type [%d]."), mReaderType);
		  return false;
	}
}
...
{	
	...
	{
		watch.stop();			
		MLOG(ZQ::common::Log::L_DEBUG,CLOGFMT(C2SessFile,"checkFileAttr() session[%s], it cost [%lu]us to locate the head of file content from file[%s]"),
...
{
	  MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(C2SessFile,"checkFileAttr() get client type [%d]."), mReaderType);
	  if ( CLIENT_TYPE_DISKAIO != mReaderType){
			mFilePosStart = 0;
...
bool C2SessFile::checkRequestRange( const TransferRange& range )
{
	...
	if( mFilePosEnd <= mFilePosStart )
	{
		MLOG(ZQ::common::Log::L_WARNING, CLOGFMT(C2SessFile,"invalid file attribute: data range[%ld-%ld] of file[%s]"),
...
TransferRange composeFileRange( C2SessFile& file , int readerType )
{
	TransferRange range;
	...
	if( readerType == CLIENT_TYPE_DISKAIO ) {
