...
namespace StreamService{
    ...
    bool GetRequest::process()
    {
        ...
        if ("index" == _params.subType && !_params.isPwe)
        {
            setTimeout(_params.indexTimeout);
        }else if("index" != _params.subType && !_params.isPwe){
    ...
    {
		//_log(ZQ::common::Log::L_DEBUG, CLOGFMT(GetRequest, "onHttpBody() entrry with size[%d] status[%d]"),size, _status);
        ...
        if (currBufLeftSize > size)
        {
			...
			bool isIndexFile = _params.subType == "index";
			if (_params.isPwe && isIndexFile && _dataSize > 0)
			{
            ...
            while (leftDataSize > 0)
            {
                ...
                if ( ( _params.isPwe && isIndexFile && _dataSize > 0) ||  currBufLeftSize == 0)
                {
                    // return _instanceKey.c_str(), _xSessionID.c_str() buffer and fill next buffer
                    _cb->onData(_data, _dataSize);
                    //memset(_data, 0, _dataSize);
