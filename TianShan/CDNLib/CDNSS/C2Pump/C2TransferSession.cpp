...
int C2Session::checkFileData( const RequestParamPtr request,bool& cacheAble, RequestResponseParamPtr response ) {
	int readerType = request->getConfUrlRule()->readerType;
    AioFile* f = mSvc.getCacheCenter().open( mRequestFileName, readerType  );
    int64 offset = 0;
    if( mCurrentDataRunner->getTransferRange().bStartValid) {
	...
	return checkFileAttr(request, cacheAble, response, bu);
}

int  C2Session::checkFileAttr(const RequestParamPtr request, bool& cacheAble, RequestResponseParamPtr response, const BufferUser& bu )
{
