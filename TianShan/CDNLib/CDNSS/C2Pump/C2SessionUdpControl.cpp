...
int32 C2Session::parseUdpRequest( const SessionUdpControlRequestParamPtr request, SessionUdpControlResponseParamPtr response , bool bToBeLoad) {
	...
	if(mIndexDataReader == NULL) {
		mIndexDataReader =  getC2StreamerService()->getUdpInfoQuerier()->parse(response->filename, readerType);
	}
    //result = mIndexDataReader->parse(response->filename,readerType);
	if( mIndexDataReader == NULL) {
		return errorCodeBadRequest;
	}
	std::string ext;
	...
	MLOG.debug( REQFMT(parseUdpRequest, "get sub file info: request scale[%f] timeOffset[%ld] inTimeOffset[%ld] outTimeOffset[%ld]  -> response ext[%s] scale[%f] inTimeOffset[%ld] outTimeOffset[%ld] inDataOffset[%ld] outDataOffset[%ld] duration[%ld] dataBitrate[%ld]"),
			...
			response->dataOffset, response->outDataOffset, response->duration, response->bitrate);
...
SessionDataRunner::Ptr C2Session::makeShadowUdpSessionRunner( SessionDataRunner::Ptr from, SessionUdpControlResponseParamPtr response, bool bPauseRunner ) {
	...
	if(!bPauseRunner) {
		...
		runner = new UdpPauseFrameRunner( mEnv, mSvc, this, mSvc.getCacheCenter(), *this, from->getRunnerId());
	}
