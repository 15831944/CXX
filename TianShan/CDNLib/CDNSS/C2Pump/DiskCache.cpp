...
CacheCopy::Error CacheCopy::flush()
{
	...
	//return writev(_exchangeKey.pathName, cch, _contentBody) == (sizeof(cch) + _contentBody->length());
...
int CacheLoader::run()
{
	...
	while (!_dir._bQuit)
	{
		...
		//    b) clean the orphan buffers by attentionCounts
		{
			...
			for (CacheDir::AwaitIOSinkList::iterator it = _dir._awaitIOSinks.begin(); it < _dir._awaitIOSinks.end();)
			{
				...
				if (nextAttention <= 0)
				{
					...
					_dir._log.debug(CLOGFMT(CacheLoader,"dir[%s] buId[%ld] skip due to expiration subPath[%s]."), _dir.path().c_str(), it->reqBufId, it->subPath.c_str() );
				...
				do 
				{
					if (!fullpath.empty())
					{
						_dir._log.debug(CLOGFMT(CacheLoader,"dir[%s] subPath[%s] buId[%ld] skip due to empty fullpath"), _dir.path().c_str(), it->subPath.c_str(), it->reqBufId);
						break;
					}
					...
					{
						_dir._log.debug(CLOGFMT(CacheLoader,"dir[%s] subPath[%s] buId[%ld] skip loading because it is being loaded by th[%d]"), 
					...
					if (_dir._awaitReadBufMap.end() == itBuf)
					{
						_dir._log(ZQ::common::Log::L_WARNING, CLOGFMT(CacheLoader, "dir[%s] file[%s] sink[%p] buId[%ld] has no await buf associated"),
			...
			_dir._log((cOrphanBuf>0) ? ZQ::common::Log::L_WARNING : ZQ::common::Log::L_DEBUG, CLOGFMT(CacheLoader, "dir[%s] found %d expired, await bufs[%d] and sinks[%d], focus-file[%s], cleaned %d orphan bufs: %s"), _dir.path().c_str(), 
		...
		for (CacheDir::NotificationList::iterator it = sinksToExpire.begin(); !_dir._bQuit && it < sinksToExpire.end(); it++)
		{
			...
			_dir._log(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheLoader,"dir[%s] expiring buId[%ld]"), _dir.path().c_str(), it->bu.reqId() );
		...
		{
			_dir._log.debug( CLOGFMT(CacheLoader,"dir[%s] buId[%ld] skip due to empty fullpath"), _dir.path().c_str(), bu.reqId());
			continue;
		}
		...
		_dir._log.debug(CLOGFMT(CacheLoader, "dir[%s] loading file[%s] with buId[%ld]"), _dir.path().c_str(), subpath.c_str(), bu.reqId() );
		...
		if (CacheCopy::eOK != errLoad)
		{
			...
			_dir._log(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheLoader, "dir[%s] file[%s] failed to load, buId[%ld] err(%d)"), _dir.path().c_str(), subpath.c_str(), bu.reqId(), errLoad);
		}
		else
		{
		...
		int64 stampLoaded = ZQ::common::now();
		...
		
		{
			...
			for (CacheDir::AwaitIOSinkList::iterator it = _dir._awaitIOSinks.begin(); it < _dir._awaitIOSinks.end(); )
			{
				...
				char tmp[20];
				snprintf(tmp, sizeof(tmp), "%ld,", n.bu.reqId());
				buIds += tmp;
		...
		_dir._log(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheLoader, "dir[%s] file[%s] of url[%s] loaded[%c], found %d sinks, leaving await bufs[%d] and sinks[%d], buIds: %s"), _dir.path().c_str(), 
			subpath.c_str(), ccp->_originKey.url.c_str(), (CacheCopy::eOK == errLoad)?'T':'F', (int)sinksToNotify.size(), (int)_dir._awaitReadBufMap.size(), (int)_dir._awaitIOSinks.size(), buIds.c_str());
		...
		for (CacheDir::NotificationList::iterator it = sinksToNotify.begin(); !_dir._bQuit && it < sinksToNotify.end(); it++)
		{
			...
			if (ccp->_originKey.url == it->sink.url && ccp->_contentBody)
			{
				...
				_dir._log(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheLoader,"dir[%s] url[%s] buId[%ld] scheduling HIT notification"), 
				_dir.path().c_str(), ccp->_originKey.url.c_str(), it->bu.reqId());
				if (CacheSinkDispatcher::schedule(_dir._thpool, false, it->sink.cb, DiskCacheSink::cacheErr_Hit, tmpccp))
					cNoticeHit++;
			}
			else
			{
				_dir._log(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheLoader,"dir[%s] url[%s] buId[%ld] scheduling MISSED notification"), 
					_dir.path().c_str(), ccp->_originKey.url.c_str(), it->bu.reqId());
		...
		_dir._log(ZQ::common::Log::L_INFO, CLOGFMT(CacheLoader, "dir[%s] file[%s]=>url[%s] loaded[%c], notified (%dH+%dM)/%d sinks, expired %d/%d sinks, took %d/%dmsec ccp ref[%p==>%d]."), _dir.path().c_str(), 
			subpath.c_str(), ccp->_originKey.url.c_str(), (CacheCopy::eOK == errLoad)?'T':'F', cNoticeHit, cNoticeMissed, (int)sinksToNotify.size(), cExp, (int)sinksToExpire.size(), (int) (stampLoaded - stampStart), (int) (ZQ::common::now() - stampStart), ccp._ptr, ccp->__getRef());
...
int CacheFlusher::run()
{
	...
	while (!_dir._bQuit)
	{
#pragma message ( __MSGLOC__ "TODO: CacheFlusher pass access counters to _counterFlow")
...

		{
			...
			{
#pragma message ( __MSGLOC__ "TODO: when CacheFlusher idle, determin how many access counters to pass to _counterFlow")
#ifdef ENABLE_COUNTERFLOW
			...
			if(_dir._cacheCopies.find( subpath) != _dir._cacheCopies.end() )
			{
				...
				_dir._log(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheFlusher,"run() using[%s] get ccp[%p==>%d]"), subpath.c_str(), ccp.get(), ccp->__getRef());
			}
			if (!ccp)
				continue;
		...
		if (CacheCopy::eOK == errFlush)
			_dir._log(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheFlusher, "dir[%s] flushed file[%s]=>url[%s]"), _dir.path().c_str(), subpath.c_str(), ccp->_originKey.url.c_str());
		else
			_dir._log(ZQ::common::Log::L_ERROR, CLOGFMT(CacheFlusher, "dir[%s] failed to flush file[%s]=>url[%s], err(%d)"), _dir.path().c_str(), subpath.c_str(), ccp->_originKey.url.c_str(), errFlush);
		...
		_dir._log(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheFlusher, "dir[%s] file[%s]=>url[%s] found [%d] to-notify, [%d] expired"), 
		...
		_dir._log(ZQ::common::Log::L_INFO, CLOGFMT(CacheFlusher, "dir[%s] file[%s]=>url[%s] flushed, notified %d/%d sinks, expired %d/%d sinks, buf [%ld] took %d/%dmsec ccp ref[%p==>%d]"), 
			_dir.path().c_str(), subpath.c_str(), ccp->_originKey.url.c_str(), cNotified, (int)sinksToNotify.size(), cExpired, (int)sinksToExpire.size(),
			ccp->_contentBody.reqId(), (int) (stampFlushed - stampStart), (int) (ZQ::common::now() - stampStart), ccp._ptr, ccp->__getRef());
		...
		{
			_dir._log(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheFlusher, "start to evict cache[%ldMB/%ldMB]."), freeMB, totalMB);
  ...
  _cdirsTop(SUBDIR_TOP_MIN), _cdirsLeaf(SUBDIR_LEAF_MIN), _flushInterval(FLUSH_INTERVAL_DEFAULT)
{
	...
	{
		try {
			CacheFlusher::Ptr worker = new CacheFlusher(*this);
			worker->start();
...
BufferUser CacheDir::load_async(const std::string& subPath, const std::string& fileName, const int64 offsetInFile, const int64 stampAsOfOrign, int64 bitrate, long opaqueData, DiskCacheSink* cbCompletion)
{
	...
	BufferUser bu;
	_log(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheDir, "dir[%s] read() [%s]=>[%s] cb[%p]"), _homePath.c_str(), url.c_str(), subPath.c_str(), cbCompletion);
	...
	//if (NULL != ccp && != ccp->_contentBody)
	if(NULL != ccp && bu.valid())
	{
#ifdef ENABLE_COUNTERFLOW
		...
		{
			_log(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheDir, "dir[%s] read() hit in memory [%s]=>[%s] cb[%p]"), _homePath.c_str(), url.c_str(), subPath.c_str(), cbCompletion);
		...
		_log(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheDir, "dir[%s] read() swap-missed in memory [%s]=>[%s] cb[%p], cached-copy[%s]mem cached[%ld] ccp ref[%p==>%d]."), _homePath.c_str(), url.c_str(), subPath.c_str(), cbCompletion, ccp->_originKey.url.c_str(), memCacheSize, ccp._ptr, ccp->__getRef());
	...
	_log.debug(CLOGFMT(CacheDir, "dir[%s] read() not in memory [%s]=>[%s] cb[%p], scheduled to load mem cache[%ld]."), _homePath.c_str(), url.c_str(), subPath.c_str(), cbCompletion, memCacheSize);
...
BufferUser CacheDir::scheduleRead(const std::string& subPath, const std::string& fileName, const int64 offsetInFile, const int64 stampAsOfOrign, int64 bitrate, long opaqueData, DiskCacheSink* cbCompletion)
{
...

	{
		...
		if (_awaitReadBufMap.end() == iter )
		{
			...
			_log(ZQ::common::Log::L_DEBUG, CLOGFMT(AwaitBufMap,"dir[%s] await bufmap[%u] bufId[%ld]."), path().c_str(), _awaitReadBufMap.size(), bu.reqId());
		}
		else
		{
			bu = iter->second;
		...
		_log(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheDir, "get buId[%ld] to load, awaitIoSinks size[%u] subPath[%s]"), bu.reqId(), _awaitIOSinks.size(), subPath.c_str() );
		aios.reqBufId = bu.reqId();
		_awaitIOSinks.push_back(aios);
		_eventReadWorkers.signal();
		_log(ZQ::common::Log::L_DEBUG, CLOGFMT(AwaitBufMap,"size[%d] awaitIOSinks size[%u] buId[%ld] subPath[%s]."),(int)_awaitReadBufMap.size(), _awaitIOSinks.size(), bu.reqId(), subPath.c_str());
	}

	_log(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheDir,"start to read file[%s:%ld] buId[%ld] in disk."), fileName.c_str(), bu.offsetInFile(), bu.bufReqId() );
	return bu;
}
...
void CacheDir::update_async(const std::string& subPath, BufferUser buf, const std::string& url, int64 stampAsOfOrigin, DiskCacheSink* cbCompletion)
{
	...
	bool bDirtyCopyAdded =false;
	{
		...
		{
			_log(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheDir, "dir[%s] write() adding cache-copy [%s]=>[%s] cb[%p]"), _homePath.c_str(), url.c_str(), subPath.c_str(), cbCompletion);
			ccp = new CacheCopy();
	    ...
	    _cacheCopies[subPath] = ccp;
		_log(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheDir, "_cacheCopies[%d]"), _cacheCopies.size());
	}
	...
	_log(ZQ::common::Log::L_DEBUG, CLOGFMT(CacheDir, "dir[%s] write() updated [%s]=>[%s] cb[%p]: %s, dirty-added[%c]"), _homePath.c_str(), url.c_str(), subPath.c_str(), cbCompletion, DiskCacheSink::errorToA(err), bDirtyCopyAdded?'T':'F');
...
bool CacheDir::getSpace(uint64& totalMB, uint64& freeMB)
{
	...
	_log.debug(CLOGFMT(CacheDir, "get diisk size[%ldMB/%ldMB]."), freeMB, totalMB);
	return true;
}
