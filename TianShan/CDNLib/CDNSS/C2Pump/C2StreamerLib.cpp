...
const char* UdpSessionStateToString( UdpSessionState state) {
	switch(state) {
	case UDPSTATE_NONE: return "None";
	case UDPSTATE_INIT:	return "Init";
	case UDPSTATE_PLAY:	return "Play";
	case UDPSTATE_PAUSE: return "Pause";
	case UDPSTATE_DONE:	return "Done";
...
{
	return (errorCode >= 200 && errorCode <=299) || errorCode == errorWorkingInProcess;
}


bool RequestResponseParam::response( ) {
	if( httpHandler == NULL )
		return true;
	return httpHandler->postResponse();
}
...
void RequestResponseParam::setLastErr( const RequestParamPtr& request , int32 errCode , const char* fmt , ... )
{
	...
	va_list args;
	va_start(args, fmt);
	int n = vsnprintf( szLocalBuffer, sizeof(szLocalBuffer)-2, fmt, args);
	va_end(args);
	if( n > 0 ) {
		szLocalBuffer[n] = '\0';
	}
	errorText = szLocalBuffer;
	getEnvironment()->getLogger()->error( SESSFMT(Response,"[%s] %s"), convertC2MethodToString(request->method) , szLocalBuffer );
}
...
int64 TransferRange::rangeSize() const {	
	if(!bEndValid || endPos < 0 )
		return -1;
	int64 start = startPos;
	if( !bStartValid ) {
		start = 0;
	}
...
C2HttpResponseHanlder::C2HttpResponseHanlder( ZQHttp::IResponse* handler , ZQHttp::IConnection* conn, LibAsync::EventLoop* loop)
...
int C2HttpResponseHanlder::addBodyContent( const char* data , size_t size )
{	
	if( !mbComplete )
	{	
		return mResponseHandler->addBody( data, size );
	}
	return -1;
}

bool C2HttpResponseHanlder::registerWrite( ZQHttp::IChannelWritable::Ptr cb ) {
	return mResponseHandler->registerWrite(cb);
}
...
int32	cUdpRun( const SessionUdpControlRequestParamPtr request, SessionUdpControlResponseParamPtr response ) {
	...
	if(!sess)
	{
		response->setLastErr(request , errorCodeContentNotFound , "can't find session [%s], [%ld]", sessId.c_str(), request->subSessionId );
...
int32   cUdpInfoQuery( const SessionUdpControlRequestParamPtr request, SessionUdpControlResponseParamPtr response ){
	if ( request->subSessionId == 0)
	{
		...
		return svc->UdpInfoQuery( request, response );
	}
	return errorCodeContentNotFound;
}
