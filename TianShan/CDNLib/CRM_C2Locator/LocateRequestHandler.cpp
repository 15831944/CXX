...
void LocateRequestHandler::onRequest(const CRG::IRequest* req, CRG::IResponse* resp)
{
	...
	{  
		...
		if (_env._conf.assetStack.paids.end() != itorPID && volumeList.size() > 1)
		{
			...
			_log(ZQ::common::Log::L_INFO, ReqLOGFMT(LocateRequestHandler, "Pid[%s]Paid[%s] VolumeIndex[%d], adjusted volume lists [%s]"), reqData.providerId.c_str(),reqData.assetId.c_str(),nVolumeIndex, strVolumes.c_str());
			bAssetStack = true;
		}
	}
	/////////////////////////////////////////
