...
TransferSessions C2LocatorImpl::listSessionsByClient(const std::string& client, const Ice::Current& c)
{
	...
	while (evicIter && evicIter->hasNext())
	{
		...
		{
			envExtLog(::ZQ::common::Log::L_WARNING,CLOGFMT(C2LocatorImpl, "listSessionsByClient() with [client: %s] catch UnmarshalOutOfBoundsException when processing session: [%s]"), client.c_str(), ident.name.c_str());
		}
		catch(::Ice::Exception &ex)
		{
			envExtLog(::ZQ::common::Log::L_WARNING,CLOGFMT(C2LocatorImpl, "listSessionsByClient() with [client: %s] catch [%s] when processing session: [%s]"), client.c_str(), ex.ice_name().c_str(), ident.name.c_str());
		}
		catch(...)
		{
			envExtLog(::ZQ::common::Log::L_WARNING,CLOGFMT(C2LocatorImpl, "listSessionsByClient() with [client: %s] catch unknown exception when processing session: [%s]"), client.c_str(), ident.name.c_str());
		}
	}
	return ret;
}
...
TransferSessions C2LocatorImpl::listSessionsByPort(const std::string& port, const Ice::Current& c)
{
	...
	while (evicIter && evicIter->hasNext())
	{
		...
		{
			envExtLog(::ZQ::common::Log::L_WARNING,CLOGFMT(C2LocatorImpl, "listSessionsByPort() with [port: %s] catch UnmarshalOutOfBoundsException when processing session: [%s]"), port.c_str(), ident.name.c_str());
		}
		catch(::Ice::Exception &ex)
		{
			envExtLog(::ZQ::common::Log::L_WARNING,CLOGFMT(C2LocatorImpl, "listSessionsByPort() with [port: %s] catch [%s] when processing session: [%s]"), port.c_str(), ex.ice_name().c_str(), ident.name.c_str());
		}
		catch(...)
		{
			envExtLog(::ZQ::common::Log::L_WARNING,CLOGFMT(C2LocatorImpl, "listSessionsByPort() with [port: %s] catch unknown exception when processing session: [%s]"), port.c_str(), ident.name.c_str());
		}
	}
	return ret;
}
...
bool C2LocatorImpl::commit(const std::string& reqId, ::TianShanIce::SCS::TransferSessionPrx transferSession, int& err)
{
	...
	try
	{
		...
		envExtLog(ZQ::common::Log::L_INFO,CLOGFMT(TESTXXX,"%s"),ZQTianShan::Util::dumpStringMap(props).c_str());
	...
	catch( const Ice::SocketException& ex)
	{
		...
		envExtLog(::ZQ::common::Log::L_ERROR,CLOGFMT(C2LocatorImpl, "[%s] catch [%s] when commit TransferSession(%s)"), reqId.c_str(), ex.ice_name().c_str(), strTransferId.c_str());
	}
    ...
    {
        err = e.errorCode;
        envExtLog(ZQ::common::Log::L_ERROR, CLOGFMT(C2LocatorImpl, "[%s] TianShan Exception: %s during commit TransferSession(%s). category=%s code=%d message=%s."), reqId.c_str(), e.ice_name().c_str(), sessIdent.name.c_str(), e.category.c_str(), e.errorCode, e.message.c_str());
    }
    catch (::Ice::Exception& ex)
    {
        envExtLog(::ZQ::common::Log::L_ERROR,CLOGFMT(C2LocatorImpl, "[%s] catch [%s] when commit TransferSession(%s)"), reqId.c_str(), ex.ice_name().c_str(), strTransferId.c_str());
    ...
    catch (...)
    {
        ...
        envExtLog(::ZQ::common::Log::L_ERROR,CLOGFMT(C2LocatorImpl, "[%s] catch unknown Exception  when commit TransferSession(%s)"), reqId.c_str(), strTransferId.c_str());
    }
    return false;
}
...
void C2LocatorImpl::destroy(::Ice::Identity &sessIdent)
{
	...
	try
	{
		...
		if (!transferSessionPrx)
		{
			envExtLog(::ZQ::common::Log::L_WARNING, CLOGFMT(C2LocatorImpl, "destroy() transfer session(%s) already gone."), sessIdent.name.c_str());
...
void C2LocatorImpl::InitEventSink()
{
	...
	{
		envExtLog(ZQ::common::Log::L_ERROR, CLOGFMT(C2LocatorImpl, "register to eventchannel catch exception(%s)"),ex.ice_name().c_str());
	}
	catch(const Ice::Exception& ex)
	{
		envExtLog(ZQ::common::Log::L_ERROR, CLOGFMT(C2LocatorImpl, "register to eventchannel catch exception(%s)"),ex.ice_name().c_str());
...
void C2LocatorImpl::OnRestore(const ::Ice::Current& c)
{
	...
	while (evicIter && evicIter->hasNext())
	{
		...
		{
			envExtLog(::ZQ::common::Log::L_ERROR,CLOGFMT(C2LocatorImpl, "catch UnmarshalOutOfBoundsException when restore TransferSession(%s) from db"), strTransferId.c_str());
			removeTransferSession(ident);
		}
		catch(::Ice::Exception &ex)
		{
			envExtLog(::ZQ::common::Log::L_ERROR,CLOGFMT(C2LocatorImpl, "catch [%s] when restore TransferSession(%s) from db"), ex.ice_name().c_str(), strTransferId.c_str());
			removeTransferSession(ident);
		}
		catch(...)
		{
			envExtLog(::ZQ::common::Log::L_ERROR,CLOGFMT(C2LocatorImpl, "restore TransferSession(%s) from db catch unknown error"), strTransferId.c_str());
			removeTransferSession(ident);
		}
	}
...
void C2LocatorImpl::updateReplica_async(const ::TianShanIce::AMD_ReplicaSubscriber_updateReplicaPtr& amd, const ::TianShanIce::Replicas& replicas, const ::Ice::Current& c)
{
	...
	for(it = replicas.begin(); it != replicas.end(); ++it)
	{
		...
		{
			// warning here
			envExtLog(ZQ::common::Log::L_WARNING, CLOGFMT(C2LocatorImpl, "updateReplica() required property %s missed of port %s"), STREAMERPROP_CAPACITY, port.name.c_str());
			continue;
		}
		...
		if(!hostName.empty()) {
			if(!checkRemoteHealth(hostName)) {
				envExtLog(ZQ::common::Log::L_INFO, CLOGFMT(C2LocatorImpl, "updateReplica() The host %s is not healthy. Set the port %s down"), hostName.c_str(), port.name.c_str());
				port.isUp = false;
			}
		} else {
	...
	for(size_t i = 0; i < availablePorts.size(); ++i) {
		...
		buf << " port(" << pInfo.name << "):"
			<< " ip(" << ZQ::common::Text::join(pInfo.addressListIPv4) << ";" << ZQ::common::Text::join(pInfo.addressListIPv6)<< ")"
			<< " status(" << pInfo.activeBandwidth << "(" << pInfo.activeTransferCount << ")/" << pInfo.capacity << ");";
	}
	envExtLog(ZQ::common::Log::L_INFO, CLOGFMT(C2LocatorImpl, "updateReplica() %d ports are available. %s"), availablePorts.size(), buf.str().c_str());
	// monitor the capacity changing
	...
	if(bwDelta != 0) {
		...
		envExtLog(ZQ::common::Log::L_INFO, CLOGFMT(C2LocatorImpl, "updateReplica() total bandwidth report: current(%lld)"), _env.portPerf.totalBw);
	}
...
int C2LocatorImpl::run()
{
	...
	while (evicIter && evicIter->hasNext())
	{
		...
		{
			envExtLog(::ZQ::common::Log::L_ERROR,CLOGFMT(C2LocatorImpl, "run() catch [%s] when check the state of transfer session(%s)"), ex.ice_name().c_str(), ident.name.c_str());
		}
		catch(...)
		{
...
void StreamEventHandler::OnEndOfStream(const std::string &proxy, const std::string &id, const TianShanIce::Properties &, const ::Ice::Current&) const
{
	...
	if(locator_->querySessionByStream(id, ident))
	{
		...
		{
			envExtLog(::ZQ::common::Log::L_ERROR,CLOGFMT(C2LocatorImpl, "OnEndOfStream() catch [%s] when process the EndOfStream event. session(%s), stream(%s)"), ex.ice_name().c_str(), ident.name.c_str(), id.c_str());
		}
		catch(...)
		{
