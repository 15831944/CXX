...
public:
    ...
    virtual void OnTimer(const Ice::Current& = Ice::Current())
    {
        ...
        { // with ports
            _env._watch.watch(_ident, _renewTimeout - longestRenewTime);
        }
        else
        { // no port
...
public:
    ...
    virtual void OnTimer(const Ice::Current& = Ice::Current())
    {
        ...
        _env._watch.watch(_ident, _tpMgr.nReducingIntervalMsec_);
    }
...
bool TransferPortManager::restore()
{
    ...
    while (evicIter && evicIter->hasNext())
    {
        ...
        {
            _log(::ZQ::common::Log::L_WARNING,CLOGFMT(TransferPortManager, "catch UnmarshalOutOfBoundsException when restore TransferSession(%s) from db"), ident.name.c_str());
            _env._eC2TransferSession->remove(ident);
        }
        catch(::Ice::Exception &ex)
        {
            _log(::ZQ::common::Log::L_WARNING,CLOGFMT(TransferPortManager, "catch Ice Exception when restore TransferSession(%s) from db"), ident.name.c_str());
            _env._eC2TransferSession->remove(ident);
        }
        catch(...)
        {
            _log(::ZQ::common::Log::L_WARNING,CLOGFMT(TransferPortManager, "restore TransferSession(%s) from db catch unknown error"), ident.name.c_str());
            _env._eC2TransferSession->remove(ident);
        }
    }
...
Ice::Long TransferPortManager::removeUnavailablePorts(Ice::Long timeout)
{
    ...
    while(it != _tpMap.end())
    {
        ...
        if(renewTime > timeout)
        { // need remove
            if(it->second.activeTransferCount <= 0)
            {
                _log(ZQ::common::Log::L_INFO, CLOGFMT(TransferPortManager, "Remove port(%s) info due to not renew in %lldmsec"), it->first.c_str(), renewTime);
                _tpMap.erase(it++);
                continue;
            }
            else // can't remove at this point
            {
                _log(ZQ::common::Log::L_INFO, CLOGFMT(TransferPortManager, "Port(%s) not renew in %lldmsec, but can't be removed due to %lld session still running on it."), it->first.c_str(), renewTime, it->second.activeTransferCount);
                ++nRemovingFailed;
            }
        }
        else // no need remove
    ...
    if(bwDelta == 0) {
        _log(::ZQ::common::Log::L_INFO, CLOGFMT(TransferPortManager, "removeUnavailablePorts() total bandwidth report: current(%lld)"), _env.portPerf.totalBw);
    } else if (bwDelta < 0) {
        _log(::ZQ::common::Log::L_INFO, CLOGFMT(TransferPortManager, "removeUnavailablePorts() total bandwidth decreased: current(%lld) delta(%lld)"), _env.portPerf.totalBw, bwDelta);
    } else {
        _log(::ZQ::common::Log::L_WARNING, CLOGFMT(TransferPortManager, "removeUnavailablePorts() total bandwidth increased: current(%lld) delta(%lld)"), _env.portPerf.totalBw, bwDelta);
    }
