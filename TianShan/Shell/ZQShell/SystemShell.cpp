...
bool loadConfig() {
    ...
    */
    ...
    res = node->getAttributeValue("argument", value);
    if(res) {
        serviceConfig.argument = value;
    }
    
    res = node->getAttributeValue("instanceId", value );
    if( res ){
        serviceConfig.svcInstanceId = value;
    }
    res = node->getAttributeValue("restartInterval", value);
    if(res) {
        std::istringstream is(value);
        is >> serviceConfig.restartInterval;
    }

    res = node->getAttributeValue("restartTimeout", value);
    if(res) {
        std::istringstream is(value);
        is >> serviceConfig.restartTimeout;
    }

    res = node->getAttributeValue("restartTries", value);
    if(res) {
        std::istringstream is(value);
    ...
    res = node->getAttributeValue("aliveTimeout", value);
    if(res) {
        std::istringstream is(value);
        is >> serviceConfig.aliveTimeout;
    }

    res = node->getAttributeValue("imagePath", value);
    if(res) {
    ...
    res = node->getAttributeValue("backupFiles", value);
    if(res) {
...
pid_t startProcess() {
    ...
    if(pid < 0) {
        std::ostringstream os;
        os << "failed to create process ("
            << serviceConfig.imagePath << "): ("
            << strerror(errno) << ")";
        ERR(os.str().c_str());
...
void initConfig() {
    logConfig.loggingMask = false;
    logConfig.snmpLoggingMask = false;
...
void printConfig() {
    std::cerr << "Log:"
        << "\nenabled: "
        << std::boolalpha << logConfig.loggingMask
        << "\npath: "
        << logConfig.path
        << "\nname: "
        << logConfig.name
        ...
        << std::boolalpha << logConfig.snmpLoggingMask
        << "\n\n"
        << "Service:"
        << "\nargument: "
        << serviceConfig.argument
        << "\naliveTimeout: "
        << serviceConfig.aliveTimeout
        << "\nrestartTimeout: "
        << serviceConfig.restartTimeout
        << "\nrestartMaxInterval: "
        << serviceConfig.restartInterval
        << "\nrestartTries: "
        << serviceConfig.restartTries
        << "\nappAliveWait: "
        << serviceConfig.appAliveWait
        << "\nimagePath: "
        << serviceConfig.imagePath
        << "\nstartupWait: "
        << serviceConfig.startupWait
        << "\n" << std::endl;
}
...
static bool _CopyFile(string _pFrom, string _pTo,short int flags)
{
    ...
    {  
        ...
        dir_t=opendir(tdirect);
        LOG(Log::L_DEBUG, "create %s success",tdirect);
    }
    else
    {
        LOG(Log::L_DEBUG, "Execute the command : chmod and chown for the Directory :%s ",tdirect);
    ...
    while((sp=readdir(dir_s))!=NULL)
    {
        if(strcmp(sp->d_name,".")!=0&&strcmp(sp->d_name,"..")!=0)
        {
            ...
            if(S_ISLNK(temp_sbuf.st_mode))
            {
                LOG(Log::L_DEBUG, "ln file not copy....");
                continue ;
            }      
            else if((S_IFMT&temp_sbuf.st_mode)==S_IFREG)
            {
                LOG(Log::L_DEBUG, "start copying file %s to %s ",temp_spath,temp_tpath);
                Copy_FileWithMMap(temp_spath,temp_tpath);
                LOG(Log::L_INFO, " copy %s  ended ",temp_spath);
                strcpy(temp_tpath,tpath);
                strcpy(temp_spath,spath);
            }
            else if((S_IFMT&temp_sbuf.st_mode)==S_IFDIR)
            {
                LOG(Log::L_DEBUG, "start copying directory %s to %s ",temp_spath,temp_tpath);
...
static bool  _CopyFileExCMD(string _pFrom, string _pTo,short int flags)
{
    ...
    LOG(Log::L_DEBUG, "    start copying %s to %s ...",_pFrom.c_str(),_pTo.c_str());
    system(exec_buf) ;
    LOG(Log::L_INFO, "     copying %s done", _pFrom.c_str());
    return true ;   
}
static void DatafileToLogdir(void* pvoid)
{
    ...
    {
        LOG(Log::L_DEBUG, "skip backing up DB per serviceConfig.backfiles not configured");
        return ;
    }
    ...
    {   
        tp_signpath=*itr;
        LOG(Log::L_INFO, "Beging copying %s in function DatafileToLogdir().....", tp_signpath.c_str());
...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    if(shell_pid < 0) {
        fprintf(stderr, "failed to create process (%s): [%d] [%s]\n", 
            serviceConfig.imagePath.c_str(), errno, strerror(errno));
        return (errno);
    }
    else if(shell_pid > 0) {
        _exit(0);   
    ...
    */
    ...
    if(setsid() == (-1)) {
        fprintf(stderr, "failed to set session id [%s]", strerror(errno));
        return (errno);
    }
    /* change to root */
    if(chdir("/") < 0) {
        fprintf(stderr, "can't change working directory to '/'");
        return (errno);
    }
    ...
    */
    if(!initSignals()) {
        syslog(LOG_ERR, "failed to init signals: (%s)", strerror(errno));
        return (-1);
    }
    ...
    */
    ...
    try{
        ...
        syslog(LOG_ERR, "%s", "unknown error while loading config");    
        cleanup();
        return (-1);
    }
    ...
    */
    if(logConfig.loggingMask)
    {
        try {
            ...
            syslog(LOG_ERR, "failed to initialize log (%s): (%s)", 
                logConfig.name.c_str(), ex.getString());
            cleanup();
            return (-1);
        }
    }
    ...
    mmib->addObject(new ZQ::SNMP::SNMPObject("loggingMask",         &logConfig.loggingMask,         ZQ::SNMP::AsnType_Int32),   ".1");
    mmib->addObject(new ZQ::SNMP::SNMPObject("snmpLoggingMask",     &logConfig.snmpLoggingMask,     ZQ::SNMP::AsnType_Int32),   ".2");
    mmib->addObject(new ZQ::SNMP::SNMPObject("logDir",              &logConfig.path,                ZQ::SNMP::AsnType_String),  ".3");
    mmib->addObject(new ZQ::SNMP::SNMPObject("logSize",             &logConfig.size,                ZQ::SNMP::AsnType_Int32),   ".4");
    mmib->addObject(new ZQ::SNMP::SNMPObject("logLevel",            &logConfig.level,               ZQ::SNMP::AsnType_Int32),   ".5");

    mmib->addObject(new ZQ::SNMP::SNMPObject("imagePath",           &serviceConfig.imagePath,       ZQ::SNMP::AsnType_String),  ".6");
    mmib->addObject(new ZQ::SNMP::SNMPObject("aliveTimeout",        &serviceConfig.aliveTimeout,    ZQ::SNMP::AsnType_Int32),   ".7");
    mmib->addObject(new ZQ::SNMP::SNMPObject("restartTries",        &serviceConfig.restartTries,    ZQ::SNMP::AsnType_Int32),   ".8");
    mmib->addObject(new ZQ::SNMP::SNMPObject("restartInterval",     &serviceConfig.restartInterval, ZQ::SNMP::AsnType_Int32),   ".9");
    mmib->addObject(new ZQ::SNMP::SNMPObject("startupWait",         &serviceConfig.startupWait,     ZQ::SNMP::AsnType_Int32),   ".10");
    mmib->addObject(new ZQ::SNMP::SNMPObject("backupFiles",         &serviceConfig.backfiles,       ZQ::SNMP::AsnType_String),  ".11");

    mmib->addObject(new ZQ::SNMP::SNMPObject("startTimeout",        &serviceConfig.restartTimeout,  ZQ::SNMP::AsnType_Int32),   ".12");
    ...
    */
    ...
    while (!stop) 
    {
        ...
        if(logConfig.loggingMask) 
        {
            LOG(Log::L_INFO, "service[%s], creating process[%s] at %d-th try, interval[%d]sec", argv[1], serviceConfig.imagePath.c_str(), restartCount, sleepPriorRestart);
        }
        ...
        if (res == svc_pid)
        { 
            snprintf(buf, sizeof(buf)-2, "service[%s] child[%s] started but soon quit", argv[1], serviceConfig.imagePath.c_str());
            ERR(buf);
            continue;
        }
        ...
        while(!stop) //loop of receiving heart beat
        {
            ...
            {
                // timeout receiving any hearbeat
                if (errno == ETIMEDOUT)
                {
                    ...
                    if(logConfig.loggingMask) 
                    {
                        LOG(Log::L_INFO, "service[%s] shell[%d] wait timeout, stopping service", argv[1], shell_pid);
                    }
