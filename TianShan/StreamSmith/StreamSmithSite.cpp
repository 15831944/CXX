...
//	vecPluginPath.push_back("lalala");
	...
	{		
		...
		if(!share->isLoaded())
		{
			glog(Log::L_ERROR,SSSITE("SetupStreamSmithSite()  can't load the plugin %s and Last error code=%d"),
								vecPluginPathAndInfo[i].filePath.c_str(), SYS::getLastErr());
			// continue;
			return false;
		}
...
// 				{
// 					const char* pDisableAutoDelete = _pReq->getContext(CLIENT_REQUEST_DISABLE_AUTO_DELETE);
// 					if( pDisableAutoDelete && stricmp( pDisableAutoDelete , "1" ) == 0 )
// 					{
// 						//do nothing
// 					}
// 					else
