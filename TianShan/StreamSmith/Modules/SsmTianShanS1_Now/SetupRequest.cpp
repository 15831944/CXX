	...
	bool HandleSetup::prepareResponse()
	{
		...
		else // non-QAM mode
		{
			...
			if (_bC2)
			{
                ...
                {
                    snprintf(_szBuf, sizeof(_szBuf)-1, "couldn't find (sessionInterface) from resources, weiwoo session(%s)", _cltSessCtx.srvrSessID.c_str());
                    SSMLOG(ErrorLevel, HandlerFmt(HandleSetup, "%s"), _szBuf);
				...
				//_pResponse->setHeader(HeaderContentType, "text/xml");
				std::string source = sessionInterfaceVar.strs[0];
