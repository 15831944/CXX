...
#define SC_ANNOUNCE_STATECHANGED			"8802"
#define SC_ANNOUNCE_STATECHANGED_STRING		"\"State Changed\""
...
#define GETPARA_MINCACHESIZE	2000
	...
	public:
		...
		{
			ZQ::common::MutexGuard gd(_eventReceiveTimeLocker);
			_lastEventReceivedTime = t;
		}
		int64 getLastEventRecvTime() const
		{
			ZQ::common::MutexGuard gd(_eventReceiveTimeLocker);
			return _lastEventReceivedTime;
		}
		...
		bool getSessStatus(const std::string sessionId, SessStatus& ssOut);
	...
	private:
		...
		bool regesterSnmp(IStreamSmithSite* pSite);
		void unregisterSnmp();

        uint32 snmp_getLogLevel_Main() { return _fileLog.getVerbosity(); }
        void   snmp_setLogLevel_Main(const uint32& newLevel) { _fileLog.setLevel(newLevel); }
        uint32 snmp_getLogLevel_Ice() { return _iceFileLog.getVerbosity(); }
        void   snmp_setLogLevel_Ice(const uint32& newLevel) { _iceFileLog.setLevel(newLevel); }
	...
	public: 
		...
		int32											_iceOverrideTimeout;
		ZQ::common::NativeThreadPool					_eventSinkPool;
		int64											_lastEventReceivedTime;
		ZQ::common::Mutex								_eventReceiveTimeLocker;
		StreamEventSinker*								_streamEventSinker;
	...
	{
	public:
		StreamEventSinker( Environment& env , ZQADAPTER_DECLTYPE objAdapter);
