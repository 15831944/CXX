...
#define MODULE_NAME "ssm_tianshan_s1"

ZQ::common::Log* s1log = NULL;
ZQ::common::Config::Loader<TianShanS1::TSConfig> _tsConfig("");
...
bool Environment::doInit(const char* cfgPath, IStreamSmithSite* pSite)
{
	...
	try
	{
		_fileLog.open(_logPath.c_str(), 7);
	}
...
//		_sysLog(ErrorLevel, "create log caught %s", ex.getString());
	...
	_tsConfig.setLogger(s1log);
	
	if (false == loadConfig())
		return false;

	_fileLog.setFileSize(_tsConfig._pluginLog._size);
	_fileLog.setFileCount(_tsConfig._pluginLog._maxCount);
	_fileLog.setLevel(_tsConfig._pluginLog._level);
	_fileLog.setBufferSize(_tsConfig._pluginLog._bufferSize);
	...
	{
		SSMLOG(WarningLevel, CLOGFMT(Environment, "Register SessionView caught unexpect exception"));
	}

//	_tsConfig.snmpRegister("ssm_NGOD2");
	

	{
		//publish log and syntax file
		TSConfig::LogPublishs::const_iterator it  = _tsConfig._logpublish.begin();
		for( ; it != _tsConfig._logpublish.end() ; it ++ )
		{
	...
	_cacheInterval = _tsConfig._rtspSession._timeout /3 *1000;
	if (_cacheInterval < GETPARA_MININTERVAL)
		_cacheInterval = GETPARA_MININTERVAL;
	if (_cacheInterval > _tsConfig._rtspSession._timeout * 800)
		_cacheInterval = _tsConfig._rtspSession._timeout * 800;

	int32	size = _tsConfig._rtspSession._pingCacheSize;
...
{
    _mmib.addObject(new ZQ::SNMP::SNMPObjectByAPI<Environment, uint32>("rtspLogS1",   *this, ZQ::SNMP::AsnType_Int32, &TianShanS1::Environment::snmp_getLogLevel_Main, &TianShanS1::Environment::snmp_setLogLevel_Main));
    _mmib.addObject(new ZQ::SNMP::SNMPObjectByAPI<Environment, uint32>("rtspLogIcetrace",   *this, ZQ::SNMP::AsnType_Int32, &TianShanS1::Environment::snmp_getLogLevel_Ice, &TianShanS1::Environment::snmp_setLogLevel_Ice));
...
RequestProcessResult Environment::doFixupRequest(IStreamSmithSite* pSite, IClientRequestWriter* pReq)
{
	...
	if (_tsConfig._response._maxResponseTimeout >0)
	{
		int64 stampStart = atoll(pReq->getContext("SYS.RequestTime"));
		int64 stampNow = ZQ::common::now();
		if (stampStart >0 && stampStart + _tsConfig._response._maxResponseTimeout < stampNow)
		{
			SSMLOG(ErrorLevel, CLOGFMT(Environment, "Req(%p:FixupRequest) expired after queue, created[%lld +%d] now[%lld]"),
				pReq, stampStart, _tsConfig._response._maxResponseTimeout, stampNow);
	...
	try 
	{
		bool ret = pFixupRequest->process();
		if (false == ret)
		{
			...
			return RequestProcessed; // fixup request has been processed, need not process in the same phase, be ready to processed in content handler
	}
	catch (...)
	{
		pFixupRequest->_bProcessSuccessfully = false;
		SSMLOG(ErrorLevel, CLOGFMT(Environment, "Req(%s:FixupRequest) caught unexpect exception")
...
// 				pContentHandler = NULL;
	...
	catch (...)
	{
		...
		SSMLOG(ErrorLevel, CLOGFMT(Environment, "session(%s), phase(%s), process(%s) caught unexpect exception")
			, pContentHandler->getSession().c_str(), pContentHandler->getPhase().c_str(), pContentHandler->getMethod().c_str());
		pResponse->printf_preheader(ResponseUnexpectServerError);
		pResponse->post();
...
RequestProcessResult Environment::doFixupResponse(IStreamSmithSite* pSite, IClientRequest* pReq)
{
	...
	case RTSP_MTHD_SETUP:				
		pFixupResponse = new SetupResponse(*this, pSite, pReq, pResponse);
		{
			...
			try 
			{
				bool ret = pFixupResponse->process();
			...
			catch (...)
			{
				...
				SSMLOG(ErrorLevel, CLOGFMT(Environment, "Req(%s:FixupResponse) caught unexpect exception")
...
{
    if(logDir) {
        _logPath = _logDir = logDir;
    }
	_logPath += FNSEPS MODULE_NAME ".log";
}

bool Environment::loadConfig()
{
	SSMLOG(ZQ::common::Log::L_NOTICE, CLOGFMT(Environment, "**************** (Load %s) ****************"), MODULE_NAME);
	if(!_tsConfig.load(_cfgDir.c_str()))
    {
        SSMLOG(ZQ::common::Log::L_INFO, "load config %s failed", _cfgDir.c_str());
        return false;
    }

	if (_tsConfig._rtspSession._timeout < 60)
		_tsConfig._rtspSession._timeout = 60;

	if (_tsConfig._rtspSession._monitorThreads < 3)
		_tsConfig._rtspSession._monitorThreads = 3;

	if (_tsConfig._response._maxFieldLen < 50)
		_tsConfig._response._maxFieldLen = 50;

	if (_tsConfig._response._maxResponseTimeout >0 && _tsConfig._response._maxResponseTimeout < 1000)
		_tsConfig._response._maxResponseTimeout = 1000;

	//check if there is Ice Override timeout
	std::vector<IceProperty::IcePropertyHolder>& iceProps = _tsConfig._iceProps._propDatas;
	...
	std::vector<float>& forwardSpeeds = _tsConfig._fixedSpeedSet.forwardSpeeds;
	std::string strForwardSpeed = _tsConfig._fixedSpeedSet.strForwardSpeeds;
	...
	std::vector<float>& backwardSpeeds = _tsConfig._fixedSpeedSet.backwardSpeeds;
	std::string strBackwardSpeed = _tsConfig._fixedSpeedSet.strBackwardSpeeds;	
	...
	SSMLOG(ZQ::common::Log::L_INFO," [%s] FixedSpeedSet",_tsConfig._fixedSpeedSet.enable ? "enable":"disable");
...
bool Environment::initIceRuntime()
{
	...
	try
	{
		...
		for (it = _tsConfig._iceProps._propDatas.begin(); it != _tsConfig._iceProps._propDatas.end(); it ++)
		...
		if( _tsConfig._iceTrace._enabled >= 1 )
		{
			try
			{
				_iceFileLog.open(iceLogDir.c_str(),
								_tsConfig._iceTrace._level,	
								_tsConfig._iceTrace._maxCount,
								_tsConfig._iceTrace._size);
	...
	try
	{
		_pAdapter = ZQADAPTER_CREATE(_pCommunicator, "LocalAdapter", _tsConfig._bind._endpoint.c_str(), SSMLOG);
		_pAdapter->activate();
	}
	catch(Ice::Exception& ex) 
	{
		SSMLOG(ErrorLevel, CLOGFMT(Environment, "create adapter(%s) caught(%s)"), _tsConfig._bind._endpoint.c_str(), ex.ice_name().c_str());
		return false;
	}
...
bool Environment::openSafeStore()
{
    ...
    //bug#20384
    if (_tsConfig._database._runtimePath.empty())
    {
        _tsConfig._database._runtimePath = _tsConfig._database._path;
    }

	_tsConfig._database._runtimePath += FNSEPS MODULE_NAME;
	std::string pathStr(_tsConfig._database._runtimePath);
	...
	try
	{
		_pStreamIdx = new TianShanS1::StreamIdx(INDEXFILENAME(StreamIdx));
		::std::vector<Freeze::IndexPtr> indexs;
		indexs.push_back(_pStreamIdx);

        updateDbEnvConfig(pathStr,"DbPrivate","0");
        updateDbEnvConfig(pathStr ,"DbRecoverFatal",_tsConfig._database._fatalRecover );
        updateDbEnvConfig(pathStr,"CheckpointPeriod",_tsConfig._database._checkpointPeriod);
        updateDbFileConfig(pathStr,"SessionContexts","SaveSizeTrigger",_tsConfig._database._saveSizeTrigger);
        updateDbFileConfig(pathStr,"SessionContexts","SavePeriod",_tsConfig._database._savePeriod);

		ZQ::common::MutexGuard lk(_lockEvictor);

#if ICE_INT_VERSION / 100 >= 303
		_pContextEvictor = ::Freeze::createBackgroundSaveEvictor(_pAdapter, _tsConfig._database._runtimePath.c_str(), "SessionContexts", 0, indexs);
#else
		_pContextEvictor = ::Freeze::createEvictor(_pAdapter, _tsConfig._database._runtimePath.c_str(), "SessionContexts", 0, indexs);
#endif
		_pContextEvictor->setSize(_tsConfig._rtspSession._cacheSize);
	...
	while (tItor->hasNext())
	{
		int64 rn_value = (int64) _tsConfig._rtspSession._timeout * 1000 + 60000 + (20 * nSessionNum++);
		...
		bool bExist = true;
		{
			try
			{
				TianShanIce::Streamer::StreamPrx prx = 	TianShanIce::Streamer::StreamPrx::checkedCast(
															_pCommunicator->stringToProxy(cltSessCtx.streamPrxID) );
			}
...
bool Environment::connectIceStorm()
{
	...
	try
	{
		...
		return _streamEventSinker->start(_tsConfig._iceStorm._endpoint);
	}
	catch (TianShanIce::BaseException& ex)
	{
		SSMLOG(ErrorLevel, CLOGFMT(Environment, "connectIceStorm(%s) caught(%s: %s)")
			, _tsConfig._iceStorm._endpoint.c_str(), ex.ice_name().c_str(), ex.message.c_str());
		return false;
	}
	catch (Ice::Exception& ex)
	{
		SSMLOG(ErrorLevel, CLOGFMT(Environment, "connectIceStorm(%s) caught(%s)")
			, _tsConfig._iceStorm._endpoint.c_str(), ex.ice_name().c_str());
			return false;
	}	
	return true;
}
...
bool Environment::getPlaylistPrx(TianShanIce::Streamer::PlaylistPrx& playlistPrx, const std::string& streamPrxID)
{
	...
	{
		SSMLOG(ErrorLevel, CLOGFMT(Environment, "getPlaylistPrx(%s) caught(%s)")
			, streamPrxID.c_str(), ex.ice_name().c_str());
		return false;
	}
	return true;
}
...
{
	int64 interval = _tsConfig._rtspSession._announceResubscribeAtIdle;
...
{
	if (0 == _tsConfig._announce._notifyEOS)
		return;
	...
	try
	{
		...
		else // TianShan spec
		{
			...
			{
                noticeParam = noticeParam + " primaryItemNPT=" + it->second;
            ...
            {
                std::string userNotice;
                ZQTianShan::Util::getPropertyDataWithDefault( props , "user.NGOD-Notice" , "" , userNotice );
                SSMLOG(DebugLevel, CLOGFMT(StreamEvent, "OnEndOfStream(%s:%s), get value[user.NGOD-Notice=%s] from event property"), proxy.c_str(), cltSessCtx.ident.name.c_str(), userNotice.c_str());

                std::string clientSessID;
                ZQTianShan::Util::getPropertyDataWithDefault( cltSessCtx.props , HeaderClientSessionId , "" , clientSessID );

                if (!clientSessID.empty())
                {
                    snprintf(szBuf, sizeof(szBuf) - 1, "%s", clientSessID.c_str());
                    pServerRequest->printHeader(HeaderClientSessionId, szBuf);
                }

                if (!userNotice.empty())
                {
                    snprintf(szBuf, sizeof(szBuf) - 1, "%s", userNotice.c_str());
                } 
                else
                {
                    std::string pos_str;
                ...
                std::string noticeStr(szBuf);
                noticeStr.append(" ");
                noticeStr.append(noticeParam);
                pServerRequest->printHeader(HeaderNotice, (char*)noticeStr.c_str());
...
{
	if (0 == _tsConfig._announce._notifyBOS)
		return;
	...
	try 
	{
		...
		else // TianShan spec
		{
			...
			if (0 == stricmp(cltSessCtx.rangePrefix.c_str(), "clock"))
			{
				...
				if(!tmp.empty())
				{
                    snprintf(noticeParamBuf, sizeof(noticeParamBuf) - 1, "BcastPos=%s primaryItemNPT=%s"
                        , bcastPos.c_str(), tmp.c_str());

					snprintf(szBuf, sizeof(szBuf) - 1, "BcastPos=%s;primaryItemNPT=%s"
						, bcastPos.c_str(), tmp.c_str());
				}
				else
				{
                    snprintf(noticeParamBuf, sizeof(noticeParamBuf) - 1, "BcastPos=%s"
                        , bcastPos.c_str());
			...
			else 
			{
				...
				if(!tmp.empty())
				{
                    snprintf(noticeParamBuf, sizeof(noticeParamBuf) - 1, "npt=%d.%03d primaryItemNPT=%s"
                        , curPos / 1000, curPos % 1000, tmp.c_str());

					snprintf(szBuf, sizeof(szBuf) - 1, "npt=%d.%03d;primaryItemNPT=%s"
						, curPos / 1000, curPos % 1000, tmp.c_str());
				}
				else
				{
                    snprintf(noticeParamBuf, sizeof(noticeParamBuf) - 1, "npt=%d.%03d"
            ...
            if(SPEC_NGOD_S1 == cltSessCtx.requestType) // NGOD spec
            {
                std::string userNotice;
                ZQTianShan::Util::getPropertyDataWithDefault( props , "user.NGOD-Notice" , "" , userNotice );
                SSMLOG(DebugLevel, CLOGFMT(StreamEvent, "OnBeginningOfStream(%s:%s), get value[user.NGOD-Notice=%s] from event property"), proxy.c_str(), cltSessCtx.ident.name.c_str(), userNotice.c_str());
                ...
                else
                {
                    ...
                    SYS::TimeStamp ts;
                    snprintf(szBuf, sizeof(szBuf) - 1, SC_ANNOUNCE_BEGINOFSTREAM " " \
                        SC_ANNOUNCE_BEGINOFSTREAM_STRING " " "event-date=%04d%02d%02dT%02d%02d%02d.%03dZ " "npt=%s"\
                        , ts.year, ts.month, ts.day, ts.hour, ts.minute, ts.second, ts.millisecond, pos_str.c_str());
                }
                std::string tianshanNoticeParam(noticeParamBuf);
                std::string noticeStr(szBuf);
                noticeStr.append(" ");
                noticeStr.append(tianshanNoticeParam);
                pServerRequest->printHeader(HeaderNotice, (char*)noticeStr.c_str());
...
{
	if (0 == _tsConfig._announce._notifyScaleChanged)
		return;
	...
	try 
	{
		...
		else // TianShan spec
		{
			...
			if (0 == stricmp(cltSessCtx.rangePrefix.c_str(), "clock"))
			{
				...
				if(!tmp.empty())
				{
                    snprintf(noticeParamBuf, sizeof(noticeParamBuf) - 1, "BcastPos=%s Scale=%f primaryItemNPT=%s"
				...
				{
                    snprintf(noticeParamBuf, sizeof(noticeParamBuf) - 1, "BcastPos=%s Scale=%f"
			...
			else 
			{
				...
				if(!tmp.empty())
				{
                    snprintf(noticeParamBuf, sizeof(noticeParamBuf) - 1, "npt=%d.%03d Scale=%f primaryItemNPT=%s"
				...
				{
                    snprintf(noticeParamBuf, sizeof(noticeParamBuf) - 1, "npt=%d.%03d Scale=%f"
			...
			if (SPEC_NGOD_S1 == cltSessCtx.requestType)// NGOD spec
			{
				std::string userNotice;
				ZQTianShan::Util::getPropertyDataWithDefault( props , "user.NGOD-Notice" , "" , userNotice );
				SSMLOG(DebugLevel, CLOGFMT(StreamEvent, "OnSpeedChanged(%s:%s), get value[user.NGOD-Notice=%s] from event property"), proxy.c_str(), cltSessCtx.ident.name.c_str(), userNotice.c_str());
...
{
	if (0 == _tsConfig._announce._notifyStateChanged)
		return;
	...
	try 
	{
		...
		if (0 == stricmp(cltSessCtx.rangePrefix.c_str(), "clock"))
		{
			...
			if(!tmp.empty())
			{
                snprintf(noticeParamBuf, sizeof(noticeParamBuf) - 1, "BcastPos=%s presentation_state=%s primaryItemNPT=%s"
			...
			{
                snprintf(noticeParamBuf, sizeof(noticeParamBuf) - 1, "BcastPos=%s presentation_state=%s"
		...
		else 
		{
			...
			if(!tmp.empty())
			{
                snprintf(noticeParamBuf, sizeof(noticeParamBuf) - 1, "npt=%d.%03d presentation_state=%s primaryItemNPT=%s"
			...
			{
                snprintf(noticeParamBuf, sizeof(noticeParamBuf) - 1, "npt=%d.%03d presentation_state=%s"
        ...
        if (cltSessCtx.requestType == SPEC_NGOD_S1) // NGOD_S1 spec
        {
            std::string clientSessID;
            ZQTianShan::Util::getPropertyDataWithDefault( cltSessCtx.props , HeaderClientSessionId , "" , clientSessID);
            ...
            {
                SYS::TimeStamp ts;
                snprintf(szBuf, sizeof(szBuf) - 1, SC_ANNOUNCE_STATECHANGED " " \
                    SC_ANNOUNCE_STATECHANGED_STRING " eventdate=" "%04d%02d%02dT%02d%02d%02d.%03dZ" \
                    , ts.year, ts.month, ts.day, ts.hour, ts.minute, ts.second, ts.millisecond);
            }
            std::string tianshanNoticeParam(noticeParamBuf);
            std::string noticeStr(szBuf);
...
{
	if (0 == _tsConfig._announce._notifyItemSkipped)
		return;
	...
	try 
	{
		...
		else // TianShan spec
		{
			...
			if (0 == stricmp(cltSessCtx.rangePrefix.c_str(), "clock"))
			{
				...
				{
                    snprintf(noticeParamBuf, sizeof(noticeParamBuf) - 1, "BcastPos=%s prev_item=%s current_item=%s primaryItemNPT=%s"
				...
				{
                    snprintf(noticeParamBuf, sizeof(noticeParamBuf) - 1, "BcastPos=%s prev_item=%s current_item=%s"
			...
			else 
			{
				...
				if (tempMap.find("sys.primaryItemNPT") != tempMap.end())
				{
                    snprintf(noticeParamBuf, sizeof(noticeParamBuf) - 1, "npt=%d.%03d prev_item=%s current_item=%s primaryItemNPT=%s"
				...
				{
                    snprintf(noticeParamBuf, sizeof(noticeParamBuf) - 1, "npt=%d.%03d prev_item=%s current_item=%s"
			...
			if (cltSessCtx.requestType == SPEC_NGOD_S1) // NGOD_S1 spec
			{
                ...
                else
                {
                    SYS::TimeStamp ts;
                    snprintf(szBuf, sizeof(szBuf) - 1, SC_ANNOUNCE_ITEMSTEPPED " " \
                        SC_ANNOUNCE_ITEMSTEPPED_STRING " eventdate=" "%04d%02d%02dT%02d%02d%02d.%03dZ" \
                        , ts.year, ts.month, ts.day, ts.hour, ts.minute, ts.second, ts.millisecond);
                }
                std::string tianShanNoticeParam(noticeParamBuf);
                std::string noticeStr(szBuf);
...
{
	if (0 == _tsConfig._announce._notifyExit)
		return;
