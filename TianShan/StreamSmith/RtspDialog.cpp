...
void ParseProcThrd::close()
{
	...
	_buflock.enter();
	RTSPBUFFER buf;
	glog(ZQ::common::Log::L_INFO, "ParseProcThrd: big_rtsp_buf_allocated[%d], rtsp_buf_allocated[%d]",
	...
	glog(ZQ::common::Log::L_INFO, "ParseProcThrd uninitialized");				
}
...
{	
	...
	do
	{
		try 
		{
			msg = RtspMsgParser::preParse( parseReq.pReq.buf,
				parseReq.pReq.dataLen,
				bytesDecoded);			
		}
#ifdef ZQ_OS_MSWIN
		catch (...) 
		{
			glog(Log::L_ERROR, "ThreadID[%04x] %s RTSP preParse() occurred a exception", GetCurrentThreadId(), conn->getLogHint());
			break;
		}
			
		if( msg->getCompletionFlag() != RTSP_MSG_COMPLETE )
		{
			glog(Log::L_ERROR, "ThreadID[%04x] %s not a complete package", GetCurrentThreadId(), conn->getLogHint());			
			break;
		}
		...
		if (req == NULL) 
		{				
			glog(Log::L_ERROR, "ThreadID[%04x] %s invalid rtsp message", GetCurrentThreadId(), conn->getLogHint());						
			break;
		}
		...
		{
			glog(Log::L_ERROR, "ThreadID[%04x] %s RTSP parse() occurred a exception", GetCurrentThreadId(), conn->getLogHint());					
			break;
		}
#else
		catch (...) 
		{
			glog(Log::L_ERROR, "ThreadID[%08x] %s RTSP preParse() occurred a exception", pthread_self(), conn->getLogHint());
			break;
		}
			
		if( msg->getCompletionFlag() != RTSP_MSG_COMPLETE )
		{
			glog(Log::L_ERROR, "ThreadID[%08x] %s not a complete package", pthread_self(), conn->getLogHint());			
			break;
		}
		...
		if (req == NULL) 
		{				
			glog(Log::L_ERROR, "ThreadID[%08x] %s invalid rtsp message", pthread_self(), conn->getLogHint());						
			break;
		}
		...
		{
			glog(Log::L_ERROR, "ThreadID[%08x] %s RTSP parse() occurred a exception", pthread_self(), conn->getLogHint());					
			break;
		}
...
{
#ifdef ZQ_OS_MSWIN
	glog(Log::L_INFO, "ThreadID[%d] RTSP parser thread entered", GetCurrentThreadId());	
#else
	glog(Log::L_INFO, "ThreadID[%08x] RTSP parser thread entered", pthread_self());	
#endif
...
#ifdef ZQ_OS_MSWIN
	glog(Log::L_INFO, "ThreadID[%d] RTSP parser thread left", GetCurrentThreadId());	
#else
	glog(Log::L_INFO, "ThreadID[%08x] RTSP parser thread left", pthread_self());	
#endif
	return 0;
}
...
// 			//do something
// 			RtspMsg* msg;
// 			RtspConnection*	conn = parseReq.pConn;
// 			int bytesDecoded=0;
// 
// 			try 
// 			{
...
// 			{
// 				msg = 0;
// 				glog(Log::L_ERROR, "%s RTSP preParse() occurred a exception", conn->getLogHint());			
// 				// what to do?
// 				break;
// 			}
// 
...
// 			{
// 				// what to do?
// 				glog(Log::L_ERROR, "%s [%d]:[%s] not a complete package", conn->getLogHint(), parseReq.pReq.dataLen,parseReq.pReq.buf);			
// 				freeRtspBuf(parseReq.pReq);
// 				delete msg;
// 
// 				break;
...
//             {
//                 // Here msg is still in base class, 
// 				// need to create derived class
//                 // and assign msg to the derived class
//                 RtspMsg* newMsg = RtspMsgParser::createRtspMsg(   (msg->getStartLine()).getDataBuf(), 
...
// 					"RTSP/1.0 400 Bad request" CRLF CRLF;
// 				conn->send((const uint8* )errMsg, strlen(errMsg));
// 				glog(ZQ::common::Log::L_ERROR, "%s OnRequest() invalid rtsp message: Seq(%s)", conn->getLogHint(), msg->getCSeq().getData());				
// 				delete msg;
// 			}
// 			else
// 			{
...
{	  
	...
	sprintf(_szLogHint, "SOCKET: "FMT64" %s:%s", communicator->getCommunicatorId(), _strPeerIP.c_str(), _strPeerPort.c_str());

	glog(Log::L_INFO, "RtspConnection OnConnected():\t SOCKET() connect from client [%s:%s] to server [%s:%s] and id[%llu]",
...
RtspConnection::~RtspConnection()
{
	...
	glog(Log::L_INFO, "RtspConnection destruction():\t client [%s:%s] to server [%s:%s] and id[%llu]",
		_strPeerIP.c_str(), _strPeerPort.c_str(),
		_strLocalIP.c_str(),_strLocalPort.c_str(),
		commId );
}
...
bool RtspConnection::close()
{
	...
	DEBUG_DETAIL(DEBUG_DETAIL_LEVEL3) {
		glog(Log::L_DEBUG, "RtspConnection::close():\t "
				"this(%llu), _lastSessionId = %s", _mainConn->getCommunicatorId(), 
...
#define PRINT_REQ_COUNT(PRE)	\
	glog(Log::L_DEBUG, PRE "REQ_COUNT = %d, this = %p", req_count, this)
	  ...
	  _rtspReq(req), _conn(conn), _response(*this, dwRecvTime),_ref(0),_requestInitTime(dwRecvTime)
{
	DEBUG_DETAIL(DEBUG_DETAIL_LEVEL3) {
		REQ_COUNT_INC();
		PRINT_REQ_COUNT("RtspClientRequest construct ");
		glog(Log::L_DEBUG, "RtspClientRequest::RtspClientRequest()\t"
			"this(%p), _conn = %p", this, conn);
	}
...
RtspClientRequest::~RtspClientRequest()
{
	...
	glog(Log::L_DEBUG, "RtspClientRequest::~RtspClientRequest():\t"
		"this = %p, release _conn = %llu", this, _conn->getConnectionIdentity());
...
//	SOCKET socket = (SOCKET )conn->getSocket();
	...
	if (conn->isActive()) 
	{
		...
		glog(ZQ::common::Log::L_DEBUG,"RtspServerResponse::post(): send out data with COMM[%lld] ",conn->getConnectionIdentity());
		if (conn->send((uint8* )res.c_str(), len) <= 0)
		{
			glog(Log::L_INFO, "RtspServerResponse::post():  failed to send data through COMM[%lld]",conn->getConnectionIdentity());			
		}
		else
			glog(ZQ::common::Log::L_DEBUG,"RtspServerResponse::post(): send out data with COMM[%lld] and time consumption [%lld]ms",conn->getConnectionIdentity(), ZQ::common::now()-timeTemp);
	} 
	else 
	{
		glog(Log::L_DEBUG, "RtspServerResponse::post():  COMM[%lld] is closed", conn->getConnectionIdentity() );		
	}
	...
	if (conn->getRemoteIP(ipaddr, sizeof(ipaddr), &port) == NULL)
	{
		glog(Log::L_ERROR, "RtspServerResponse::post()\t"
			"getRemoteIP() failed");
		return 0;
	}
...

    {
        ...
        record.processedTimeMSec	= timeCost;
        record.durationMSec			= timeCostTotal;
	...
	glog(ZQ::common::Log::L_INFO,"Request processed: session[%s] seq[%s] verb[%s] duration=[%lld/%lld]msec "
								" startline [%s] socket(%lld %s:%hu) ",
...
bool RtspDialog::onRead(const int8 *buffer, size_t bufSize)
{
    ...
    {	
		...
		{
			glog(Log::L_ERROR, "%s RTSP chopping() occurred a exception", szLogHint);
			break;			
		}
		
#if defined(DEBUG)
		glog(ZQ::common::Log::L_DEBUG, "%s OnRead() recv_byte[%d], req_count[%d], bytesNeedtoDecode[%d], bytesDecoded[%d], nBytesSkipped[%d]",
		...
		if (bCompletePkg)
		{
			...
			{
				//warning
				glog(ZQ::common::Log::L_WARNING, "%s onRead() req_index[%d] req_length[%d] bytes_skipped[%d]",
	...
	glog(ZQ::common::Log::L_DEBUG, "%s OnRead() recv_byte[%d], req_count[%d], bytesNeedtoDecode[%d] total_spent[%lld]ms",
...
void RtspDialog::dummyPing()
{
	 ...
	 {
		 glog(ZQ::common::Log::L_DEBUG, CLOGFMT(RtspConnection, "dummyPing() ignore"));
		 return;
	 }
	...
	if( sessId.empty() )
	{
		glog(ZQ::common::Log::L_DEBUG, CLOGFMT(RtspConnection, "dummyPing() conn[%s] session[NULL]"), _conn->getConnIdent());
		return;
	}
	...
	glog(ZQ::common::Log::L_INFO, CLOGFMT(RtspConnection, "dummyPing() conn[%s] session[%s]cseq[%d]"), pConn->getConnIdent(), (char*)sessId.c_str(), i+ 9990000);
#endif//_RTSP_PROXY
}
...
void RtspDialog::onError()
{
	DEBUG_DETAIL(DEBUG_DETAIL_LEVEL1) {
		glog(Log::L_DEBUG, "RtspDialog::onError()");
	}
	...
	if (sessionID.empty()) {
		...
		glog(Log::L_DEBUG, "RtspDialog::onError()\t"
			"a connection will close (conn = %p)", _conn);
		_conn->close();
...
void RtspDialog::fireTeardownForConnectionLostRequest()
{
	...
	if (!gRtspProxyConfig.lDummyPing) 
	{
		glog(ZQ::common::Log::L_DEBUG,CLOGFMT(RtspConnection,"fireTeardownForConnectionLostRequest() conn[%s] is nop destroyed, leave "), _conn->getConnIdent() );	
		return;
	}
	...
	RtspConnectionPtr pConn = _conn;
	glog(ZQ::common::Log::L_INFO,CLOGFMT(RtspConnection,"fireTeardownForConnectionLostRequest() conn[%s] is destroyed"), pConn->getConnIdent() );	
...
int RtspServerRequest::post()
{
	...
	if (conn->isActive()) {
		if (conn->send((uint8* )res.c_str(), len) <= 0)
			glog(Log::L_INFO, "RtspServerRequest::post(): failed to send data through COMM[%llu]", conn->getConnectionIdentity() );
	} else {
		glog(Log::L_DEBUG, "RtspServerRequest::post():  COMM[%llu] is closed",conn->getConnectionIdentity() );
	}
...
//	SOCKET socket = (SOCKET )conn->getSocket();
	...
	glog(ZQ::common::Log::L_DEBUG,"OutMessage socket: %010lld %s:%hu [session:%s] [seq:%s]",
...
void RtspServerRequest::release()
{
	...
	if(r==0)
	{
		...
		{
			glog(Log::L_DEBUG, 
				"RtspServerRequest::release():\t release this(%p), ref = %d ,should be deleted ", 
				this, r);
		}
		delete this;
	}
	else
	{
		...
		{
			glog(Log::L_DEBUG, 
				"RtspServerRequest::release():\t release this(%p), ref = %d  ", 
				this, r);
		}
	}	
}
void RtspServerRequest::reference()
{
	...
	DEBUG_DETAIL(DEBUG_DETAIL_LEVEL3) {
		glog(Log::L_DEBUG, 
			"RtspServerRequest::reference():\tthis(%p), ref = %d ", 
			this, r);
	}
}
