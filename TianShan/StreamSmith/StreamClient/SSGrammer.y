...
%token HELP EXIT SLEEP CLEAR
%token CONNECT DISCON 
%token LIST CREATE PUSH PUSHF PLAY PAUSE RESUME SEEK SEEKITEM SPEED RATE ERASE  CREATEUDP
...
cmd:  HELP { admin.help(); }
	...
	| LIST content { $$=$2; }
	| CREATE createargs { $$=$2; }
	| CREATEUDP ADDR INTEGER INTEGER { admin.createudpPlaylist($2,$3,$4);}
	| CREATEUDP ADDR INTEGER { admin.createudpPlaylist($2,$3);}
	| PUSH STR INTEGER { admin.pushItem($2, $3); }
	| PUSHF STR STR INTEGER { admin.pushItemWithFlag($2, $3, $4); }
	| PLAY { admin.play(); }
