...
std::string helperFixupPathname( const std::string& pathname , bool bSupportNpvr )
{
	...
	if( bSupportNpvr ) 
	{// reverse find the second slash
		//just like RaidDevices\DVL1\aaa
		...
		else
		{
			...
			{
				strNewName = pathname.substr( pos + 1 );
			}
		}
	}
	else
	{//find the last slash
		std::string::size_type lastSlashPos = pathname.find_last_of("\\");
		if( lastSlashPos != std::string::npos )
		{
			strNewName = pathname.substr( lastSlashPos + 1 );
		}
		else
		{
			strNewName = pathname;
...
{
	return helperFixupPathname( pathname, mPortalCtx->bSupportNpVr );
//	return mStore.fixupPathname( mStore , pathname );
}

bool vstrmCSPortalI::createPathOfVolume( const std::string& pathOfVolume ,const std::string& volume)
{
...
bool vstrmCSPortalI::findFiles( const std::string& searchPattern , std::vector<std::string>& files )
{
	...
	do
	{
		...
		if( pDelimiter )
		{
			fileName = fixupPathname(pDelimiter);
...
std::string vstrmCSPortalI::memberFileNameToContentName( const std::string& memberFilename )
{
	...
	std::transform(extfn1.begin(), extfn1.end(), extfn1.begin(), (int(*)(int)) std::toupper);
	if( mbEdgeServer && ( extfn1.find("0X") == 0  || extfn1.find("0x") == 0 ) )
	{
...
void vstrmCSPortalI::setMetaData( IN ContentImpl& content , IN const std::string& key , const uint64& value )
{
	char szBuf[256];
	szBuf[ sizeof(szBuf) - 1 ] = 0;
	snprintf( szBuf , sizeof(szBuf)-1 , "%I64u" , value );
	content.metaData[key] = szBuf;
	MLOG(ZQ::common::Log::L_DEBUG,PORTALFMT(setMetaData,"MainFile[%s] set metaData[%s]=[%s]"),
		content.ident.name.c_str(),key.c_str() , szBuf);
}
...
//	ZQTianShan::Util::getPropertyDataWithDefault( content.metaData , SYS_PROP(ContentPropertyParsedOK) , 0 , bParseOkByLoadAll );
	...
	if( bLocalContentForVstrm != 1 )//need record all metadata
	{
		...
		setMetaData( content , METADATA_MAIFILENAME() , idxData.getMainFilePathName() );
	...
	else
	{
		...
		if( lFileSize <= 0 )
		{
			...
			getMetaData( content , METADATA_MAIFILENAME() , mainFileName );
	...
	if( idxData.isPWE() )
	{
		setMetaData( content, METADATA_ContentInPWE, 1 );
	}
	else
	{
		content.metaData.erase(METADATA_ContentInPWE);
	}
	...
	MLOG( ZQ::common::Log::L_INFO , PORTALFMT(populateAttrsFromFile,"end populate properties for Content[%s]"),
		mainFilePathnameReal.c_str());
		...
		if( !mbEdgeServer )
		{
			...
			if(content)
			{
				if( getMetaData(content->metaData , METADATA_nPVRLeadCopy ,indexFilePath ))
				{
					...
					{
						bNpVr = true;
						MLOG(ZQ::common::Log::L_INFO, PORTALFMT(getFileSet,"MainFile[%s] current content is a nPVR copy"),mainFilePathName.c_str() );
					}
				}
			}
			if( !bNpVr )
...
uint64 vstrmCSPortalI::checkResidentialStatus( uint64 flagsToTest, ContentImpl::Ptr pContent,
												...
												const ::std::string& mainFilePathname )
{
	MLOG(ZQ::common::Log::L_INFO, PORTALFMT(checkResidentialStatus,"MainFile[%s] checkResidential with flag[%s]"),
	...
	if( flagsToTest & ( 1 << TianShanIce::Storage::frfResidential) )
	{
		...
		int64 stampByFillBySS = 0;
		if( pContent )
		{
			ZQTianShan::Util::getPropertyDataWithDefault( pContent->metaData, METADATA_STAMPFilledBySS, -1 , stampByFillBySS );
		}
		if( pContent && pContent->stampLastUpdated <=0 && stampByFillBySS <= 0 )
		{
		...
		{
			MLOG(ZQ::common::Log::L_DEBUG, PORTALFMT( checkResidentialStatus,"failed to check fileSet's frfResidential for MainFile[%s]"),
				mainFilePathname.c_str( ) );
		}
	}	
	...
	MLOG(ZQ::common::Log::L_INFO, PORTALFMT(checkResidentialStatus,"MainFile[%s] check Residential return [%s]"),
		mainFilePathname.c_str(), showCheckResidentialFlag(flagToRet).c_str());
...
void vstrmCSPortalI::getFileSetFromFS( const std::string& mainFilePathName , const std::string& indexFilePath ,  
										bool bNpvrCopy  , STRINGSET& fileSet, bool* bAllImportant)
{
	ZQ::IdxParser::IndexFileParser idxParser( *mIdxParserEnv );
	ZQ::IdxParser::IndexData idxData;
...
void vstrmCSPortalI::getRealFileSetFromMetaData( const TianShanIce::Properties& metaData , STRINGSET& fileSet )
{
	...
	getMetaData( metaData , METADATA_MAIFILENAME() , value );
	if( !value.empty() )
	{
		fileSet.push_back(value);
	}
											...
											bool* bAllImportant)
{
	...
	if( bEdgeServer )
	{
		if( getMetaData(metaData , METADATA_MAIFILENAME() , value ) )
		{
			fileSet.push_back( value );
		}		
	}
