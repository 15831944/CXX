...
bool	Playlist::doPause( bool bChangeState ,  OUT StreamControlResultInfo& info )
{
	...
	_lastErrCode=ERR_PLAYLIST_SUCCESS;	
	glog(ZQ::common::Log::L_INFO,PLSESSID(doPause,"done"));
	return ( status == VSTRM_SUCCESS );
}
	...
	try
	{
		...
		if( (isCompleted()  || _currentStatus!=PLAYLIST_PLAY)&& _isGoodItemInfoData )
		{
			if(_currentStatus==PLAYLIST_PAUSE)
			{
				...
				{
					glog(Log::L_INFO,PLSESSID(OnTimer,"current status[LAYLIST_PAUSE] and TimeoutOnPause[true], kill the playlist"));
					try
					{
						ClearAllResource();
						return;
				...
				{
					onPauseTimeout();
					glog(Log::L_INFO,PLSESSID(OnTimer,"current status[PLAYLIST_PAUSE], TimeoutOnPause[false], renew the timer"));
					SetTimerEx(gStreamSmithConfig.lPlaylistTimeout);
					return;
				}
			}
			glog(Log::L_INFO,PLSESSID(OnTimer,"playlist timed out at idle: state[%d], isComplete[%d]"),
			...
			{
				glog(Log::L_ERROR,PLSESSID(OnTimer,"caught unknown exception when invoke ClearAllResource()"));
			}
		}
		else 
		{
			if(_bCleared)
			{
				glog(Log::L_DEBUG,PLSESSID(OnTimer,"playlist is cleared, skip to clear resource"));
				return;
			}
			if(m_iErrInfoCount>=100)
			{
				ERRLOG(Log::L_ERROR,PLSESSID(OnTimer,"failed to read attributes from VstrmSess, tried 100 times, terminate playlist"));
...
void Playlist::destroy()
{
	...
	glog(Log::L_INFO, PLSESSID(destroy,"leave destroy"));
}
