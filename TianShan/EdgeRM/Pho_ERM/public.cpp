...
//	envlog(ZQ::common::Log::L_INFO,"Session[%s] add or update Resource with type[%s] key[%s]",
											   ...
											   const std::string& strkey)
{
	...
	if(itResMap==rcMap.end())
	{
		...
		ZQTianShan::_IceThrow <TianShanIce::InvalidParameter>(EXPFMT("ERMEdgePHO",1001,"%s"),(szBuf));
	}
	...
	if(it==itResMap->second.resourceData.end())
	{
		...
		ZQTianShan::_IceThrow <TianShanIce::InvalidParameter>(EXPFMT("ERMEdgePHO",1002,"%s"),(szBuf));
	}
	return it->second;	
}
