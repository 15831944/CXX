...
{
    DWORD dwErrCode = SYS::getLastErr();
	return SYS::getErrorMessage();
}
...
bool validatePath  ( const char * wszPath )
{
    ...
    DWORD dwErr = SYS::getLastErr();
    if ( dwErr == ERROR_PATH_NOT_FOUND || dwErr == ERROR_FILE_NOT_FOUND )
    {
        if (!::CreateDirectory(wszPath, NULL))
        {
            dwErr = SYS::getLastErr();
            if ( dwErr != ERROR_ALREADY_EXISTS)
            {
               glog(ZQ::common::Log::L_ERROR,  CLOGFMT(Common, "Failed to create directory %s Reason : %s"),
				   wszPath, SYS::getErrorMessage().c_str() );
               return false;
            }
        }
    }
    ...
    {
         glog(ZQ::common::Log::L_ERROR, CLOGFMT(Common,"Failed to access directory %s Reason : %s"), 
