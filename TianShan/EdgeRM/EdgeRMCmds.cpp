		...
		try
		{
			...
			for(routeIter; routeIter != routeInfos.end();routeIter++)
			{
				...
				for(portIter;portIter != portInfos.end();portIter++)
				{
					...
					if(NULL == devicePrx)
					{
						envlog(ZQ::common::Log::L_ERROR, CLOGFMT(ExportDevicesCmd, "get deviceProxy[%s] failed"), deviceIter->ident.name);
						return false;
					}
...
*/
				...
				else
				{
					...
					if(deviceExPrx == NULL)
					{
						envlog(ZQ::common::Log::L_ERROR, CLOGFMT(ImportDeviceCmd, "can not open existed deivce[%s] of endpoint[%s]"), _edgeDevice->ident.name.c_str(), _edgeDevice->tftpUrl);
						return;
					}
					_edgeDevices.push_back(devicePrx);
					//get local channels
