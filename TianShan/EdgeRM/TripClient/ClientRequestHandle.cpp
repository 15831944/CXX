...
int ClientRequestHandle::run()
{
	...
	ZQ::ERRP::ERRPMsg::Ptr pMsg = ZQ::ERRP::ERRPMsg::parseMessage(mBuf,mBufSize,byteProcessed);
	if(!pMsg)
	{
	...
	case ZQ::ERRP::MsgType_OPEN:
		{
			...
			glog(ZQ::common::Log::L_INFO,CLOGFMT(ClientRequestHandle,"Session[%lld] recieved confirm message to open request"),mComm->getCommunicatorId());
			break;
		}
	case ZQ::ERRP::MsgType_KEEPALIVE:
		{
			if(mLastMsgRecved == LASTMSG_OPEN)//this message is confirmed to open request
			{
				...
				glog(ZQ::common::Log::L_INFO,CLOGFMT(ClientRequestHandle,"Session[%lld] recieved confirm message to keep alive.Previous message:open request"),mComm->getCommunicatorId());
			}
			else if(mLastMsgRecved == LASTMSG_KEEPALIVE)//this message is confirmed to keepAlive,and start heart beat here
			{
				...
				glog(ZQ::common::Log::L_INFO,CLOGFMT(ClientRequestHandle,"Session[%lld] recieved confirm message to update."),mComm->getCommunicatorId());
			}
	...
	default:
		glog(ZQ::common::Log::L_ERROR,CLOGFMT(ClientRequestHandle,"Session[%lld] missed message type %d"),mComm->getCommunicatorId(),messageType);
