...
#define	PLFMT(x,y) 	"%s/%08X/CdnSSCommand[%s]\t"y,mStreamCtx.id().c_str(),pthread_self(),#x
#endif	
							 ...
							 TianShanIce::SRM::ResourceMap &crResource)
{
	...
	if(shadowIndexNotFound && gCdnSSConfig.c2StreamerConfig.clientType != 2 )  // expose index == false will not trigger this problem
	{
	...
	else if( gCdnSSConfig.c2StreamerConfig.clientType == 2 ) {
	...
	if( transferRate <=0 ) {
		...
		ENVLOG(ZQ::common::Log::L_INFO,PLFMT( doCommit, "no ransferrate is found, use asset't bitrate[%lld] instead"),transferRate);
	}

	ENVLOG(ZQ::common::Log::L_INFO,	PLFMT(doCommit,"initiate transfer with clientTransfer[%s] transferAddress[%s] "
		"ingressCapacity[%lld] allocatedIngressCapacity[%lld] fileName[%s] transferRate[%lld] "
		"transferTimeout[%d] range[%s] delay[%lld] exts[%s] upStreamBW[%lld]" ),
	...
	C2Streamer::TransferInitRequestParamPtr request = new C2Streamer::TransferInitRequestParam();
...
{	
	C2Streamer::TransferTermRequestParamPtr request = new C2Streamer::TransferTermRequestParam();
