...
bool CdnStreamerManager::listStreamer( SsReplicaInfoS& infos ) const
{
	...
	for( ; it != mStreamers.end() ; it ++ )
	{
		...
		ENVLOG(ZQ::common::Log::L_INFO, CLOGFMT(CdnStreamerManager,"listStreamer [%s] [%s/%s] capacity[%lld] found"),
...
void CdnStreamerManager::reportStreamerState( const StreamerAttr& attr )
{
	...
	ENVLOG(ZQ::common::Log::L_INFO,
		CLOGFMT(CdnStreamerManager,"Streamer attribute changed name[%s] address[%s/%s] capacity[%lld] state[%s]	activeTransferCount[%d] activeBandwidth[%lld]"),
		attr.portName.c_str(),
	...
	while ( !mbQuit )
	{
		...
		if( cacheStore )
		{
			...
			else
			{
				...
				ENVLOG(ZQ::common::Log::L_INFO,CLOGFMT(CdnStreamerManager,"update C2Service load: local streaming [%lld/%lld] pass through streaming [%lld/%lld], upStreaming ip [%s]"),
...
bool CdnStreamerManager::setC2Conf(C2Streamer::C2StreamerEnv& c2env )
{
	...
	for( ; itPortSpeed != portSpeedConf.end() ; itPortSpeed ++ )
	{
		if(! itPortSpeed->portName.empty())
		{
			...
			(getSsEnv()->getMainLogger())(ZQ::common::Log::L_INFO,CLOGFMT(setC2Conf,"SET port[%s]'s speed to [%lld] nat[%d/%d] "),
...
{	
	  ...
	  if (cpuNum <= 0)
			cpuNum = 2;
	  LibAsync::HttpProcessor::setup( cpuNum );
	...
	ENVLOG(ZQ::common::Log::L_INFO,CLOGFMT(CdnStreamerManager,"Start Http Engine at[%s][%s], threadcount[%d]"),
	...
	if( gCdnSSConfig.c2StreamerConfig.serverType == 1 ) {
		...
		C2Streamer::C2AsyncHttpHandlerFactory::Ptr asyncfac = new C2Streamer::C2AsyncHttpHandlerFactory(*mLibAsyncHttpServer.get(), 
															(env->getMainLogger()),fac,httpThreadCount);
...
{
	//query all port 
	C2Streamer::ResourceStatusRequestParamPtr request = new C2Streamer::ResourceStatusRequestParam();
...
void CdnStreamerManager::getShadowIndex(const std::string& upstreamUrl, const std::string& name,
		...
		const std::string& subtype  )
{
	static int clientType = gCdnSSConfig.c2StreamerConfig.clientType;
	if( clientType == 2 ) { //http fetcher
		...
		if( subtype.length() > 0 &&subtype.at(0) != '.')
			filename = filename + ".";
		filename += subtype;
		c2sessmanager.getAssetAttribute(filename);
		return;
	}
	std::string key = pid+paid;
	ZQ::common::MutexGuard gd(mC2ShadowIndexMutex);
...
{
	SESSLOG(ZQ::common::Log::L_INFO, EventFMT( (env->getNetId().c_str()) ,IngressCapacity, update , 0, "client[%s] update ingressCapacity[%lld]") ,
...
bool ContentFileAttributeBridge::getFileDataRange( const std::string& filename , const std::string& sessionId , int64& startByte , int64& endByte )
{
	...
	try
	{
		...
		ENVLOG(ZQ::common::Log::L_DEBUG,CLOGFMT(ContentFileAttributeBridge,"Session[%s]: get file[%s] data range[%llu - %llu], time cost[%lld/%lld/%lld]us"),
...
bool ContentFileAttributeBridge::isFileBeingWritten( const std::string& filename , const std::string& sessionId)
{
	...
	try
	{
		...
		{
			openWatch.stop();
			ENVLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(ContentFileAttributeBridge,"Session[%s]: content[%s] for file[%s] is not found, time cost[%lld]us"),
		...
		TianShanIce::Storage::ContentState state =  content->getState();
		getStateWatch.stop();
 		ENVLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(ContentFileAttributeBridge,"Session[%s]: got content[%s] for file[%s], state[%s] , time cost[%lld/%lld]us"),
