...
#include "memoryDebug.h"
	...
	#define	SSFMT(x,y) 	"[SsServiceImpl]%s TID[%08X][%16s]\t"##y,streamId.c_str(),GetCurrentThreadId(),#x
#else 
	#define	SSFMT(x,y) 	"[SsServiceImpl]%s TID[%08X][%16s]\t" y,streamId.c_str(),pthread_self(),#x
#endif
...
void SsServiceImpl::releaseStreamingResource( const TianShanIce::Properties& streamingRes )
{
	...
	if( streamerId.empty() || streamPort.empty() )
	{
		ENVLOG(ZQ::common::Log::L_WARNING,
			CLOGFMT(SsServiceImpl,"releaseStreamingResource() empty streamer id [%s] or stream port[%s]"),
			streamerId.c_str() ,
			streamPort.c_str() );
		return;
	}
								...
								const TianShanIce::Properties& props )
{
	...
	try
	{
		...
		if( enableNAT ){ZQTianShan::Util::getResourceDataWithDefault(srmResources,TianShanIce::SRM::rtEthernetInterface,"pokeholeSession","",natSessionId);}		
		bHasAttribute = true; 
	}
	catch( const TianShanIce::BaseException& ex)
	{		
...
void SsServiceImpl::removeServantFromDB( const Ice::Identity& id , const TianShanIce::Properties& resource )
{
	...
	if(mStreamEvictor)
	{
		...
		{
			ENVLOG(ZQ::common::Log::L_ERROR,
				CLOGFMT(SsServiceImpl,"failed to remove [%s/%s] from DB because ice exception [%s]"),
				id.category.c_str() , id.name.c_str() , ex.ice_name().c_str() );
		}
	}
	else
...
uint32	SsServiceImpl::reportReplica( )
{
...

	{
		...
		try
		{
			...
			{
				ENVLOG(ZQ::common::Log::L_ERROR,
					CLOGFMT(SsServiceImpl,"failed to connect to replica subscriber[%s]"),
					subscriberEP.c_str() );
			...
			else
			{
				...
				ENVLOG(ZQ::common::Log::L_INFO,
					CLOGFMT(SsServiceImpl,"report replica ok , and get update interval [%d]: %s "),
		...
		{
			ENVLOG(ZQ::common::Log::L_ERROR,
				CLOGFMT(SsServiceImpl,"failed to report replica to subscriber [%s], caught TianShan exception[%s]"),
				subscriberEP.c_str() ,
				ex.message.c_str()	);
		}
		catch( const Ice::Exception& ex )
		{
			ENVLOG(ZQ::common::Log::L_ERROR,
				CLOGFMT(SsServiceImpl,"failed to report replica to subscriber [%s] , caught ice exception[%s]"),
				subscriberEP.c_str() ,
				ex.ice_name().c_str()	);
		}
		catch(...)
		{
			ENVLOG(ZQ::common::Log::L_ERROR,
				CLOGFMT(SsServiceImpl,"failed to report replica to subscriber [%s] , caught unknown exception"),
				subscriberEP.c_str() );
		}
	}
