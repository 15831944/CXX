...
HRESULT NSSService::OnInit( )
{
	...
	glog(ZQ::common::Log::L_INFO, CLOGFMT(NSSService, "netID='%s'"), strNetId.c_str());
	...
	for (::ZQTianShan::NSS::NSSBaseConfig::NSSHolderVec::iterator iter = gNSSConfig._nssBaseConfig.NSSVec.begin(); iter != gNSSConfig._nssBaseConfig.NSSVec.end(); iter++)
	{
		...
		{
			glog(ZQ::common::Log::L_INFO, CLOGFMT(NSSService, "find NetID=%s"), strNetId.c_str());
			pNSSBaseConfig = &(*iter);
		}
	}
	...
	{
		glog(ZQ::common::Log::L_ERROR, CLOGFMT(NSSService, "couldn't find specified netID[%s]"), strNetId.c_str());
		return S_FALSE;
	}
