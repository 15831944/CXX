...
void NGODSession::OnANNOUNCE(RTSPClient& rtspClient, RTSPMessage::Ptr& pInMessage)
{
	...
	if (pInMessage->headers.find("StreamStatus") != pInMessage->headers.end())
	{
		...
		if (!streamStatus.empty() && 0 == pNSSBaseConfig->_videoServer.customer.compare("henan"))
		{
			...
			_log(Log::L_DEBUG, CLOGFMT(NGODSession, "OnANNOUNCE(), Session[%s] cseq(%d) added props[user.NGOD-StreamStatus] per customer[henan]"), _sessGuid.c_str(), pInMessage->cSeq);
		}
	}
...
: RTSPClient(log, thrdpool, bindAddress, baseURL, userAgent, verbosityLevel, bindPort), _group(group), _type(type), _bDecimalNpt(false), _bNptHandshaked(true), _cContinuousTimeout(0)
{
	if (_bTryDecimalNpt)
	{
		_bDecimalNpt = true;
...
void NGODClient::OnRequestError(RTSPClient& rtspClient, RTSPRequest::Ptr& pReq, RequestError errCode, const char* errDesc)
{
	...
	char buf[64];
	_log(Log::L_ERROR, CLOGFMT(NGODClient, "OnRequestError() conn[%s] %s(%d): %d %s; %s +%d"),
...
bool NGODClient::waitForResponse(uint32 cseq)
{
	...
	if (pNSSBaseConfig->_videoServer.SessionInterfaceDisconnectAtTimeout > 1 && isConnected() && ++_cContinuousTimeout >= (uint)pNSSBaseConfig->_videoServer.SessionInterfaceDisconnectAtTimeout)
	{
		_log(Log::L_WARNING, CLOGFMT(NGODClient, "waitForResponse() SessionGroup[%s] conn[%s]: counted timeout[%d]msec continously for [%d]times but limitation[%d], give it up and reconnecting"),
			_group.getName().c_str(), connDescription(), _messageTimeout, _cContinuousTimeout, pNSSBaseConfig->_videoServer.SessionInterfaceDisconnectAtTimeout);
...
void NGODClient::wakeupByCSeq(uint32 cseq)
{
	Event::Ptr pEvent =NULL;
	...
	_log(Log::L_DEBUG, CLOGFMT(NGODClient, "wakeupByCSeq() conn[%s](%d): signalling syncEvent[%p]"),
		connDescription(), cseq, pEvent.get());
