...
{//contentStateKey
	...
	{//get content's attribute such as duration and 
		...
		int32 ret = SsServiceImpl::doGetStreamAttr( serviceImpl , *this, it->sessionId , it->setupInfo , paras );
...
Ice::Long SsStreamImpl::calculateVideoDuration( iter itFirst, iter itLast )
{
	if ( !(iterValid(itFirst) && iterValid(itLast)))
	{
		ENVLOG(ZQ::common::Log::L_ERROR,PLFMT(calculateVideoDuration,"invalid input parameter"));
		return -1;
	}
	Ice::Long lRet = 0;
	iter it = itFirst;
	...
	for (;it <= itLast;++it)
	{
		playtimes = (it->setupInfo.flags & TianShanIce::Streamer::PLISFlagPlayTimes) >> 4;
...
int32 SsStreamImpl::getCurrentItemNpt()
{
	int32 lRet = 0;
	::TianShanIce::ValueMap vmap;
	::Ice::Current cur;
	if (getInfo(TianShanIce::Streamer::infoPLAYPOSITION,vmap,cur))
	{
		ZQTianShan::Util::getValueMapData(vmap,"itemOffset",lRet);
	}
	return lRet;
}

int32 SsStreamImpl::getCurrentPlaylistNpt()
{
	int32 lRet = 0;
	::TianShanIce::ValueMap vmap;
	::Ice::Current cur;
	if (getInfo(TianShanIce::Streamer::infoSTREAMNPTPOS,vmap,cur))
	{
		ZQTianShan::Util::getValueMapData(vmap,"itemOffset",lRet);
	}
	return 0;
