...
#define MSGTRSPORTLOGFMT(_C, _X) "%-18s [%s/%p] " _X, #_C, _name.c_str(), this
    ...
    :_log(log), _serverConfig(serverConfig), _ic(ic)
{
    ...
    {
        _log(Log::L_ERROR, MSGTRSPORTLOGFMT(MessageTransporter, "MessageTransporter() Caught [%s] during initializing Freeze."), e.ice_name().c_str());
        throw;
    }
}
MessageTransporter::~MessageTransporter()
{
    ...
    {
        _log(Log::L_ERROR, MSGTRSPORTLOGFMT(MessageTransporter, "~MessageTransporter() Caught [%s] during close Freeze connection."), e.ice_name().c_str());
    }
    catch(...)
    {
        _log(Log::L_ERROR, MSGTRSPORTLOGFMT(MessageTransporter, "~MessageTransporter() Caught kunkown exception during close Freeze connection."));
    }
}
...
MessageChannel* MessageTransporter::createChannel(const ChannelConfig& config)
{
    ...
    if(it == _channels.end()) // prevent creating duplicate channel
    {
        ...
        try{
            ...
            _log(Log::L_INFO, MSGTRSPORTLOGFMT(MessageTransporter, "createChannel() Created channel [%s] at [%p] successfully."), config.name.c_str(), channel);

            return channel;

        }catch(const Ice::Exception &e){
            _log(Log::L_ERROR, MSGTRSPORTLOGFMT(MessageTransporter, "createChannel() Caught Ice exception [%s] during creating channel [%s]."), e.ice_name().c_str(), config.name.c_str());
        } catch (const ZQ::common::Exception& e) {
            _log(Log::L_ERROR, MSGTRSPORTLOGFMT(MessageTransporter, "createChannel() Caught ZQ exception [%s] during creating channel [%s]."), e.getString(), config.name.c_str());
        } catch (...) {
            _log(Log::L_ERROR, MSGTRSPORTLOGFMT(MessageTransporter, "createChannel() Caught unknown exception during creating channel [%s]."), config.name.c_str());
        }
    }
    else
    {
        _log(Log::L_WARNING, MSGTRSPORTLOGFMT(MessageTransporter,
