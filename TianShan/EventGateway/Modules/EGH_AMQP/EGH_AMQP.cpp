...
// EGH_AMQP.cpp : Defines the entry point for the DLL application.
//
...
#ifdef EGH_AMQP_EXPORTS
#define EGH_AMQP_API __EXPORT
#else
#define EGH_AMQP_API __DLLRTL
#endif


#define THIS_MODULE_NAME "EGH_AMQP"

ZQ::common::Config::Loader<EventGateway::AMQP::AMQPConfig> gConfig(THIS_MODULE_NAME ".xml");
...
{
public:
	explicit TargetManager(const EventGateway::AMQP::AMQPConfig::Servers& servers);
	~TargetManager();

	typedef std::pair< std::string, std::string > Target;
	static Target parse(const std::string& target);

	EventGateway::AMQP::MessageChannel* get(const std::string& target);
	void clear();
private:
	EventGateway::AMQP::AMQPConfig::Servers _serversConf;
	EventGateway::AMQP::TransportManager* _transportMgr;
};
...
extern "C"
{
	EGH_AMQP_API bool EventGw_Module_Entry_init(EventGateway::IEventGateway* gateway)
	{
		using namespace EventGateway::AMQP;
		reset();
		_gw = gateway;

		_gw->superLogger()(ZQ::common::Log::L_INFO, "====================EGH_AMQP INIT====================");
		...
		{ // create event helpers
			...
			for(AMQPConfig::HandlersConfig::const_iterator it = gConfig.handlers.begin(); it != gConfig.handlers.end(); ++it)
			{
				if(!it->enabled)
					continue;
	...
	EGH_AMQP_API void EventGw_Module_Entry_uninit()
	{
		// unsubscribe
		{ // content event helper
			for(size_t i = 0; i < _eventHelpers.size(); ++i)
...
TargetManager::TargetManager(const EventGateway::AMQP::AMQPConfig::Servers& servers)
...
EventGateway::AMQP::MessageChannel* TargetManager::get(const std::string& target)
{
	using namespace EventGateway::AMQP;
	...
	if(NULL == _transportMgr)
	{
		try
		{
			_transportMgr = new EventGateway::AMQP::TransportManager((*_plog));
		}
		catch(...)
		{
	...
	else
	{
		...
		if(_serversConf.find(server) != _serversConf.end())
		{
			...
			if( serverConf.channels.find(channel) != serverConf.channels.end())
			{
				...
				{
					(*_plog)(Log::L_ERROR, CLOGFMT(TargetManager, "Caught unknown exception during initializing JMS MesssageBase."));
					return NULL;
				}
			}
			else
