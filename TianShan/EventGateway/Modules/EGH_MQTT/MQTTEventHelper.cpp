...
//  	for(it1=_tmpl.begin();it1!=_tmpl.end();++it1)
//  		_log(ZQ::common::Log::L_DEBUG, CLOGFMT(EventHelper, "format: %s"),it1->first.c_str(),it1->second.c_str());
//  	std::map<std::string,std::string>::iterator itc;
//  	for(itc=ctx.begin();itc!=ctx.end();++itc)
//  		_log(ZQ::common::Log::L_DEBUG, CLOGFMT(EventHelper, "not format msg: %s=%s"),itc->first.c_str(),itc->second.c_str());
	int Npt;
	char NptsM[32];
	std::string strNpt;
	std::map<std::string, std::string>::const_iterator it;
	...
	{
		strNpt = it->second;
		sscanf(strNpt.c_str(),"%x",&Npt);
		sprintf(NptsM, "%d.%03d", Npt/1000,Npt%1000); 
		ctx["sys.npt"] = NptsM;
	}
	it = ctx.find("sys.nptPrimary");
	if(it != ctx.end())
	{
		strNpt = it->second;
		sscanf(strNpt.c_str(),"%d",&Npt);
		sprintf(NptsM, "%d.%03d", Npt/1000,Npt%1000); 
		ctx["sys.nptPrimary"] = NptsM;
	}
    Message msg;
    try
    {
