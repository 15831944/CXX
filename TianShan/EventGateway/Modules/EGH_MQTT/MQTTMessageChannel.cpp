...
namespace MQTT{
	...
	bool MessageChannel::connect()
	{
		...
		char* serUri;
		serUri = (char*)address.data();
		nReply = MQTTClient_create(&_connect,address.c_str(),clientId.c_str(),MQTTCLIENT_PERSISTENCE_NONE,NULL);
		if(nReply != MQTTCLIENT_SUCCESS)
		{
			_log(ZQ::common::Log::L_ERROR, MQTTLOGFMT(MessageChannel, "faile to create connection with error:%d"),MQTTCLIENT_SUCCESS);
			//MQTTClient_destroy(&_connect);
			return false;
		}
		...
		{
			_log(ZQ::common::Log::L_ERROR,  MQTTLOGFMT(MessageChannel, "failed to connecting...."));
			//MQTTClient_destroy(&_connect);
 			return false;
		}
		else
		{
			_log(ZQ::common::Log::L_INFO,  MQTTLOGFMT(MessageChannel, "Connect to service id OOOOKKKK...."));
		}
	...
	bool MessageChannel::publish(const std::string& message)
	{
		...
		// 	}
		_log(ZQ::common::Log::L_INFO,  MQTTLOGFMT(MessageChannel, "published mqtt[%s]: %s"), _topicName.c_str(), message.c_str());
		return true;
	}
	...
	void MessageChannel::checkConnection()
	{
		...
		if(reply == 1)
		{
			...
			_log(ZQ::common::Log::L_DEBUG,  MQTTLOGFMT(MessageChannel, "Client lost connect"));
	}
