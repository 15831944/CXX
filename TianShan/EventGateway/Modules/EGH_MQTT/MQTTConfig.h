#ifndef __EventGw_MQTT_Config_H__
#define __EventGw_MQTT_Config_H__
...
namespace EventGateway{
    ...
    struct ChannelConfig
    {
        std::string name;
		std::string  id; 
		std::string msgType; // message type: struct_id  MQTC  MQTM  MQTW ...
		int32 TTL;
		int32   QOS;
		int32 optionEnabled;
		...
		*/
        ...
        static void structure(ZQ::common::Config::Holder<ChannelConfig> &holder)
		{
			...
			//set MQTTClient_connectOptions from config file
			holder.addDetail("MqttCli_ConnOpts", "qos", &ChannelConfig::QOS, "0", optReadOnly);
			holder.addDetail("MqttCli_ConnOpts", "reliable", &ChannelConfig::reliable,  "1", optReadOnly);
			holder.addDetail("MqttCli_ConnOpts", "struct_id", &ChannelConfig::msgType, "MQTC",optReadOnly);
			holder.addDetail("MqttCli_ConnOpts", "MQTTVersion", &ChannelConfig::MQTTVersion, "0", optReadOnly);
			holder.addDetail("MqttCli_ConnOpts", "cleansession", &ChannelConfig::cleansession, "1", optReadOnly);
			holder.addDetail("MqttCli_ConnOpts", "retryInterval", &ChannelConfig::retryInterval, "20", optReadOnly);
			holder.addDetail("MqttCli_ConnOpts", "connectTimeout", &ChannelConfig::connectTimeout, "30", optReadOnly);
			holder.addDetail("MqttCli_ConnOpts", "keepAliveInterval", &ChannelConfig::keepAliveInterval, "60", optReadOnly);
    ...
    struct MQTTConfig
    {
        ...
        static void structure(ZQ::common::Config::Holder<MQTTConfig> &holder)
        {
            using namespace ZQ::common::Config;
            holder.addDetail("EGH_MQTT/Log", "level", &MQTTConfig::logLevel, NULL, optReadOnly);
            holder.addDetail("EGH_MQTT/Log", "size", &MQTTConfig::logFileSize, NULL, optReadOnly);
            holder.addDetail("EGH_MQTT/Log", "count", &MQTTConfig::logCount, "5", optReadOnly);
            holder.addDetail("EGH_MQTT/SafeStore", "path", &MQTTConfig::safeStorePath, NULL, optReadOnly);

            holder.addDetail("EGH_MQTT/RabbitMQServer", &MQTTConfig::readRabbitMQServer, &MQTTConfig::registerRabbitMQServers, Range(1, -1));
            holder.addDetail("EGH_MQTT/EventHandlers/Handler", &MQTTConfig::readHandler, &MQTTConfig::registerHandlers);
        }
