...
//
// Ident : $Id: BaseAquaRecSession.cpp $
// Branch: $Name:  $
// Author: Hui Shao
// Desc  : 
//
...
bool BaseCPHelper::reg(const ::std::string& provisionId, BaseAquaRecSession::Ptr pSess)
{
	...
	
	{
		...
		if (_sessMap.end() != it)
		{
			BaseAquaRecSession::Ptr& ptr= it->second;			
...
{
	std::string new_provisionId = provisionId;
	if (!_bInCleanup)
	{
		try
		{
	...
	EngHelperLog(ZQ::common::Log::L_DEBUG, CLOGFMT(BaseCPHelper, "unreg() sess[%s] in CPE"), new_provisionId.c_str());
	try{
		if (NULL != _mgr)
			_mgr->unregisterHelperSession(new_provisionId.c_str());
	} catch(...) {}
...
// -----------------------------
// class BaseAquaRecSession
// -----------------------------
#undef cpelog 
#define cpelog (*_helper._pCPELogger)
BaseAquaRecSession::BaseAquaRecSession(BaseCPHelper& helper, const ::TianShanIce::ContentProvision::ProvisionSessionExPtr& pSess)
: _helper(helper), _sess(pSess), _pPushSource(NULL)
{
	...
	EngSessLog(ZQ::common::Log::L_DEBUG, CPHSESSLOGFMT(BaseAquaRecSession, "BaseAquaRecSession() this[0x%08x]"), this);
}

BaseAquaRecSession::~BaseAquaRecSession()
{
	EngSessLog(ZQ::common::Log::L_DEBUG, CPHSESSLOGFMT(BaseAquaRecSession, "~BaseAquaRecSession() this[0x%08x]"), this);
}

bool BaseAquaRecSession::prime()
{
	if (!_bPreloaded)
	{
		if (!preLoad())
...
void BaseAquaRecSession::notifyStarted(const ::TianShanIce::Properties& params)
{
	try
	{
		_sess->notifyStarted(params, ::Ice::Current());
		EngSessLog(ZQ::common::Log::L_INFO, CPHSESSLOGFMT(BaseAquaRecSession, "notifyStarted()"));
	}
	catch(...)
	{
		EngSessLog(ZQ::common::Log::L_WARNING, CPHSESSLOGFMT(BaseAquaRecSession, "notifyStarted() caught unknown exception"));
	}	
}

void BaseAquaRecSession::notifyStopped(bool errorOccurred, const ::TianShanIce::Properties& params)
{
	...
	try
	{
		...
		EngSessLog(ZQ::common::Log::L_INFO, CPHSESSLOGFMT(BaseAquaRecSession, "notifyStopped() errorOccurred=%c"), errorOccurred?'Y':'N');
	}
	catch(...)
	{
		EngSessLog(ZQ::common::Log::L_WARNING, CPHSESSLOGFMT(BaseAquaRecSession, "notifyStopped() errorOccurred=%c caught unknown exception")
			, errorOccurred?'Y':'N');
	}
}

void BaseAquaRecSession::notifyStreamable(bool streamable)
{
	try
	{
		_sess->notifyStreamable(streamable, ::Ice::Current());
		EngSessLog(ZQ::common::Log::L_INFO, CPHSESSLOGFMT(BaseAquaRecSession, "notifyStreamable() streamable=%c"), streamable?'Y':'N');
	}
	catch(...)
	{
		EngSessLog(ZQ::common::Log::L_WARNING, CPHSESSLOGFMT(BaseAquaRecSession, "notifyStreamable() streamable=%c caught unknown exception")
			, streamable?'Y':'N');
	}
}

void BaseAquaRecSession::updateProgress(const ::Ice::Long processed, const ::Ice::Long total)
{
	try
	{
	...
	#if 0
		EngSessLog(ZQ::common::Log::L_DEBUG, CPHSESSLOGFMT(BaseAquaRecSession, "updateProgress() %lld/%lld"), processed, total);
	#endif // _DEBUG
	}
	catch(...)
	{
		EngSessLog(ZQ::common::Log::L_WARNING, CPHSESSLOGFMT(BaseAquaRecSession, "updateProgress() %lld/%lld caught unknown exception")
			, processed, total);
	}
}

void BaseAquaRecSession::destroy()
{
