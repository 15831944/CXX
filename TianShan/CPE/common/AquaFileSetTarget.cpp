	...
	namespace ContentProvision {
		...
		bool AquaFilesetTarget::openSubFiles()
		{
			for(int i=0;i<_nInputCount;i++)
			{
				...
				{
					MOLOG(Log::L_INFO, CLOGFMT(FsIo, "[%s] ingore main file %s"),_strLogHint.c_str(), _subFiles[i].strFilename.c_str());
					continue;
				}
		...
		void AquaFilesetTarget::initSubFiles()
		{
			...
			if(_bAquaIndexTmpfile)
			{
				...
				MOLOG(Log::L_INFO, CLOGFMT(FsIo, "[%s] original IndexFileName[%s], Aqua Temp IndexFilename[%s]"),_strLogHint.c_str(), _strIndexFilename.c_str(), _subFiles[i].strFilename.c_str());
			}
		...
		{
			MOLOG(Log::L_INFO, CLOGFMT(FsIo, "[%s]processOutPut() ProcessBytes[%lld] IsErrorOccurred[%d]"), _strLogHint.c_str(), GetGraph()->IsErrorOccurred());
		...
		bool AquaFilesetTarget::Receive(MediaSample* pSample, int nInputIndex)
		{
			...
			{
				MOLOG(Log::L_DEBUG, CLOGFMT(FsIo, "Receive() input index: %d and IngoreMainfile is [%d] will skip main file"),nInputIndex, _bIngoreMainfile);
				unsigned int dwActualLen = pSample->getDataLength();
				subFile.llProcOffset += dwActualLen;	
				GetGraph()->freeMediaSample(pSample);
				return true;
...
*/
		...
		bool AquaFilesetTarget::renameBackIndexTmpName()
		{
			...
			if(_bAquaIndexTmpfile)
			{
				...
				MOLOG(Log::L_INFO, CLOGFMT(FsIo, "rename file [%s] to [%s] %s"), _subFiles[1].strFilename.c_str(), _strIndexFilename.c_str(), bret?"success":"failure");
			}
			return bret;
		}
