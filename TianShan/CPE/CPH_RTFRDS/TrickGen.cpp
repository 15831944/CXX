...
class CTFGen : public ZQTianShan::ContentProvision::BaseGraph, public ZQ::common::BufferPool,public ZQ::common::NativeThread
{
public:
	CTFGen()
		: _bQuit(false) 
...
bool SetRegEdit(LPCTSTR dataset,char* module)
{
	...
	if(ret0!=ERROR_SUCCESS)
	{
		...
		{
			MOLOG(Log::L_ERROR, CLOGFMT(TrickGen, "The key %s did not exist then failed to create it"),
				dataset);
		}
		else
		{
			MOLOG(Log::L_ERROR, CLOGFMT(TrickGen, "The key %s existed then failed to open it"),
				dataset);
		}
		return FALSE;
	}
	...
	if(ret1!=ERROR_SUCCESS)
	{
		...
		if(ret1!=ERROR_SUCCESS)
		{
			MOLOG(Log::L_ERROR, CLOGFMT(TrickGen, "Failed to set configuration [%s] = [%u],error is %d"),
				firstV.c_str(),defaultOnlyFlag,GetLastError());
			::RegCloseKey(hKEY);
			return FALSE;
		}
		MOLOG(Log::L_INFO, CLOGFMT(TrickGen, "Set configuration [%s] = [%u]"),
...
bool InitCPH(std::string& strCfgDir,std::string& strlogName)
{
	...
	{
		MOLOG(Log::L_ERROR, CLOGFMT(TrickGen, "Failed to load configuration [%s]"), 
	...
	{
		MOLOG(Log::L_INFO, CLOGFMT(TrickGen, "Load configuration from [%s] successfully"),
			_gCPHCfg.getConfigFileName().c_str());
	...
	//
	if (!RTFProcess::initRTFLib(_gCPHCfg.rtfMaxSessionNum, &glog, _gCPHCfg.rtfMaxInputBufferBytes,
		_gCPHCfg.rtfMaxInputBuffersPerSession, _gCPHCfg.rtfSessionFailThreshold))
	{
		filelog.flush();
		printf("Failed to initialize trick generation library, check log for detail\n");
		return false;
	}
...
bool CTFGen::preLoad(std::string& strSrcFile, int nBandwidth, std::string& strFilename,std::string& strtrickspeed, int nTypeH264, int nSrcFileset)
{
	initialize(65536, 800, 400);
	SetLog(&filelog);
	SetMemAlloc(this);
	...
	MOLOG(Log::L_DEBUG, CLOGFMT(TrickGen, "[%s] bandwidth[%d], max_limit_rate[%d], maxbandwidth[%d]"), _strLogHint.c_str(), 
...
#endif
	{
		...
		if (outPutNum < 2)
		{
			MOLOG(Log::L_ERROR, CLOGFMT(TrickGen, "[%s] Not specify trick speed"), _strLogHint.c_str());
			return false;
		}
...
//			pTarget->enablePacing(false);
	...
	if (!Init())
	{
		MOLOG(Log::L_ERROR, CLOGFMT(TrickGen, "[%s] Failed to initialize graph with error: %s"), _strLogHint.c_str(), _strLastErr.c_str());
		return false;
	}

	if (!Start())
	{
		MOLOG(Log::L_INFO, CLOGFMT(TrickGen, "[%s] failed to start graph withe error: %s"), _strLogHint.c_str(), _strLastErr.c_str());
		return false;
	}
	
	_bCleaned = false;
	MOLOG(Log::L_INFO, CLOGFMT(TrickGen, "[%s] init successful"), _strLogHint.c_str());
	return true;
}
...
int CTFGen::run( void )
{
	...
	if (!bRet)
	{
		...
		MOLOG(Log::L_INFO, CLOGFMT(TrickGen, "[%s] provision done, status[failure], error[%s]"), 
	...
	MOLOG(Log::L_INFO, CLOGFMT(TrickGen, "[%s] provision done, status[success], spent[%d]ms, actualspeed[%d]bps"), 
		_strLogHint.c_str(), nTimeSpentMs, nActualBps);

	return true;
}
void CTFGen::OnProgress(LONGLONG& prcvBytes)
{
...
{
	MOLOG(Log::L_INFO, CLOGFMT(TrickGen, "[%s] notifyStreamable() called"), _strLogHint.c_str());
}

void CTFGen::OnMediaInfoParsed(MediaInfo& mInfo)
{
	MOLOG(Log::L_INFO, CLOGFMT(TrickGen, "[%s] OnMediaInfoParsed() called"), _strLogHint.c_str());
