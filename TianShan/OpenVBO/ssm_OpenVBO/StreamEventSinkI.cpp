...
{
	// only send to STB
	_fileLog(ZQ::common::Log::L_INFO, CLOGFMT(StreamEventSinkI, "sendANNOUNCE_SessionInProgress() Session[%s]"), sessionContext.ident.name.c_str());
	...
	for (; iter != srmConnections.end(); iter++)
	{
		...
		if (!SRMAnnounce)
		{
			_fileLog(ZQ::common::Log::L_ERROR, CLOGFMT(StreamEventSinkI, "session[%s] failed to create STB ANNOUNCE Request"), sessionContext.ident.name.c_str());
			continue;
		}
		...
		SRMAnnounce->post();
		ANNOUNCELOG(ZQ::common::Log::L_INFO, CLOGFMT(StreamEventSinkI, "session[%s] Event(Session-In-Progress) has been posted to STB"), sessionContext.ident.name.c_str());
	}
}
...
int	StreamEventDispatchRequest::run()
{
	...
	if (NULL == sessionProxy)
	{
		ANNOUNCELOG(ZQ::common::Log::L_ERROR, CLOGFMT(StreamEventSinkI, "failed to get session[%s] context for stream[%s]"), idents[0].name.c_str(), _uid.c_str());
		return -1;
	}
	...
	case ON_ITEM_STEP:
		{
			...
			//strNotice = generatorNoticeString(OPENVBO_ANNOUNCE_ITEMSTEPPED, OPENVBO_ANNOUNCE_ITEMSTEPPED_STRING, currentDate);		
		}
	case ON_EXIT:
		strEvent = "On-Exit";
		break;
