...
#define GET_VALUE_FROM_MAP_BY_INDEX(index)   (it = erroeToCountMapRef.find(index), (it != erroeToCountMapRef.end()) ? it->second : 0 )
    ...
    for(int idxRow = 1; idxRow <= STATISTICS_COUNT; idxRow++)
    {
        ...
        SvcMIB_ExportReadOnlyVar(fixString("countSubTotal#", idxRow), &pStats->totalCount, AsnType_Int32, subOid);

        subOid = subOidTbl, subOid.append(3), subOid.append(idxRow);
        SvcMIB_ExportReadOnlyVar(fixString("latencyMax#", idxRow), &pStats->timeCostMax, AsnType_Int32, subOid);

        subOid = subOidTbl, subOid.append(4), subOid.append(idxRow);
        SvcMIB_ExportReadOnlyVar(fixString("latencyAvg#", idxRow), &pStats->timeCostAvg, AsnType_Int32, subOid);
