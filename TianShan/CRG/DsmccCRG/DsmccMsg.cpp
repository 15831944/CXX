...
DsmccMsg::DsmccMsg(HardHeader& hardHeader, ProtocolType protocolType)
...
DsmccMsg::Ptr DsmccMsg::parseMessage(const uint8* buf, size_t maxLen, size_t& bytesProcessed, ProtocolType protocolType)
{
	Ptr pMsg = NULL;
	const uint8* p = buf;
	bytesProcessed =0;
	...
	case 0x0000:
	default:
		pMsg = new DsmccMsg(tmpHeader, protocolType);
	}
...
bool ClientSessionSetupRequest::parseMotoPrivateData()
{
	uint16 privateDataLength = (uint16)_privateDataBytes.size();
...
bool ClientSessionSetupRequest::parseTangbergPrivateData()
{
	uint16 privateDataLength = (uint16)_privateDataBytes.size();
	uint16 processedLength = 0;

	if(privateDataLength < 39)
		return false;
	//parse ProtocolId, ProtocolVersion, DescriptorCount
	_ProtocolId = _privateDataBytes[processedLength]; processedLength++;
	_ProtocolVersion = _privateDataBytes[processedLength]; processedLength++;

    //parse ServiceGateWay 
	uint8 serviceGateway[16];
	memcpy(&serviceGateway, &_privateDataBytes[processedLength], sizeof(serviceGateway)); processedLength+= sizeof(serviceGateway);
    _serviceGateway = std::string(&serviceGateway[0], &serviceGateway[0] + 16);
	int nlen = _serviceGateway.size();
	//parse ServiceGateWayDataLength 
	uint32 serviceGateayDataLen;
	memcpy(&serviceGateayDataLen, &_privateDataBytes[processedLength], sizeof(serviceGateayDataLen));processedLength+= sizeof(serviceGateayDataLen);
	serviceGateayDataLen = ntohl(serviceGateayDataLen);

	//parse service
	uint8 service[16];
	memcpy(&service, &_privateDataBytes[processedLength], sizeof(service)); processedLength+= sizeof(service);
     _service = std::string(&service[0], &service[0] + sizeof(service));
	 nlen = _service.size();
	//parse DataLenth
	uint32 serviceDataLen;
	memcpy(&serviceDataLen, &_privateDataBytes[processedLength], sizeof(serviceDataLen));processedLength+= sizeof(serviceDataLen);
	serviceDataLen = ntohl(serviceDataLen);
   ...
   //parse ServiceData
	...
	for(uint8 i = 0; i < DescriptorCount; i++)
	{
		...
		{
		case 0x01:
			memcpy(&_assetIdPayLoad, &_privateDataBytes[processedLength], length);
			_assetIdPayLoad = ntohl(_assetIdPayLoad);
			processedLength += length;	
			break;
		case 0x02:
			uint8 buf[8];
		...
		case 0x05: // AppRequest Data 
			{
				if(privateDataLength < processedLength +length)
					return false;
				uint8 appTag = _privateDataBytes[processedLength]; processedLength++;
				switch (appTag)
				{
				case 0x01: //purchase or order Offering, no data
					break;
				case 0x02: // view already purchased video, no data
					break;
				case 0x03: //purchase or order subscription package, no data
					break;
				case 0x04: //SVOD authorization list request, no data
					break;
				case 0x05: //Rental List Request, Start position and count of rentals to be returnted
					break;
				case 0x06: //preview request, no data
					break;
				case 0x08: //settop Error Report, Settop Errors
					break;
				case 0x09: //Rental Deletion Request, purchase Id
					break;
				case 0x0B: //service Name, Serive Name
					break;
				default:
					break;
				}
				processedLength += length - sizeof(appTag);
			}
			break;
		case 0xF0:
			memcpy(&_serviceGroup, &_privateDataBytes[processedLength], length);
			_serviceGroup = ntohs(_serviceGroup);
			processedLength += length;
		default:
			if(privateDataLength < processedLength +length)
				return false;
	...
	return (processedLength == privateDataLength ? true : false);
}
bool ClientSessionSetupRequest::parsePrivateData()
{
	if(_protocolType == Protocol_MOTO)
	{
		return parseMotoPrivateData();
	}
	else if(_protocolType == Protocol_Tangberg)
	{
		return parseTangbergPrivateData();
	}
	else
		return false;
}
...
/*	std::string sessionId(_minBody.sessionId, _minBody.sessionId + sizeof(_minBody.sessionId));
	...
	char strBuff[8];
	memset(strBuff, 0, 8);
	snprintf(strBuff, 8, "%d", _protocolType);
	std::string strProtocolType = strBuff;
	MAPSET(StringMap, metadata, CRMetaData_ProtocolType, strProtocolType);
	...
	{
		memset(strTemp, 0, 65);
		sprintf(strTemp, "%u",  _assetIdPayLoad);
		std::string assetIdPayLoad = strTemp;
		MAPSET(StringMap, metadata, CRMetaData_assetIdPayLoad, assetIdPayLoad);

		if(!_serviceGateway.empty())
			MAPSET(StringMap, metadata, CRMetaData_ServiceGateWay, _serviceGateway);
		if(!_service.empty())
			MAPSET(StringMap, metadata, CRMetaData_Service, _service);

///////////////modify
		memset(strTemp, 0, 65);
		sprintf(strTemp, "%u",  _ProtocolId);
		std::string protocolId = strTemp;
		MAPSET(StringMap, metadata, CRMetaData_AppPDprotocolId,protocolId);
		memset(strTemp, 0, 65);
		sprintf(strTemp, "%u",  _ProtocolVersion);
		std::string protocolVersion = strTemp;
		MAPSET(StringMap, metadata, CRMetaData_AppPDprotocolVersion,protocolVersion);

		//add test for assetId
		//MAPSET(StringMap, metadata, CRMetaData_assetId, assetIdPayLoad);
	}
...
bool ClientSessionSetupRequest::readMetaData(const StringMap& metadata)
{
	...
	{
		_privateDataBytes.push_back(_ProtocolId);
		_privateDataBytes.push_back(_ProtocolVersion);
		itorMd = metadata.find(CRMetaData_assetIdPayLoad);
		if(itorMd == metadata.end() || itorMd->second.empty())
		{
			return false;
		}
		_assetIdPayLoad= atoi(itorMd->second.c_str());
		pdDescriptorCount++;

		//read privateData 
        uint32 pdServiceGatewayLength = 0;
		uint32 pdServiceDataLength = 0;
		
		itorMd = metadata.find(CRMetaData_ServiceGateWay);
		if(itorMd == metadata.end() || itorMd->second.empty()|| itorMd->second.size() != 16)
		{
			return false;
		}
		_serviceGateway = itorMd->second;

		itorMd = metadata.find(CRMetaData_Service);
		if(itorMd == metadata.end() || itorMd->second.empty() ||itorMd->second.size() != 16)
		{
			return false;
		}
		_service = itorMd->second;

        //insert ServiceGateWay
		_privateDataBytes.insert(_privateDataBytes.begin() + _privateDataBytes.size(), _serviceGateway.begin(), _serviceGateway.end());

		//insert ServiceGateWayDataLength uint32, 4Byte
		//readPrivateAppData(pdServiceGatewaylength); // update it

		//insert Service
		_privateDataBytes.insert(_privateDataBytes.begin() + _privateDataBytes.size(), _service.begin(), _service.end());

		//insert ServiceDataLength uint32, 4Byte
		//readPrivateAppData(pdServiceDataLenth); // update it

		_privateDataBytes.push_back(pdDescriptorCount); //update it 

		//insert assetid payload info 
		_privateDataBytes.push_back(0x01);
		_privateDataBytes.push_back(0x04);
		{
			uint8 buf[4];
			memset(buf, 0, sizeof(buf));
			uint32 uTemp = htonl(_assetIdPayLoad);
		...
		//insert appresquest data
		_privateDataBytes.push_back(0x05);
		_privateDataBytes.push_back(0x01);
		_privateDataBytes.push_back(0x01);
		pdDescriptorCount++;

		//updata ServiceGateWayDataLength;
		pdServiceGatewayLength = _privateDataBytes.size() - 22  + 8;
		//_privateDataBytes[18, 19 20 21];
		{
			uint8 buf[4];
			memset(buf, 0, sizeof(buf));
			uint32 uTemp = htonl(pdServiceGatewayLength);
			memcpy(buf, &uTemp, sizeof(uTemp));
			_privateDataBytes.insert(_privateDataBytes.begin() + 18, buf, buf + 4);
		}
	
		pdServiceDataLength =  _privateDataBytes.size() - 42 + 4;
		//_privateDataBytes[38,39, 40 ,41];
		{
			uint8 buf[4];
			memset(buf, 0, sizeof(buf));
			uint32 uTemp = htonl(pdServiceDataLength);
			memcpy(buf, &uTemp, sizeof(uTemp));
			_privateDataBytes.insert(_privateDataBytes.begin() + 38, buf, buf + 4);
		}

		//update server data DescriptorCount
		_privateDataBytes[42] = pdDescriptorCount;
	}
...
///////////////  class ClientSessionSetupRequest  ////////////////////
/////////////// //////////////////////////////////////////////////////
ClientSessionSetupConfirm::ClientSessionSetupConfirm(HardHeader& hardHeader, ProtocolType protocolType):DsmccMsg(hardHeader, protocolType)
{
...
bool ClientSessionSetupConfirm::parseMotoPrivateData()
{
	uint16 privateDataLength = (uint16)_privateDataBytes.size();
	uint16 processedLength = 0;
...
bool ClientSessionSetupConfirm::parseTangbergPrivateData()
{
	uint16 privateDataLength = (uint16)_privateDataBytes.size();
	uint16 processedLength = 0;
	...
	uint8 service[16];
	memcpy(&service, &_privateDataBytes[processedLength], sizeof(service)); processedLength+= sizeof(service);
	_service = std::string(&service[0], &service[0] + 16);
	...
	for(uint8 i = 0; i < DescriptorCount; i++)
	{
		...
		case 0x03:
			...
			break;
		case 0x04:
			memcpy(&_streamHandel, &_privateDataBytes[processedLength], length);processedLength += length;
			_streamHandel = ntohl(_streamHandel);
			break;
		case 0x06: // AppResponse Data 
			{
				if(privateDataLength < processedLength + length)
					return false;
				_responseType = _privateDataBytes[processedLength]; processedLength++;
				switch(_responseType)
				{
				case 0x01: //Response Codes
					processedLength += length - sizeof(_responseType);
					break;
				case 0x02: //Subscription Package Authorization list
					processedLength += length - sizeof(_responseType);
					break;
				case 0x05: //purchase response
					{
						if(privateDataLength < processedLength + 8)
							return false;
						uint32 purchaseId = 0;
						uint32 OfferingID = 0;
						memcpy(&purchaseId, &_privateDataBytes[processedLength], sizeof(purchaseId));processedLength += sizeof(purchaseId);
						memcpy(&OfferingID, &_privateDataBytes[processedLength], sizeof(OfferingID));processedLength += sizeof(OfferingID);
						_purchaseId = ntohl(purchaseId);
						_offeringId = ntohl(OfferingID);
					}
					break;
				default:
					processedLength += length - sizeof(_responseType);
...
uint32 ClientSessionSetupConfirm::toMetaData(StringMap& metadata)
{
	...
	memset(strTemp, 0, sizeof(strTemp));
	itoa(_IpAddress, strTemp, 10);
	std::string IpAddress = strTemp;
	MAPSET(StringMap, metadata, CRMetaData_IPaddress, IpAddress);

	memset(strTemp, 0, sizeof(strTemp));
	itoa(_IpPort, strTemp, 10);
	std::string ipPort = strTemp;
	MAPSET(StringMap, metadata, CRMetaData_Port, ipPort);

	memset(strTemp, 0, sizeof(strTemp));
	itoa(_streamHandel, strTemp, 10);
	std::string streamHandel = strTemp;
	MAPSET(StringMap, metadata, CRMetaData_StreamHandelId, streamHandel);

	if(_protocolType == Protocol_Tangberg)
	{
		memset(strTemp, 0, sizeof(strTemp));
		itoa(_purchaseId, strTemp, 10);
		std::string purchaseId = strTemp;
		MAPSET(StringMap, metadata, CRMetaData_purchaseId, purchaseId);

		memset(strTemp, 0, sizeof(strTemp));
		itoa(_offeringId, strTemp, 10);
		std::string offeringId = strTemp;
		MAPSET(StringMap, metadata, CRMetaData_offeringId, offeringId);

		memset(strTemp, 0, sizeof(strTemp));
		itoa(_responseType, strTemp, 10);
		std::string responseType = strTemp;
		MAPSET(StringMap, metadata, CRMetaData_responseType, responseType);
		...
		///////////////modify
		memset(strTemp, 0, sizeof(strTemp));
		itoa(_ProtocolId, strTemp, 10);
		std::string ProtocolId = strTemp;
		MAPSET(StringMap, metadata, CRMetaData_AppPDprotocolId, ProtocolId);
		memset(strTemp, 0, sizeof(strTemp));
		itoa(_ProtocolVersion, strTemp, 10);
		std::string ProtocolVersion = strTemp;
		MAPSET(StringMap, metadata, CRMetaData_AppPDprotocolVersion, ProtocolVersion);
	}
	return (uint32)metadata.size();
...
bool ClientSessionSetupConfirm::readMetaData(const StringMap& metadata)
{
	...
	else if(_protocolType == Protocol_Tangberg)
	{
		...
		itorMd = metadata.find(CRMetaData_AppPDprotocolId);
		if(itorMd == metadata.end() || itorMd->second.empty())
		{
			return false;
		}
		_ProtocolId = atoi(itorMd->second.c_str());
		itorMd = metadata.find(CRMetaData_AppPDprotocolVersion);
		if(itorMd == metadata.end() || itorMd->second.empty())
		{
			return false;
		}
		_ProtocolVersion = atoi(itorMd->second.c_str());
		...
		//insert appResponse Data
		itorMd = metadata.find(CRMetaData_responseType);
		if(itorMd == metadata.end() || itorMd->second.empty())
		{
			return false;
		}
		_responseType = atoi(itorMd->second.c_str());
		_privateDataBytes.push_back(0x06);
		pdDescriptorCount++;
		switch(_responseType)
		{
		case 0x01:
			_privateDataBytes.push_back(0x0);
			break;
		case 0x02:
			_privateDataBytes.push_back(0x0);
			break;
		case 0x05:
			...
			_purchaseId = atoi(itorMd->second.c_str());

			itorMd = metadata.find(CRMetaData_offeringId);
			if(itorMd == metadata.end() || itorMd->second.empty())
			{
				return false;
			}
			_offeringId = atoi(itorMd->second.c_str());
			{
				memset(buf, 0, sizeof(buf));
				uint32 purchaseId = htonl(_purchaseId);
				memcpy(buf, &purchaseId, sizeof(purchaseId));
				_privateDataBytes.insert(_privateDataBytes.begin() + _privateDataBytes.size(), buf, buf +4);

				memset(buf, 0, sizeof(buf));
				uint32 offeringId = htonl(_offeringId);
				memcpy(buf, &offeringId, sizeof(offeringId));
				_privateDataBytes.insert(_privateDataBytes.begin() + _privateDataBytes.size(), buf, buf +4);
			}
			break;
		default:
		...
		//updata ServiceGateWayDataLength;  +8  pdServiceGatewaylength and pdServiceDataLenth
		pdServiceGatewayLength = _privateDataBytes.size() - 22  + 8;
		//_privateDataBytes[18, 19 20 21];
		{
			uint8 buf[4];
