...
{   
	...
	if (NULL != fin && fin != stdin)
	 ...
	 ZQ::DSMCC::DsmccMsg::Ptr pMsg = ZQ::DSMCC::DsmccMsg::parseMessage(strBuf +0x2a, len - 0x2a, processedLength, Protocol_Tangberg);
	 ...
	 if(pMsg)
	 {
		 ...
		 pMsg = ZQ::DSMCC::DsmccMsg::parseMessage(toBuf, length, processedLength, Protocol_Tangberg);
		 ...
		 case MsgID_SetupRequest:
			 pMsg = new ClientSessionSetupRequest(tmpHeader, Protocol_Tangberg);
			 break;
		 case MsgID_SetupConfirm:
			 pMsg = new ClientSessionSetupConfirm(tmpHeader, Protocol_Tangberg);
			 break;
		 case MsgID_ReleaseRequest:                //ClientSessionReleaseRequest
			 pMsg = new ClientSessionReleaseRequest(tmpHeader, Protocol_Tangberg);
			 break;
		 case MsgID_ReleaseConfirm:    
			 pMsg = new ClientSessionReleaseConfirm(tmpHeader, Protocol_Tangberg);
			 break;
		 case MsgID_ReleaseIndication:    
			 pMsg = new ClientSessionReleaseIndication(tmpHeader, Protocol_Tangberg);
			 break;
		 case MsgID_ReleaseResponse:   
			 pMsg = new ClientSessionReleaseResponse(tmpHeader, Protocol_Tangberg);
			 break;
		 case MsgID_ProceedingIndication: 
			 pMsg = new ClientSessionProceedingIndication(tmpHeader, Protocol_Tangberg);
			 break;
		 case MsgID_InProgressRequest: 
			 pMsg = new ClientSessionInProgressRequest(tmpHeader, Protocol_Tangberg);
			 break;
		 case 0x0000:
		 default:
			 break;
