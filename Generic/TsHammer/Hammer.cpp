...
Hammer::~Hammer() {
	...
	RTSPClients::iterator client = _clients.begin();
...
void Hammer::start() {
	...
	for(uint i = 0; i < _parser.clientCount; ++i) {
		ZQ::common::RTSPClient* client = new ZQ::common::RTSPClient(
...
//		client->setTimeout(_parser.clientIdleTimeout);
	...
	ZQ::common::RTSPClient::setVerboseFlags(_parser.logger.flags);
...
ZQ::common::RTSPClient* Hammer::getClient(uint idx) const {
...
ZQ::common::Action& RequestAction::operator() (ZQ::common::Action::Context& ctx, const ZQ::common::Action::Properties& input, ZQ::common::Action::Properties& output) {
	...
	ZQ::common::RTSPClient* client = engine->getClient(seq.clientIdx);
	if(!client) {
...
ConsoleWriter::ConsoleWriter(Hammer& hammer, uint clients, uint iterations, uint loops, bool print)
...
void ConsoleWriter::printHeader() {
	if(_print) {
		fprintf(stdout, "TsHammerTest : Client[%5d]  Iteration[%5d]     Loops[%5d]\n", 
			_st.clients, _st.iterations, _st.loops);
	}
}
