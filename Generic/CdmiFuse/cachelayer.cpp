...
int MBWriter::write( size_t offset, const char* buf, size_t& size, unsigned long long originalOffset, bool bDirectWrite ) {
	...
	if( mMbi->dataSize() > 0 && size > 0 ) {
		...
		if(!intersectOrAdjacent(mMbi->dataStartPos(), mMbi->dataSize(), offset, size ) )
			bOK = false;
	}

	int rc  = -1;
	if( !(bOK && !bDirectWrite )) {
		MLOG(ZQ::common::Log::L_INFO,CLOGFMT(MBWriter,"write(), random write or flush, perform degrade write action for [%s]"),
			mMbi->toString().c_str());
...
//	MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(WriteBuffer, "putMemory() put with file[%s]"),
//			writers[0].mbi()->toString().c_str());
	...
	s3.stop();
//	MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(WriteBuffer, "putMemory() file[%s] cost[%lld/%lld/%lld]"),
//			writers[0].mbi()->toString().c_str(),s1.cost(), s2.cost(), s3.cost());
}
...
MBWriter WriteBuffer::getMemory( const MBMetaData& metadata, int  ) {
	StopWatch s1, s2, s3, s4 ,s5, s6,s7,s8,s9,s10;
	s1.start();
	s2.start();
	s3.start();
	s4.start();
	...
	s9.start();
	s10.start();
//	MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(WriteBuffer, "getMemory() get with file[%s]"),
//			metadata.toString().c_str());
	ZQ::common::MutexGuard gd(mLocker);
	s1.stop();
	MemoryBlockInfo* mbi = 0;
...
//	MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(WriteBuffer, "getMemory() file[%s] cost[%lld/%lld/%lld/%lld/%lld/%lld/%lld/%lld/%lld/%lld]"),
//			metadata.filePath.c_str(), s1.cost(), s2.cost(), s3.cost(), s4.cost(), s5.cost(), s6.cost(),
//			s7.cost(), s8.cost(), s9.cost(), s10.cost());
	return writer;
}
...
std::vector<MBWriter> WriteBuffer::getMemory( const MBMetaData& metadata ) {
	...
	s1.start();
	s2.start();
//	MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(WiteBuffer, "getMemory() with file [%s]"),
//			metadata.toString().c_str());
	ZQ::common::MutexGuard gd(mLocker);
	s1.stop();
	...
	s2.stop();
//	MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(WriteBuffer, "getMemory2() file[%s] cost[%lld/%lld]"),
//		 metadata.filePath.c_str(), s1.cost(), s2.cost());
	return writers;
}
...
bool WriteBuffer::evict( const MBMetaData& metadata, bool bForce, bool bDirect ) {
	...
	MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(WriteBuffer, "evict() with file [%s]"), metadata.toString().c_str());
	...
	s4.stop();
	MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(WriteBuffer, "evict() file[%s]  cost[%lld/%lld/%lld/%lld]"),
			metadata.toString().c_str(), s1.cost(), s2.cost(), s3.cost(), s4.cost() );
	return true;
}

bool WriteBuffer::evict( const std::vector<MemoryBlockInfo*>& mbis ) {
	assert( mbis.size() > 0 );
	MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(WriteBuffer, "evict() with file[%s]"),
			mbis[0]->toString().c_str());
	StopWatch s1, s2, s3;
	s1.start();
	s2.start();
	s3.start();
	...
	s3.stop();
	MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(WriteBuffer, "evict() file[%s] cost[%lld/%lld/%lld]"),
			mbis[0]->toString().c_str(), s1.cost(), s2.cost(), s3.cost());
	return true;
}


void WriteBuffer::flushDirect( MemoryBlockInfo* mbi ) {
	MLOG(ZQ::common::Log::L_DEBUG, CLOGFMT(WriteBuffer(), "flushDirect() file[%s]"),
			mbi->toString().c_str());
	MBWriter writer(mTank,mbi);
	size_t size  = 0;
	mFlushingCount ++;
