...
public:
	...
	CdmiFuseOps(ZQ::common::Log& log, ZQ::common::NativeThreadPool& thrdPool, const std::string& rootUrl, const std::string& userDomain, const std::string& homeContainer, const uint32 flags = 0x0f, const FuseOpsConf& conf = FuseOpsConf(), const uint retryInterval= 50);
	virtual ~CdmiFuseOps();
...
protected: // implementation of CDMI APIs
...
public:
	static bool getServerSideConfig(ZQ::common::Log& log, ZQ::common::NativeThreadPool& thrdPool, const std::string& rootURL, uint16 curlflags = 0x0f);
	...
	static int getContainer(Json::Value& result, ZQ::common::Log& log, ZQ::common::NativeThreadPool& thrdPool,const std::string &rootUrl, uint16 curlflags = 0x0f);
