...
#include "Hmac_sha1.h"  // https://code.google.com/p/winbondport/source/browse/trunk/wpa/hmac_sha1.c?r=17
#include "base_64.h"
#include <algorithm>
...
int ChildReader::searchByParent(void)
{
	...
	_fuse._log(ZQ::common::Log::L_DEBUG, CLOGFMT(ChildReader, "searchByParent() reader[%d] reading children _uri[%s] for txn[%s]"), _Id, _uri.c_str(), _txnId.c_str());
	...
	char hint[512]= "";
	snprintf(hint,  sizeof(hint) -1, CLOGFMT(ChildReader, "searchByParent() ChildReader[%s]: %s(%d)<=[%s]took %dms"),
		finalURL.c_str(), CdmiFuseOps::cdmiRetStr(cdmiRetCode), cdmiRetCode, strStatus.c_str(), (int)(ZQ::common::now() - stampStart));
   ...
   //1.2 parser cdmi response body
	...
	{	
		...
		{
			_fuse._log(ZQ::common::Log::L_ERROR, CLOGFMT(ChildReader, "searchByParent() ChildReader[%s] failed to parse response len[%d]: %s"),_uri.c_str(), strResponse.length(), reader.getFormatedErrorMessages().c_str());
			cdmiRetCode = CdmiFuseOps::cdmirc_RequestFailed;
		}
	}
	catch(std::exception& ex)
	{
		_fuse._log(ZQ::common::Log::L_ERROR, CLOGFMT(ChildReader, "searchByParent() ChildReader[%s] failed to parse response caught exception[%s]"),_uri.c_str(), ex.what());
		cdmiRetCode = CdmiFuseOps::cdmirc_RequestFailed;
	}
	catch (...)
	{
		_fuse._log(ZQ::common::Log::L_ERROR, CLOGFMT(ChildReader, "searchByParent() ChildReader[%s] failed to parse response caught unknown exception[%d]"),_uri.c_str(), SYS::getLastErr());
		cdmiRetCode = CdmiFuseOps::cdmirc_RequestFailed;
	}
	...
	for (Json::Value::iterator it = children.begin(); !_bQuit && it != children.end(); it++)
	{
		...
		if (!JSON_HAS(child, "metadata") || !_fuse.dataObjMetadataToFstat(name, child["metadata"], fi))
		{
			...
			_fuse._log(ZQ::common::Log::L_ERROR, CLOGFMT(ChildReader, "searchByParent() parent[%s] reader[%d] of txn[%s] got illegal child: %s"), _parentName.c_str(), _Id, _txnId.c_str(), childstr.c_str());
			name = "";
		}
	...
	_fuse._log((c < (_children2read.size()*3/4)) ? ZQ::common::Log::L_WARNING : ZQ::common::Log::L_INFO, CLOGFMT(ChildReader, "searchByParent() parent[%s] reader[%d] has read %d of %d children for txn[%s], subdirs[%d], took %dmsec"),
		_parentName.c_str(), _Id, c, (int)_children2read.size(), _txnId.c_str(), cSubDirs, (int)(ZQ::common::now() - stampStart));
	return 0;
}

int ChildReader::readByEnumeration(void)
{
	_fuse._log(ZQ::common::Log::L_DEBUG, CLOGFMT(ChildReader, "readByEnumeration() reader[%d] reading %d children for txn[%s]"), _Id, _children2read.size(), _txnId.c_str());
	int c =0;
	...
	_fuse._log(ZQ::common::Log::L_DEBUG, CLOGFMT(ChildReader, "readByEnumeration() reader[%d] read %d of %d children for txn[%s]"), _Id, c, _children2read.size(), _txnId.c_str());
	return 0;
}
...
void ChildReader::final(int retcode, bool bCancelled)
{
	try {
		...
		_fuse._log(ZQ::common::Log::L_DEBUG, CLOGFMT(ChildReader, "reader[%d] for txn[%s] cancelled[%c]"), _Id, _txnId.c_str(), bCancelled?'T':'F');
...
CdmiFuseOps::CdmiFuseOps(ZQ::common::Log& log, ZQ::common::NativeThreadPool& thrdPool, const std::string& rootUrl, const std::string& userDomain, const std::string& homeContainer, const uint32 flags, const FuseOpsConf& conf, const uint retryInterval )
...
{  
	_log(ZQ::common::Log::L_DEBUG, CLOGFMT(CdmiFuseOps, "nonCdmi_ReadDataObject[%s]"), uri.c_str());
	...
	_log(ZQ::common::Log::L_INFO, CLOGFMT(CdmiFuseOps, "nonCdmi_ReadDataObject[%s], offset[%lld] len[%u]"), uri.c_str(), startOffset, sizeTotal);
...
bool CdmiFuseOps::generateSignature(std::string& signature, const std::string& uri, const std::string& contentType, ZQ::common::CURLClient::HTTPMETHOD method, const std::string& xAquaDate)
{
	...
	{
		_log(ZQ::common::Log::L_ERROR, CLOGFMT(CdmiFuseOps, "generateSignature() invalid input parameters secretKey[%s] or uri[%s] or xAquaDate[%s]"),
	...
	if(!bret)
	{
		_log(ZQ::common::Log::L_ERROR, CLOGFMT(CdmiFuseOps, "generateSignature() failed to encode to base64 with secretKey[%s]"), _secretKeyId.c_str());
		_secretKeyId = "";
		return false;
	}
	...
	char buf[512]= "";
	snprintf(buf,  sizeof(buf) -1, CLOGFMT(CdmiFuseOps, "generateSignature secretKey[%s] signature:"), _secretKeyId.c_str());
	_log.hexDump(ZQ::common::Log::L_DEBUG, signature.c_str(), (int)outBufLen, buf, true);

//	_log(ZQ::common::Log::L_INFO, CLOGFMT(CdmiFuseOps, "generateSignature secretKey[%s] signature[%s]"),_secretKeyId.c_str(), signature.c_str());
...
bool CdmiFuseOps::getServerSideConfig(ZQ::common::Log& log, ZQ::common::NativeThreadPool& thrdPool,const std::string& rootUrl, uint16 curlflags)
{
	ZQ::common::MutexGuard g(_lkLogin);

	log(ZQ::common::Log::L_DEBUG, CLOGFMT(CdmiFuseOps, "getServerSideConfig()populating the server configuration from %s"), rootUrl.c_str());
	...
	if(!cdmiClient->init())
	{
		log(ZQ::common::Log::L_ERROR, CLOGFMT(CdmiFuseOps, "getServerSideConfig[%s] failed to init libcurl"), url.c_str());
		cdmiClient = NULL;
		return false;
	}
	...
	if (!pRequestBody || !pResponseBody)
	{
		log(ZQ::common::Log::L_ERROR,  CLOGFMT(CdmiFuseOps, "getServerSideConfig() [%s] failed to allocate BufferList for request/response"), url.c_str());
		cdmiClient = NULL;
		return false;
	}
	...
	if (reqBodyLen <0)
	{
		log(ZQ::common::Log::L_ERROR,  CLOGFMT(CdmiFuseOps, "getServerSideConfig() [%s] failed to set request boby"), url.c_str());
		cdmiClient = NULL;
		return false;
	}
	...
	if (respBodyLen <0)
	{
		log(ZQ::common::Log::L_ERROR, CLOGFMT(CdmiFuseOps, "getServerSideConfig() [%s] failed to set response buffer"), url.c_str());
		cdmiClient = NULL;
		return false;
	}
	...
	if(!bRet)
	{
		log(ZQ::common::Log::L_ERROR, CLOGFMT(CdmiFuseOps, "getServerSideConfig() [%s] request failed took %dms"), url.c_str(), (int)(ZQ::common::now() - lStartTime));
		cdmiClient = NULL;
		return false;
	}
	...
	if (!CdmiRet_SUCC(cdmiRetCode))
	{
		log(ZQ::common::Log::L_ERROR, CLOGFMT(CdmiFuseOps, "getServerSideConfig() [%s] failed to get root URL: %s(%d)<=[%s]: %s took %dms"),
			url.c_str(), cdmiRetStr(cdmiRetCode), cdmiRetCode, strStatus.c_str(), cdmiClient->getErrorMessage().c_str(), (int)(ZQ::common::now() - lStartTime));
		cdmiClient = NULL;
		return false;
	}
...
*/ 
	...
	char buf[512]= "";
	snprintf(buf,  sizeof(buf) -1, CLOGFMT(CdmiFuseOps, "getServerSideConfig[%s]: %s(%d)<=[%s] took %dms"), 
		url.c_str(), cdmiRetStr(cdmiRetCode), cdmiRetCode, strStatus.c_str(), (int)(ZQ::common::now() - lStartTime));
	log.hexDump(ZQ::common::Log::L_INFO, (char*)strResponse.c_str(), (int)dataSize, buf, true);
	...
	{	
		if(!strResponse.empty() && !reader.parse(strResponse, result))
		{
			log(ZQ::common::Log::L_ERROR, CLOGFMT(CdmiFuseOps, "getServerSideConfig[%s] failed to parse response len[%d] took %dms: %s"),
				url.c_str(), strResponse.length(), (int)(ZQ::common::now() - lStartTime), reader.getFormatedErrorMessages().c_str());
			return false;
		}
		/*       
		...
		*/
		...
		{
			log(ZQ::common::Log::L_ERROR, CLOGFMT(CdmiFuseOps, "getServerSideConfig[%s]Auqa server missed some parameter"), url.c_str());
			return false;
		}
		...
		log(ZQ::common::Log::L_INFO, CLOGFMT(CdmiFuseOps, "getServerSideConfig[%s]rootURI[%s]httpport[%d]httpsPort[%d]defaultDomainURI[%s]frontendIPs[%s] "),
			url.c_str(),_serverSideParams.rootURI.c_str(), _serverSideParams.portHTTP, _serverSideParams.portHTTPS, _serverSideParams.domainURI.c_str(), frontendIps.c_str());

		return true;
	}
	catch(std::exception& ex)
	{
		log(ZQ::common::Log::L_ERROR, CLOGFMT(CdmiFuseOps, "getServerSideConfig[%s] failed to parse response caught exception[%s]"),url.c_str(), ex.what());
	}
	catch (...)
	{
		log(ZQ::common::Log::L_ERROR, CLOGFMT(CdmiFuseOps, "getServerSideConfig[%s] failed to parse response caught unknown exception[%d]"),url.c_str(), SYS::getLastErr());
	}
	return false;
}

int CdmiFuseOps::getContainer(Json::Value& result,ZQ::common::Log& log, ZQ::common::NativeThreadPool& thrdPool,const std::string &rootUrl , uint16 curlflags)
{
	int64 lStartTime = ZQ::common::now();
	ZQ::CDMIClient::CDMIHttpClient::Ptr cdmiClient = new ZQ::CDMIClient::CDMIHttpClient((char*)rootUrl.c_str(), log, thrdPool, curlflags, ZQ::common::CURLClient::HTTP_GET); 
...
std::string CdmiFuseOps::getServerIp(bool next, bool forceReadConfig)
{
	if(forceReadConfig || _serverSideParams.portHTTP <= 0 || _serverSideParams.portHTTPS <= 0 || _serverSideParams.rootURI.empty())
	{
		...
		if(!getServerSideConfig(_log, _thrdPool, _rootUrl, (_flags>>16)))
			return "";
							...
							ZQ::common::CURLClient::HTTPMETHOD method, int clientFlags, std::string uriAffinity)
{
	...
	responseHeaders = cdmiClient->getResponseHeaders();
	stampNow = ZQ::common::now();
	_log(ZQ::common::Log::L_DEBUG, CLOGFMT(CdmiFuseOps, "%s[%s]@%s by[%s]: %s(%d)<=[%s]: %s took %dms, txn %dmsec; content-len[%d]"), purposeStr, finalURL.c_str(), stageName, cliDesc.c_str(),
		cdmiRetStr(cdmiRetCode), cdmiRetCode, respStatusLine.c_str(), cdmiClient->getErrorMessage().c_str(),(int)(stampNow- lStartTime), (int)(stampNow - lTxnStartTime), pResponseBody->length());
...
bool CdmiFuseOps::readFileInfoFromCache(const std::string& filepath, FileInfo& fileInfo)
{
	...
	if (_fuseOpsConf.attrCache_TTLsec >0)
		stampExpired = ZQ::common::now() - _fuseOpsConf.attrCache_TTLsec *1000;

	ZQ::common::MutexGuard g(_lkFileInfos);
	FileInfoMap::iterator itFileInfo = _fileInfos.find(filepath);
	bool bTakingCached = (_fileInfos.end() != itFileInfo);
	...
	// ignore the cached info if it is too old
	if (itFileInfo->second.stampInfoKnown < stampExpired)
		bTakingCached = false;

	if (!bTakingCached) // remove the expired file info from the cache
	{
	...
	_log(ZQ::common::Log::L_DEBUG, CLOGFMT(readFileInfoFromCache, "file[%s] found in cached info"), filepath.c_str());
...
#ifdef _DEBUG
	_log(ZQ::common::Log::L_DEBUG, CLOGFMT(cacheFileInfo, "path[%s] info cached"), pathname.c_str());
#endif // _DEBUG
}
...
#ifdef _DEBUG
	_log(ZQ::common::Log::L_DEBUG, CLOGFMT(uncacheFileInfo, "path[%s] info removed"), pathname.c_str());
#endif // _DEBUG
}
...
int CdmiFuseOps::getFileInfo(const std::string& pathname, FileInfo& fi, bool skipGuessContainer)
{
	...
	{
		_log(LOGL_COND_WARN(!CdmiRet_ClientErr(cdmirc)), CLOGFMT(GetFileInfo, "path[%s] uri[%s] failed to get fsta: %s(%d)"), pathname.c_str(), uri.c_str(), cdmiRetStr(cdmirc), cdmirc);
		return cdmirc;
	}
	...
	if (CdmiRet_SUCC(cdmirc))
	{
		...
		{
			_log(ZQ::common::Log::L_WARNING, CLOGFMT(GetFileInfo, "path[%s] uri[%s] no metadata received"), pathname.c_str(), uri.c_str());
			return cdmirc_ServerError;
		}

		if (!dataObjMetadataToFstat(pathname, result["metadata"], fi))
		{
			_log(ZQ::common::Log::L_WARNING, CLOGFMT(GetFileInfo, "path[%s] uri[%s] failed to convert fstat"), pathname.c_str(), uri.c_str());
			return cdmirc_ServerError;
		}
	}
...
/*
	...
	{
		_log(ZQ::common::Log::L_DEBUG, CLOGFMT(GetFileInfo, "path[%s] trying cdmi_ReadContainer(%s) to touch"), pathname.c_str(), uri.c_str());
...
//			FileInfoByHandle->dwFileAttributes = FILE_ATTRIBUTE_DIRECTORY;
	...
	if (bIsACertainDir)
	{
		_log(LOGL_COND_WARN(!CdmiRet_ClientErr(cdmirc)), CLOGFMT(GetFileInfo, "path[%s] cdmi_ReadContainer(%s) as folder failed: %s(%d)"), pathname.c_str(), uri.c_str(), cdmiRetStr(cdmirc), cdmirc);
		return cdmirc;
	}
	...
	_log(ZQ::common::Log::L_DEBUG, CLOGFMT(GetFileInfo, "path[%s] calling cdmi_ReadDataObject(%s) to read attributes"), pathname.c_str(), uri.c_str());
	std::string tmp;
	cdmirc = cdmi_ReadDataObject(result, uri +"?metadata", tmp);

	if (!CdmiRet_SUCC(cdmirc))
	{
		_log(LOGL_COND_WARN(!CdmiRet_ClientErr(cdmirc)), CLOGFMT(GetFileInfo, "path[%s] cdmi_ReadDataObject(%s) as file failed: %s(%d)"), pathname.c_str(), uri.c_str(), cdmiRetStr(cdmirc), cdmirc);
		return cdmirc;
	}

	if (!JSON_HAS(result, "metadata"))
	{
		_log(ZQ::common::Log::L_WARNING, CLOGFMT(GetFileInfo, "path[%s] cdmi_ReadDataObject(%s) no metadata received"), pathname.c_str(), uri.c_str());
		return cdmirc_ServerError;
	}

	if (!dataObjMetadataToFstat(result["metadata"], fi))
	{
		_log(ZQ::common::Log::L_WARNING, CLOGFMT(GetFileInfo, "path[%s] cdmi_ReadDataObject(%s) failed to convert fstat"), pathname.c_str(), uri.c_str());
		return cdmirc_ServerError;
	}
*/
...
void CdmiFuseOps::cacheChildren(const std::string& pathname, DirChildren& children)
{
	...
	children.stampLastSearch = ZQ::common::now();
	ZQ::common::MutexGuard g(_lkFileInfos);
...
CdmiFuseOps::DirChildren CdmiFuseOps::getCachedChildren(const std::string& pathname)
{
	...
	const static DirChildren NIL = {0,};
	int64 stampExpired =0;
	...
	if (itchdr->second.stampLastSearch < stampExpired)
	{ 
...
*/
	...
	client = new ZQ::CDMIClient::CDMIHttpClient((char*)forURL.c_str(), _log, _thrdPool, curlflags, method);
/*
#ifdef CURL_INSTANCE_KEEPALIVE
	if (client)
	{
...
/*	if(CheckMaster() == cdmirc_OK )
	...
	int64 lStartTime = ZQ::common::now();
	ZQ::CDMIClient::CDMIHttpClient::Ptr cdmiClient = new ZQ::CDMIClient::CDMIHttpClient((char*)url.c_str(), _log, _thrdPool, (_flags>>16), ZQ::common::CURLClient::HTTP_GET); 
	if(!cdmiClient)
	{
		cdmiClient = NULL;
		return false;
