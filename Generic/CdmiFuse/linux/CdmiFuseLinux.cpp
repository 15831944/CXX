...
long long CdmiFuseFS::addFd(CdmiFuseOpLinux* op, bool disableCache )
{
	long long id = 0;
	ZQ::common::MutexGuard gd(mLocker);
	id = mBaseOpId ++;
	
	struct FileHandleInfo fhinfo;
	fhinfo.ops = op;
	fhinfo.disableCache = disableCache;

	mOps[ id ] = fhinfo;
	(*mLogger)(ZQ::common::Log::L_DEBUG, "add file handle [%lld]",id);
	return id;
}

void CdmiFuseFS::removeFd( long long id )
{
	ZQ::common::MutexGuard gd(mLocker);
	mOps.erase( id );
	(*mLogger)(ZQ::common::Log::L_DEBUG, "remove file handle [%lld]",id);
}
...
CdmiFuseOpLinux::CdmiFuseOpLinux( ZQ::common::Log& logger, 
		...
		long long id)
	:CdmiFuseOps(logger, pool, cdmiRootUrl, userDomain, homeContainer,conf.fuseFlag,conf),
	mOpId(id),
	mFs(fs)
{
}
...
int CdmiFuseOpLinux::getattr( const char* opPath, struct stat *st )
{
	...
	CdmiFuseOps::CdmiRetCode code = getFileInfo( opPath, info, false );
...
{
	MLOG(ZQ::common::Log::L_DEBUG,LOGFMT(chmod, "chmod() path[%s] changing to mode[%o] "), opPath, mode );
	mode_t mask = 0x7;
...
int CdmiFuseOpLinux::open( const char* opPath, struct fuse_file_info* info )
{
	...
	CdmiRetCode retCode = getFileInfo( opPath, fi, false ); // cdmi_ReadDataObject( val, toUrl( opPath ) + "?metadata", location );
	...
	if( sysCode == 0 )
	{
		...
		info->fh = mFs.addFd(this, disableCache);
	...
	fuse_context* ctx = fuse_get_context();
	MLOG(ZQ::common::Log::L_INFO, LOGFMT(open, "caller[%d] open file, got [%s], keep_cache[%s], took [%lld]us"),
			(int)ctx->pid, cdmiRetStr(retCode), info->keep_cache ? "true":"false", ts.stop() );
	return sysCode;
}
...
int CdmiFuseOpLinux::read( const char* opPath, char* buf, size_t bufSize, off_t offset, struct fuse_file_info *info )
{
	...
	MLOG(ZQ::common::Log::L_INFO, LOGFMT(read, "has read offset[%ld +%u]bytes by fd[%llu]: retCode [%s], took [%lld]us"),
			offset, bufLen, info->fh, cdmiRetStr(retCode), ts.stop() );
...
int CdmiFuseOpLinux::opendir(const char* opPath, struct fuse_file_info* info )
{
	...
	MLOG(ZQ::common::Log::L_INFO, LOGFMT(opendir, "got[%s], caller[%d], took[%lld]us"), cdmiRetStr(retCode), (int)ctx->pid, ts.stop());
...
#	define MIN_FILE_PER_BATCH (50)
	...
	info->fh = mFs.addFd(this, false);
	if (dcNew._children.size() >0)
	{
		cacheChildren(opPath, dcNew);
		MLOG(ZQ::common::Log::L_DEBUG, LOGFMT(opendir, "cached children of dir[%s]"), opPath);
...
int CdmiFuseOpLinux::readdir( const char* opPath, void* buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info* info )
{
	...
	size_t count = 0 ;
	CdmiFuseFS::DIRENTVECTOR dirCache = mFs.getDirCache( info->fh );
	std::vector<std::string>::const_iterator it = dirCache.begin();
	for( ; it != dirCache.end(); it ++ )
	{
		if( filler(buf, it->c_str(),0,0) == 1)
		{
