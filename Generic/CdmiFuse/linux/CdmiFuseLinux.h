...
struct FileHandleInfo {
	...
	FileHandleInfo()
	:ops(0),disableCache(false),dirty(false), stampLastDirty(0)
	{
	}
};
...
public:
	...
	typedef std::vector<std::string> DIRENTVECTOR;

public:

	const FuseConfig& config() const{ return mConfig; }

	long long	addFd(CdmiFuseOpLinux* op, bool disableCache );
	void		removeFd( long long id );
