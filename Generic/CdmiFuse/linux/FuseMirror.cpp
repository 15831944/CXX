...
public:
	MirrorCacheTank( ZQ::common::NativeThreadPool& readPool, ZQ::common::NativeThreadPool& writePool, ZQ::common::Log& logger)
		:DataTank(readPool, writePool, logger) {
	}
...
void* mirror_init(struct fuse_conn_info *conn)
{
	...
	if( enableCache) {
		...
		cache = new MirrorCacheTank(*pool,*writePool, *pLog );
		cache->createTank( conf.fuseOpsConf.tankConf );
	}
